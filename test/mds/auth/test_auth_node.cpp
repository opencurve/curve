...
TEST_F(TestAuthNode, TestAddKey) {
    ...
    ASSERT_EQ(AuthStatusCode::AUTH_DECRYPT_FAILED, authNode_->AddKey("key"));
    ...
    EXPECT_CALL(*authStorage_, StorageKey(_))
        .WillOnce(Return(false));
    ASSERT_EQ(AuthStatusCode::AUTH_STORE_KEY_FAILED, authNode_->AddKey(key));

    // key is correct, store success
    EXPECT_CALL(*authStorage_, StorageKey(_))
        .WillOnce(Return(true));
    ASSERT_EQ(AuthStatusCode::AUTH_OK, authNode_->AddKey(key));
}
...
TEST_F(TestAuthNode, TestUpdateKey) {
    // error key, decrypt failed
    ASSERT_EQ(AuthStatusCode::AUTH_DECRYPT_FAILED, authNode_->UpdateKey("key"));
    ...
    EXPECT_CALL(*authStorage_, StorageKey(_))
        .WillOnce(Return(false));
    ASSERT_EQ(AuthStatusCode::AUTH_STORE_KEY_FAILED, authNode_->UpdateKey(key));

    // key is correct, store success
    EXPECT_CALL(*authStorage_, StorageKey(_))
        .WillOnce(Return(true));
    ASSERT_EQ(AuthStatusCode::AUTH_OK, authNode_->UpdateKey(key));
