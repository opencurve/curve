        ...
        {
            {
                ...
                LOG(INFO) << "test1......";
                EXPECT_CALL(*mockEtcdClient_, GetCurrentRevision(_)).WillOnce(Return(EtcdErrCode::EtcdCanceled));
                ASSERT_EQ(-1, allocStatistic_->Init());
            }
            {
                // 2. Failed to obtain the alloc size corresponding to the existing logicalPool
                LOG(INFO) << "test2......";
                EXPECT_CALL(*mockEtcdClient_, GetCurrentRevision(_)).WillOnce(Return(EtcdErrCode::EtcdOK));
            ...
            }
            {
                ...
                EXPECT_CALL(*mockEtcdClient_, GetCurrentRevision(_)).WillOnce(DoAll(SetArgPointee<0>(2), Return(EtcdErrCode::EtcdOK)));
                EXPECT_CALL(*mockEtcdClient_,
                            List(SEGMENTALLOCSIZEKEY, SEGMENTALLOCSIZEKEYEND,
                                 Matcher<std::vector<std::string> *>(_)))
                    .WillOnce(
