...
TEST_F(NameSpaceServiceTest, test1) {
    ...
    // begin session test, at the beginning of the session test, there are /file1,/file2, and/file4
...
TEST_F(NameSpaceServiceTest, deletefiletests) {
    ...
    // 1. If the file has a snapshot, deleting the file returns kFileUnderSnapShot
    cntl.Reset();
    CreateSnapShotRequest snapshotRequest;
    CreateSnapShotResponse snapshotResponses;
    snapshotRequest.set_filename("/file1");
    ...
    // 2. If there are files in the directory, deleting the directory returns kDirNotEmpty
    cntl.Reset();
