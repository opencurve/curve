...
TEST_F(RepoTest, testTransaction) {
    ZoneRepo z1(1, "test", 1, "first zone");
    CopySetRepo c1(1, 1, "1-2-3");
    ServerRepo s1(1,
                  "curve-nos1.dg.163.org",
                  "10.172.168.1",
                  "192.168.2.1",
    ...
    // 1. test transaction ok
    // start tansaction
    ASSERT_EQ(OperationOK, repo->SetAutoCommit(true));

    // insert data
    ASSERT_EQ(OperationOK, repo->InsertCopySetRepo(c1));
    ASSERT_EQ(OperationOK, repo->InsertZoneRepo(z1));
    ASSERT_EQ(OperationOK, repo->InsertServerRepo(s1));

    // commit
    ASSERT_EQ(OperationOK, repo->Commit());

    // query res OK
    CopySetRepo cres(0, 0);
    ZoneRepo zres(0);
    ServerRepo sres(0);
    ASSERT_EQ(OperationOK,
              repo->QueryCopySetRepo(c1.copySetID, c1.logicalPoolID, &cres));
    ASSERT_TRUE(c1 == cres);
    ASSERT_EQ(OperationOK, repo->QueryZoneRepo(z1.zoneID, &zres));
    ASSERT_TRUE(z1 == zres);
    ASSERT_EQ(OperationOK, repo->QueryServerRepo(s1.serverID, &sres));
    ASSERT_TRUE(s1 == sres);

    // 2. test transaction fail in one step
    // start trasaction
    ASSERT_EQ(OperationOK, repo->SetAutoCommit(false));

    // insert duplicate server:serverID=1, insert fail
    ZoneRepo z2(z1);
    z2.zoneID = 2;
    ASSERT_EQ(OperationOK, repo->InsertZoneRepo(z2));
    ASSERT_EQ(SqlException, repo->InsertServerRepo(s1));

    // rollback ok
    ASSERT_EQ(OperationOK, repo->RollBack());

    // query no result
    zres.zoneID = 0;
    ASSERT_EQ(OperationOK, repo->QueryZoneRepo(z2.zoneID, &zres));
    ASSERT_EQ(0, zres.zoneID);

    // 3. test transaction ok, but rollback
    ASSERT_EQ(OperationOK, repo->SetAutoCommit(false));

    // insert ok and query ok
    ASSERT_EQ(OperationOK, repo->InsertZoneRepo(z2));
    zres.zoneID = 0;
    ASSERT_EQ(OperationOK, repo->QueryZoneRepo(z2.zoneID, &zres));
    ASSERT_TRUE(z2 == zres);
    ...
    // 4. test transaction close, rollback do nothing
    ZoneRepo z3(z1);
    z3.zoneID = 3;
    ASSERT_EQ(OperationOK, repo->SetAutoCommit(true));
    ASSERT_EQ(OperationOK, repo->InsertZoneRepo(z3));
    ASSERT_EQ(OperationOK, repo->RollBack());
    zres.zoneID = 0;
    ASSERT_EQ(OperationOK, repo->QueryZoneRepo(z3.zoneID, &zres));
    ASSERT_EQ(z3.zoneID, zres.zoneID);

    // 5. test transaction func fail
    repo->getDataBase()->conn_->close();
    ASSERT_EQ(SqlException, repo->SetAutoCommit(true));
    ASSERT_EQ(SqlException, repo->Commit());
    ASSERT_EQ(SqlException, repo->RollBack());
}
}  // namespace repo
}  // namespace curve
