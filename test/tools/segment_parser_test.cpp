...
class SetmentParserTest : public ::testing::Test {
 protected:
    ...
    void PackHeader(const EntryHeader& header, char* buf,
        ...
        const uint32_t meta_field =
            (header.type << 24) | (header.checksum_type << 16);
...
TEST_F(SetmentParserTest, Init) {
    ...
    EXPECT_CALL(*localFs_, Fstat(_, _)).Times(1).WillOnce(Return(-1));
    ASSERT_EQ(-1, parser.Init(fileName));

    // 3. Success
    EXPECT_CALL(*localFs_, Fstat(_, _)).Times(1).WillOnce(Return(0));
    ASSERT_EQ(0, parser.Init(fileName));

    // 4. De-initialization
    EXPECT_CALL(*localFs_, Close(_)).Times(1).WillOnce(Return(0));
    parser.UnInit();
}
...
TEST_F(SetmentParserTest, GetNextEntryHeader) {
    ...
    EXPECT_CALL(*localFs_, Open(_, _)).Times(1).WillOnce(Return(1));
    ...
    EXPECT_CALL(*localFs_, Read(_, _, _, ENTRY_HEADER_SIZE))
        .Times(1)
        .WillOnce(DoAll(
            SetArrayArgument<1>(header_buf, header_buf + ENTRY_HEADER_SIZE),
            Return(24)));
    ASSERT_FALSE(parser.GetNextEntryHeader(&header2));
    ASSERT_FALSE(parser.SuccessfullyFinished());
