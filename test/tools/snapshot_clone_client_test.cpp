...
class SnapshotCloneClientTest : public ::testing::Test {
 protected:
    void SetUp() { metricClient_ = std::make_shared<MockMetricClient>(); }

    void TearDown() { metricClient_ = nullptr; }
...
TEST_F(SnapshotCloneClientTest, GetActiveAddr) {
    ...
    SnapshotCloneClient client(metricClient_);
    ASSERT_EQ(
        0, client.Init("127.0.0.1:5555,127.0.0.1:5556,127.0.0.1:5557", "9091"));
    EXPECT_CALL(*metricClient_, GetMetric(_, _, _))
        .Times(4)
        .WillOnce(DoAll(SetArgPointee<2>("active"), Return(MetricRet::kOK)))
        .WillOnce(DoAll(SetArgPointee<2>("active"), Return(MetricRet::kOK)))
        .WillRepeatedly(
            DoAll(SetArgPointee<2>("standby"), Return(MetricRet::kOK)));
    std::vector<std::string> activeAddr = client.GetActiveAddrs();
    ASSERT_EQ(1, activeAddr.size());
    ASSERT_EQ("127.0.0.1:5555", activeAddr[0]);

    // There is a dummyserver displaying active, and the service port access
    // failed
    EXPECT_CALL(*metricClient_, GetMetric(_, _, _))
        .Times(4)
        .WillOnce(DoAll(SetArgPointee<2>("active"), Return(MetricRet::kOK)))
    ...
    EXPECT_CALL(*metricClient_, GetMetric(_, _, _))
        .Times(5)
        .WillOnce(DoAll(SetArgPointee<2>("standby"), Return(MetricRet::kOK)))
        .WillRepeatedly(
            DoAll(SetArgPointee<2>("active"), Return(MetricRet::kOK)));
    activeAddr = client.GetActiveAddrs();
...
TEST_F(SnapshotCloneClientTest, GetOnlineStatus) {
    ...
    ASSERT_EQ(
        0, client.Init("127.0.0.1:5555,127.0.0.1:5556,127.0.0.1:5557", "9091"));
    // One online, one failed to obtain metric, and one did not match the listen
    // addr
    EXPECT_CALL(*metricClient_, GetConfValueFromMetric(_, _, _))
        .Times(3)
        .WillOnce(
            DoAll(SetArgPointee<2>("127.0.0.1:5555"), Return(MetricRet::kOK)))
        .WillOnce(
            DoAll(SetArgPointee<2>("127.0.0.1:5557"), Return(MetricRet::kOK)))
        .WillOnce(Return(MetricRet::kNotFound));
