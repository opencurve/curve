...
TEST(MDSChangeTest, MDSFailoverTest) {
    ...
    // Scenario 1: mds0, 1, 2, currentworkindex=0, mds0, mds1, and mds2 are all
    // down,
    //        All RPCs sent to them are returned as EHOSTDOWN, resulting in
    //        upper level clients constantly switching to mds and retrying
    //        Continue according to 0-->1-->2
    //        Every time rpc returns -EHOSTDOWN, it will directly trigger RPC
    //        switching. The final currentworkindex did not switch
    ...
    // This retry is a polling retry, and the number of retries per mds should
    // be close to and not exceed the total number of mds
    ...
    // Scenario 2: mds0, 1, 2, currentworkindex = 0, mds0 goes down, and it will
    // be working at this time
    //       Mds index switches to index2, and it is expected that the client
    //       will directly switch to index2 after retrying with index = 0 At
    //       this point, mds2 directly returns OK and rpc stops trying again.
    //       Expected client to send a total of two RPCs, one to mds0 and the
    //       other to mds2, skipping the middle mds1。
    mds0RetryTimes = 0;
    mds1RetryTimes = 0;
    mds2RetryTimes = 0;
    auto task2 = [&](int mdsindex, uint64_t rpctimeoutMS,
        ...
        if (mdsindex == 2) {
            mds2RetryTimes++;
            // If OK is returned this time, then RPC should have succeeded and
            // will not try again
    ...
    //      After switching to 2, mds1 resumed, and then switched to mds1, and
    //      the rpc was successfully sent. At this point, the switching order is
    //      1->2->0, 1->2->0, 1.
    mds0RetryTimes = 0;
    mds1RetryTimes = 0;
    mds2RetryTimes = 0;
    ...
    auto task3 = [&](int mdsindex, uint64_t rpctimeoutMS,
        ...
        if (mdsindex == 1) {
            mds1RetryTimes++;
            // When retrying on mds1 for the third time, success is returned
            // upwards and the retry is stopped
            if (mds1RetryTimes == 3) {
                return LIBCURVE_ERROR::OK;
            }
            return -ECONNREFUSED;
    ...
    // Scenario 4: mds0, 1, 2, currentWorkindex = 0, but the rpc request to mds1
    // consistently times out
    //       The final result returned by rpc is timeout
    //      For timeout mds nodes, they will continuously retry
    //      mds.maxFailedTimesBeforeChangeMDS and switch Current
    ...
    auto task4 = [&](int mdsindex, uint64_t rpctimeoutMS,
        ...
        if (mdsindex == 0) {
            mds0RetryTimes++;
            return mds0RetryTimes % 2 == 0 ? -brpc::ERPCTIMEDOUT : -ETIMEDOUT;
        }
    ...
    // Scenario 6: mds keeps returning EHOSTDOWN during the retry process, with
    // a total of 5 retries
    rpcexcutor.SetCurrentWorkIndex(0);
    int calledTimes = 0;
    ...
    // In each hostdown situation, sleep for 10ms and the total retry time is
    // 5s, so the total number of retries is less than or equal to 500 times In
    // order to minimize false positives, 10 redundant attempts were added
...
std::string mdsMetaServerAddr =
    "127.0.0.1:9903,127.0.0.1:9904,127.0.0.1:9905";                  // NOLINT
