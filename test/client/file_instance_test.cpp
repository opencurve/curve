...
TEST(FileInstanceTest, CommonTest) {
    ...
    // user info invlaid
    FileInstance fi;
    ASSERT_FALSE(fi.Initialize("/test", mdsclient, UserInfo{},
            CURVE_FORCE_WRITE, FileServiceOption{}));

    // mdsclient is nullptr
    FileInstance fi2;
    ASSERT_FALSE(fi2.Initialize("/test", nullptr, userInfo, CURVE_FORCE_WRITE,
                                FileServiceOption{}));
    ...
    ASSERT_FALSE(
        fi3.Initialize("/test", mdsclient, userInfo, CURVE_FORCE_WRITE, opts));

    // readonly
    FileInstance fi4;
    ASSERT_TRUE(fi4.Initialize("/test", mdsclient, userInfo, CURVE_FORCE_WRITE,
...
TEST(FileInstanceTest, OpenReadonlyAndDiscardTest) {
    ...
    ASSERT_TRUE(
        instance.Initialize("/FileInstanceTest-OpenReadonlyAndDiscardTest",
                            mdsclient, userInfo, CURVE_FORCE_WRITE, opt, true));
