...
TEST_F(ChunkServerClientTest, UpdateFileEpochSuccess) {
    ...
    CopysetPeerInfo cs;
    cs.chunkserverID = 1;
    cs.internalAddr = ChunkServerAddr(EndPoint(listenAddr_));
    uint64_t fileId = 1;
    uint64_t epoch = 1;
...
TEST_F(ChunkServerClientTest, UpdateFileEpochSuccessUsingExternalIp) {
    ...
    CopysetPeerInfo cs;
    cs.chunkserverID = 1;
    cs.internalAddr = ChunkServerAddr(EndPoint(std::string("notexist")));
    cs.externalAddr = ChunkServerAddr(EndPoint(listenAddr_));
    uint64_t fileId = 1;
    uint64_t epoch = 1;
    auto tracker = std::make_shared<TaskTracker>();
    FakeUpdateFileEpochClosure *closure = new FakeUpdateFileEpochClosure();
