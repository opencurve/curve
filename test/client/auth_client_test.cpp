...
TEST_F(AuthClientTest, TestGetToken_failed1) {
    // case1: cache is empty and refresh rpc cntl failed
    ...
    ASSERT_FALSE(AuthClient::GetInstance().GetToken(serverId, &token));
}

TEST_F(AuthClientTest, TestGetToken_failed2) {
    // case2: cache is empty, refresh failed
    std::string serverId = "mds";
    Token token;
...
TEST_F(AuthClientTest, TestGetToken_failed3) {
    ...
    EXPECT_CALL(mockAuthService_, GetTicket(_, _, _, _))
        .WillOnce(DoAll(
            ...
            Invoke(FakeRpcService<GetTicketRequest, GetTicketResponse>)));
    ASSERT_FALSE(AuthClient::GetInstance().GetToken(serverId, &token));
    EXPECT_EQ(1, AuthClient::GetInstance().MapSize());
}
...
TEST_F(AuthClientTest, TestGetToken_success) {
    ...
    std::thread t1([&]() {
        ASSERT_TRUE(AuthClient::GetInstance().GetToken(serverId, &token));
    });
    std::thread t2([&]() {
        ASSERT_TRUE(AuthClient::GetInstance().GetToken(serverId, &token));
    });
    ...
    // get from cache
    ASSERT_TRUE(AuthClient::GetInstance().GetToken(serverId, &token));
    ASSERT_EQ(1, AuthClient::GetInstance().MapSize());
