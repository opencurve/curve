...
TEST_F(CopysetClientTest, normal_test) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        copysetClient.WriteChunk(logicPoolId, copysetId, chunkId, 0,
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        copysetClient.ReadChunk(logicPoolId, copysetId, chunkId, sn,
...
TEST_F(CopysetClientTest, read_snapshot_error_test) {
    ...
    /* 非法参数 */
    {
        ...
        copysetClient.ReadChunkSnapshot(logicPoolId, copysetId, chunkId,
...
TEST_F(CopysetClientTest, delete_snapshot_error_test) {
    ...
    /* 非法参数 */
    {
        ...
        copysetClient.DeleteChunkSnapshot(logicPoolId, copysetId,
                                          chunkId, sn, reqDone, 0);
        cond.Wait();
        ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_INVALID_REQUEST,
                  reqDone->GetErrorCode());
    }
    ...
    }
    {
        ...
        EXPECT_CALL(mockChunkService, DeleteChunkSnapshot(_, _, _, _)).Times(1)
            .WillOnce(DoAll(SetArgPointee<2>(response),
                            Invoke(DeleteChunkSnapshotFunc)));
        copysetClient.DeleteChunkSnapshot(logicPoolId, copysetId, chunkId,
                                          sn, reqDone, 0);
        cond.Wait();
        ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_SUCCESS,
                  reqDone->GetErrorCode());
...
TEST_F(CopysetClientTest, get_chunk_info_test) {
    ...
    /* 非法参数 */
    {
        ...
        copysetClient.GetChunkInfo(logicPoolId, copysetId, chunkId, reqDone, 0);
        cond.Wait();
        ASSERT_EQ(CHUNK_OP_STATUS::CHUNK_OP_STATUS_INVALID_REQUEST,
                  reqDone->GetErrorCode());
    }
