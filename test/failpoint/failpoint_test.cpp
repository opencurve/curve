...
/*
 * For detailed documentation on how to use libfiu, please refer to:
 * https://blitiri.com.ar/p/libfiu/doc/man-libfiu.html Libfiu is divided into
 * two parts: the core API, which includes functions like
 * fiu_do_on/fiu_return_on/fiu_init. The core API is used to inject faults into
 * your business code and is controlled externally using the control API. The
 * control API includes functions like fiu_enable, fiu_disable,
 * fiu_enable_random, and more. These functions are used in your test code to
 * inject errors. You can find specific usage examples and methods in the code
 * snippets below.
 */
...
void modify_state_with_lamda(int& val) {  // NOLINT
    fiu_do_on(
        "side_effect_2", auto func = [&]() { val++; }; func(););
    return;
}
