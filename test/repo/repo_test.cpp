...
class RepoTest : public ::testing::Test {
 public:
  void SetUp() override {
    repo = new Repo();
    ASSERT_EQ(ConnectOK,
              repo->connectDB("curve_mds", "root", "localhost", "qwer"));
    ASSERT_EQ(ExecUpdateOK, repo->createDatabase());
    ASSERT_EQ(ExecUpdateOK, repo->useDataBase());
    ASSERT_EQ(ExecUpdateOK, repo->createAllTables());
  }
...
TEST_F(RepoTest, testChunkserverCUDA) {
  ...
  ChunkServerRepo r1(1,
                     ...
                     20 << 3,
                     10 << 3);
  ASSERT_EQ(ExecUpdateOK, repo->InsertChunkServerRepo(r1));
  ...
  r2.port = 9001;
  r2.mountPoint = "/mnt/2";
  ASSERT_EQ(ExecUpdateOK, repo->InsertChunkServerRepo(r2));

  // query chunk server id=1
  ChunkServerRepo queryRes;
  ASSERT_EQ(QueryOK, repo->QueryChunkServerRepo(r1.chunkServerID, &queryRes));
  ASSERT_TRUE(queryRes == r1);

  // query chunk server id=3, empty
  queryRes.chunkServerID = 0;
  ASSERT_EQ(QueryOK, repo->QueryChunkServerRepo(3, &queryRes));
  ASSERT_EQ(0, queryRes.chunkServerID);

  // query all chunk servers
  std::vector<ChunkServerRepo> chunkServers;
  ASSERT_EQ(QueryOK, repo->LoadChunkServerRepos(&chunkServers));
  ...
  // update used
  r1.used = 15 << 3;
  ASSERT_EQ(ExecUpdateOK, repo->UpdateChunkServerRepo(r1));
  queryRes.chunkServerID = 0;
  ASSERT_EQ(QueryOK, repo->QueryChunkServerRepo(r1.chunkServerID, &queryRes));
  ASSERT_EQ(r1.used, queryRes.used);

  // delete chunk server id=1
  ASSERT_EQ(ExecUpdateOK, repo->DeleteChunkServerRepo(r1.chunkServerID));
  queryRes.chunkServerID = 0;
  ASSERT_EQ(QueryOK, repo->QueryChunkServerRepo(r1.chunkServerID, &queryRes));
  ASSERT_EQ(0, queryRes.chunkServerID);
}
...
TEST_F(RepoTest, testServerCUDA) {
  ...
  ServerRepo s1(1,
                ...
                1,
                "first server");
  ASSERT_EQ(ExecUpdateOK, repo->InsertServerRepo(s1));
  ...
  ServerRepo s2(2,
                ...
                2,
                "second server");
  ASSERT_EQ(ExecUpdateOK, repo->InsertServerRepo(s2));

  // query server id=2
  ServerRepo queryRes(0);
  ASSERT_EQ(QueryOK, repo->QueryServerRepo(s1.serverID, &queryRes));
  ASSERT_TRUE(queryRes == s1);

  // query server id=3, empty
  queryRes.serverID = 0;
  ASSERT_EQ(QueryOK, repo->QueryServerRepo(3, &queryRes));
  ASSERT_EQ(0, queryRes.serverID);

  // query all servers
  std::vector<ServerRepo> servers;
  ASSERT_EQ(QueryOK, repo->LoadServerRepos(&servers));
  ...
  ASSERT_EQ(ExecUpdateOK, repo->UpdateServerRepo(s1));
  queryRes.serverID = 0;
  ASSERT_EQ(QueryOK, repo->QueryServerRepo(s1.serverID, &queryRes));
  ASSERT_EQ(s1.hostName, queryRes.hostName);

  // delete server id=1
  ASSERT_EQ(ExecUpdateOK, repo->DeleteServerRepo(s1.serverID));
  queryRes.serverID = 0;
  ASSERT_EQ(QueryOK, repo->QueryServerRepo(s1.serverID, &queryRes));
  ASSERT_EQ(0, queryRes.serverID);
}
...
TEST_F(RepoTest, testZoneCUDA) {
  ...
  ASSERT_EQ(ExecUpdateOK, repo->InsertZoneRepo(r1));
  ...
  ASSERT_EQ(ExecUpdateOK, repo->InsertZoneRepo(r2));

  // query zone id=2
  ZoneRepo queryRes;
  ASSERT_EQ(QueryOK, repo->QueryZoneRepo(r1.zoneID, &queryRes));
  ASSERT_TRUE(queryRes == r1);

  // query zone id=3
  queryRes.zoneID = 0;
  ASSERT_EQ(QueryOK, repo->QueryZoneRepo(3, &queryRes));
  ASSERT_EQ(0, queryRes.zoneID);

  // query all zones
  std::vector<ZoneRepo> zones;
  ASSERT_EQ(QueryOK, repo->LoadZoneRepos(&zones));
  ...
  ASSERT_EQ(ExecUpdateOK, repo->UpdateZoneRepo(r1));
  queryRes.zoneID = 0;
  ASSERT_EQ(QueryOK, repo->QueryZoneRepo(r1.zoneID, &queryRes));
  ASSERT_EQ(r1.desc, queryRes.desc);

  // delete zone id=1
  ASSERT_EQ(ExecUpdateOK, repo->DeleteZoneRepo(r1.zoneID));
  queryRes.zoneID = 0;
  ASSERT_EQ(QueryOK, repo->QueryZoneRepo(r1.zoneID, &queryRes));
  ASSERT_EQ(0, queryRes.zoneID);
}
...
TEST_F(RepoTest, testLogicalPoolCUDA) {
  ...
  ASSERT_EQ(ExecUpdateOK, repo->InsertLogicalPoolRepo(r1));
  ...
  r2.logicalPoolID = 2;
  ASSERT_EQ(ExecUpdateOK, repo->InsertLogicalPoolRepo(r2));

  // query logical pool, id=1
  LogicalPoolRepo queryRes;
  ASSERT_EQ(QueryOK, repo->QueryLogicalPoolRepo(r1.logicalPoolID, &queryRes));

  // query all logical pools
  std::vector<LogicalPoolRepo> logicalPools;
  ASSERT_EQ(QueryOK, repo->LoadLogicalPoolRepos(&logicalPools));
  ...
  // update status
  r1.status = Unhealthy;
  ASSERT_EQ(ExecUpdateOK, repo->UpdateLogicalPoolRepo(r1));
  queryRes.logicalPoolID = 0;
  ASSERT_EQ(QueryOK, repo->QueryLogicalPoolRepo(r1.logicalPoolID, &queryRes));
  ASSERT_EQ(r1.status, queryRes.status);

  // delete logical pool id=1
  ASSERT_EQ(ExecUpdateOK, repo->DeleteLogicalPoolRepo(r1.logicalPoolID));
  queryRes.logicalPoolID = 0;
  ASSERT_EQ(QueryOK, repo->QueryLogicalPoolRepo(r1.logicalPoolID, &queryRes));
...
TEST_F(RepoTest, testPhysicalPoolCUDA) {
  ...
  ASSERT_EQ(ExecUpdateOK, repo->InsertPhysicalPoolRepo(r1));
  ...
  ASSERT_EQ(ExecUpdateOK, repo->InsertPhysicalPoolRepo(r2));

  // query physical pool id=1
  PhysicalPoolRepo queryRes;
  ASSERT_EQ(QueryOK, repo->QueryPhysicalPoolRepo(r1.physicalPoolID, &queryRes));
  ASSERT_TRUE(queryRes == r1);

  // query physical pool id=3, empty
  queryRes.physicalPoolID = 0;
  ASSERT_EQ(QueryOK, repo->QueryPhysicalPoolRepo(3, &queryRes));
  ASSERT_EQ(0, queryRes.physicalPoolID);

  // query all physical pools
  std::vector<PhysicalPoolRepo> physicalPools;
  ASSERT_EQ(QueryOK, repo->LoadPhysicalPoolRepos(&physicalPools));
  ...
  ASSERT_EQ(ExecUpdateOK, repo->UpdatePhysicalPoolRepo(r1));
  queryRes.physicalPoolID = 0;
  ASSERT_EQ(QueryOK, repo->QueryPhysicalPoolRepo(r1.physicalPoolID, &queryRes));
  ASSERT_EQ(r1.desc, queryRes.desc);

  // delete physical pool id=1
  ASSERT_EQ(ExecUpdateOK, repo->DeletePhysicalPoolRepo(r1.physicalPoolID));
  queryRes.physicalPoolID = 0;
  ASSERT_EQ(QueryOK, repo->QueryPhysicalPoolRepo(r1.physicalPoolID, &queryRes));
  ASSERT_EQ(0, queryRes.physicalPoolID);
}
...
TEST_F(RepoTest, testCopySetCUDA) {
  ...
  ASSERT_EQ(ExecUpdateOK, repo->InsertCopySetRepo(r1));
  ...
  ASSERT_EQ(ExecUpdateOK, repo->InsertCopySetRepo(r2));

  // query copySet id=1, lid=1
  CopySetRepo queryRes;
  ASSERT_EQ(QueryOK,
            repo->QueryCopySetRepo(r1.copySetID, r1.logicalPoolID, &queryRes));
  ASSERT_TRUE(queryRes == r1);

  // query all copySets
  std::vector<CopySetRepo> copySets;
  ASSERT_EQ(QueryOK, repo->LoadCopySetRepos(&copySets));
  ...
  ASSERT_EQ(ExecUpdateOK, repo->UpdateCopySetRepo(r1));
