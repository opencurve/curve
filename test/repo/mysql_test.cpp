...
TEST(MySqlTest, MySqlConn) {
  ...
  // TEST4: create database and use success
  {
    try {
      ...
      statement = conn->createStatement();
      // clear exist curve_mds
      statement->executeUpdate("drop database if exists curve_mds");
      statement->executeUpdate("create database if not exists curve_mds");
      statement->executeUpdate("use curve_mds");
      SUCCEED();
    } catch (sql::SQLException &e) {
      FAIL();
    } catch (std::runtime_error &e) {
  ...
  {
    try {
      statement->executeUpdate("drop database curve_mds");
      delete (conn);
      delete (statement);
      SUCCEED();
    } catch (sql::SQLException &e) {
