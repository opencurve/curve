...
TEST_F(Ext4LocalFileSystemTest, DeleteTest) {
    // fake env
    {
        ...
        EXPECT_CALL(*wrapper, opendir(StrEq("/b"))).WillOnce(Return(dirp));
        ...
        EXPECT_CALL(*wrapper, remove(NotNull())).WillRepeatedly(Return(0));
    }
    ...
    // test delete dir
    {
        ...
        // opendir failed
        EXPECT_CALL(*wrapper, opendir(StrEq("/b"))).WillOnce(Return(nullptr));
        // List will failed
    ...
    // test delete file
    {
        ...
        EXPECT_CALL(*wrapper, remove(NotNull())).WillOnce(Return(-1));
...
TEST_F(Ext4LocalFileSystemTest, MkdirTest) {
    ...
    EXPECT_CALL(*wrapper, mkdir(NotNull(), _)).Times(0);
    ASSERT_EQ(lfs->Mkdir("/a"), 0);
    // stat failed ,mkdir success
    EXPECT_CALL(*wrapper, stat(StrEq("/a"), NotNull())).WillOnce(Return(-1));
    EXPECT_CALL(*wrapper, mkdir(StrEq("/a"), _)).WillOnce(Return(0));
    ASSERT_EQ(lfs->Mkdir("/a"), 0);
    // test relative path
    ...
    EXPECT_CALL(*wrapper, stat(StrEq("/a"), NotNull()))
        .WillOnce(DoAll(SetArgPointee<1>(info), Return(0)));
    EXPECT_CALL(*wrapper, mkdir(NotNull(), _)).WillOnce(Return(-1));
...
TEST_F(Ext4LocalFileSystemTest, DirExistsTest) {
    ...
    // stat failed
    EXPECT_CALL(*wrapper, stat(StrEq("/a"), NotNull()))
        .WillOnce(DoAll(SetArgPointee<1>(info), Return(-1)));
...
TEST_F(Ext4LocalFileSystemTest, RenameTest) {
    ...
    EXPECT_CALL(*wrapper, rename(NotNull(), NotNull())).WillOnce(Return(-1));
...
TEST_F(Ext4LocalFileSystemTest, ListTest) {
    ...
    vector<string> names;
    // opendir failed
    EXPECT_CALL(*wrapper, opendir(StrEq("/a"))).WillOnce(Return(nullptr));
    ASSERT_EQ(lfs->List("/a", &names), -errno);
    // success
    EXPECT_CALL(*wrapper, opendir(StrEq("/a"))).WillOnce(Return(dirp));
    EXPECT_CALL(*wrapper, readdir(dirp))
        .Times(4)
        .WillOnce(Return(entryArray))
...
TEST_F(Ext4LocalFileSystemTest, ReadTest) {
    ...
    ASSERT_STREQ(buf, "12");
    // pread failed
    EXPECT_CALL(*wrapper, pread(_, NotNull(), _, _)).WillOnce(Return(-1));
    ASSERT_EQ(lfs->Read(666, buf, 0, 3), -errno);
    // set errno = EINTR,and will repeatedly return -1
    errno = EINTR;
    EXPECT_CALL(*wrapper, pread(_, NotNull(), _, _)).WillRepeatedly(Return(-1));
    ASSERT_EQ(lfs->Read(666, buf, 0, 3), -errno);
...
TEST_F(Ext4LocalFileSystemTest, WriteTest) {
    char buf[4] = {0};
    // success
    EXPECT_CALL(*wrapper, pwrite(_, buf, _, _)).WillOnce(Return(1));
    EXPECT_CALL(*wrapper, pwrite(_, buf + 1, _, _)).WillOnce(Return(1));
    EXPECT_CALL(*wrapper, pwrite(_, buf + 2, _, _)).WillOnce(Return(1));
    ASSERT_EQ(lfs->Write(666, buf, 0, 3), 3);
    // pwrite failed
    EXPECT_CALL(*wrapper, pwrite(_, NotNull(), _, _)).WillOnce(Return(-1));
...
TEST_F(Ext4LocalFileSystemTest, FallocateTest) {
    ...
    // fallocate failed
    EXPECT_CALL(*wrapper, fallocate(_, _, _, _)).WillOnce(Return(-1));
...
TEST_F(Ext4LocalFileSystemTest, FstatTest) {
    ...
    // fallocate failed
    EXPECT_CALL(*wrapper, fstat(_, _)).WillOnce(Return(-1));
...
TEST_F(Ext4LocalFileSystemTest, FsyncTest) {
    ...
    // fallocate failed
    EXPECT_CALL(*wrapper, fsync(_)).WillOnce(Return(-1));
