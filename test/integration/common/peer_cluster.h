...
class PeerCluster {
 public:
    PeerCluster(const std::string& clusterName, const LogicPoolID logicPoolID,
                const CopysetID copysetID, const std::vector<Peer>& peers,
                std::vector<char**> params, std::map<int, int> paramsIndexs);
    virtual ~PeerCluster() {
        StopAllPeers();
        if (isFakeMdsStart_) {
 ...
 public:
     ...
     */
    int StartPeer(const Peer& peer, int id, const bool empty = false);
    /**
    ...
    int ConfirmLeader(const LogicPoolID& logicPoolId,
                      const CopysetID& copysetId, const std::string& leaderAddr,
                      Peer* leader);
 ...
 public:
    ...
    static int GetFollwerPeers(const std::vector<Peer>& peers, Peer leader,
 ...
 public:
    ...
    static int CreateCopyset(LogicPoolID logicPoolID, CopysetID copysetID,
 ...
 private:
    ...
    // chunkserver starts the mapping relationship of incoming parameters
    // (chunkserver id: params_'s index)
...
void WriteThenReadVerify(Peer leaderPeer, LogicPoolID logicPoolId,
                         ...
                         char fillCh, int loop, uint64_t sn = 1);
