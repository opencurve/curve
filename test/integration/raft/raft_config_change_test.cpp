...
static const char* raftConfigParam[5][16] = {
    {"chunkserver", "-chunkServerIp=127.0.0.1", "-chunkServerPort=9081",
     ...
     "-conf=./9081/chunkserver.conf", "-raft_sync_segments=true", NULL},
    {"chunkserver", "-chunkServerIp=127.0.0.1", "-chunkServerPort=9082",
     ...
     "-conf=./9082/chunkserver.conf", "-raft_sync_segments=true", NULL},
    {"chunkserver", "-chunkServerIp=127.0.0.1", "-chunkServerPort=9083",
     ...
     "-conf=./9083/chunkserver.conf", "-raft_sync_segments=true", NULL},
    {"chunkserver", "-chunkServerIp=127.0.0.1", "-chunkServerPort=9084",
     ...
     "-conf=./9084/chunkserver.conf", "-raft_sync_segments=true", NULL},
    {"chunkserver", "-chunkServerIp=127.0.0.1", "-chunkServerPort=9085",
     ...
     "-conf=./9085/chunkserver.conf", "-raft_sync_segments=true", NULL},
};
...
TEST_F(RaftConfigChangeTest, ThreeNodeBasicAddAndRemovePeer) {
    ...
    butil::Status st1 =
        RemovePeer(logicPoolId, copysetId, conf, removePeer, options);
    ASSERT_TRUE(st1.ok());
    ...
    butil::Status st3 =
        AddPeer(logicPoolId, copysetId, conf, removePeer, options);
    ASSERT_TRUE(st3.ok());
...
TEST_F(RaftConfigChangeTest, ThreeNodeRemoveShutdownPeer) {
    ...
    Configuration conf = cluster.CopysetConf();
    butil::Status st1 =
        RemovePeer(logicPoolId, copysetId, conf, shutdownPeer, options);
    ASSERT_TRUE(st1.ok());
    ...
    butil::Status st2 =
        AddPeer(logicPoolId, copysetId, conf, shutdownPeer, options);
...
TEST_F(RaftConfigChangeTest, ThreeNodeRemoveLeader) {
    ...
    Configuration conf = cluster.CopysetConf();
    butil::Status st1 =
        RemovePeer(logicPoolId, copysetId, conf, leaderPeer, options);
    ASSERT_TRUE(st1.ok());
    ...
    butil::Status st3 =
        AddPeer(logicPoolId, copysetId, conf, oldLeader, options);
    ASSERT_TRUE(st3.ok());
...
TEST_F(RaftConfigChangeTest, ThreeNodeShutdownPeerThenRemoveLeader) {
    ...
    Peer oldLeader = leaderPeer;
    /**
     * Removal is generally successful, but if one follower has already gone
     * down, the leader will automatically perform a term check and discover
     * that the majority of followers are no longer connected. At this point,
     * the leader will proactively step down, causing all requests to return as
     * failures prematurely. Therefore, the assertions below may fail, but the
     * removal itself will be successful.
     */
    //    ASSERT_TRUE(st1.ok());
    ...
    // The leader has been removed, so only the consistency of the data for two
    // replicas is verified
    ::usleep(waitMultiReplicasBecomeConsistent * 1000);
...
TEST_F(RaftConfigChangeTest, ThreeNodeHangPeerThenRemoveLeader) {
    ...
    // The leader has been removed, so verify the data consistency of the two
    // replicas
    ::usleep(waitMultiReplicasBecomeConsistent * 1000);
    std::vector<Peer> newPeers;
    for (Peer peer : peers) {
...
TEST_F(RaftConfigChangeTest, ThreeNodeHangPeerThenTransferLeaderTo) {
    ...
    for (int i = 0; i < kMaxLoop; ++i) {
        ...
        LOG(INFO) << i + 1 << " th transfer leader to " << hangPeer.address()
                  << " failed";
        ::sleep(1);
    }
...
TEST_F(
    ...
    ThreeNodeShutdownPeerAndThenAddNewFollowerFromInstallSnapshot) {  // NOLINT
    LogicPoolID logicPoolId = 2;
    CopysetID copysetId = 100001;
    uint64_t chunkId = 1;
    int length = kOpRequestAlignSize;
    ...
    // Wait snapshot, ensuring that subsequent restores must follow the
    // installation snapshot by taking two snapshots
...
TEST_F(RaftConfigChangeTest,
       ThreeNodeHangPeerAndThenAddNewFollowerFromInstallSnapshot) {  // NOLINT
    LogicPoolID logicPoolId = 2;
    CopysetID copysetId = 100001;
    uint64_t chunkId = 1;
    int length = kOpRequestAlignSize;
    ...
    butil::Status st1 =
        RemovePeer(logicPoolId, copysetId, conf, hangPeer, options);
 ...
 * 3. Restart the follower, and the follower can eventually catch up with the
 * leader through data recovery
 */
TEST_F(RaftConfigChangeTest,
       ThreeNodeRemoveDataAndThenRecoverFromInstallSnapshot) {  // NOLINT
    LogicPoolID logicPoolId = 2;
    CopysetID copysetId = 100001;
    uint64_t chunkId = 1;
    int length = kOpRequestAlignSize;
    ...
    ASSERT_FALSE(
        fs->DirExists(PeerCluster::CopysetDirWithoutProtocol(shutdownPeer)));
 ...
 */
TEST_F(RaftConfigChangeTest,
       ThreeNodeRemoveRaftLogAndThenRecoverFromInstallSnapshot) {  // NOLINT
    LogicPoolID logicPoolId = 2;
    CopysetID copysetId = 100001;
    uint64_t chunkId = 1;
    int length = kOpRequestAlignSize;
    ...
    ::system(PeerCluster::RemoveCopysetLogDirCmd(shutdownPeer, logicPoolId,
                                                 copysetId)
                 .c_str());
    std::shared_ptr<LocalFileSystem> fs(
        LocalFsFactory::CreateFs(FileSystemType::EXT4, ""));
    ASSERT_FALSE(fs->DirExists(PeerCluster::RemoveCopysetLogDirCmd(
        shutdownPeer, logicPoolId, copysetId)));
 ...
 * 3. Restart to recover the follower (additional assurance is required to
 * recover through snapshot), and hang the leader during the recovery process
 *    The install snapshot failed this time, but the new leader will be selected
 * and will continue to provide The follower recovers data, and ultimately the
 * follower data catches up with the leader and is consistent
 */
TEST_F(
    RaftConfigChangeTest,
    ThreeNodeRecoverFollowerFromInstallSnapshotButLeaderShutdown) {  // NOLINT
    LogicPoolID logicPoolId = 2;
    CopysetID copysetId = 100001;
    uint64_t chunkId = 1;
    int length = kOpRequestAlignSize;
    ...
    // 4. After a period of random sleep, hang up the leader and simulate the
    // installation snapshot when the leader hangs up
 ...
 * 2. Hang up the follower
 * 3. Restart to recover the follower (additional assurance is required to
 * recover through snapshot), and restart the leader during the recovery process
 */
TEST_F(RaftConfigChangeTest,
       ThreeNodeRecoverFollowerFromInstallSnapshotButLeaderRestart) {  // NOLINT
    LogicPoolID logicPoolId = 2;
    CopysetID copysetId = 100001;
    uint64_t chunkId = 1;
    int length = kOpRequestAlignSize;
 ...
 */
TEST_F(RaftConfigChangeTest,
       ThreeNodeRecoverFollowerFromInstallSnapshotButLeaderHang) {  // NOLINT
    LogicPoolID logicPoolId = 2;
    CopysetID copysetId = 100001;
    uint64_t chunkId = 1;
    int length = kOpRequestAlignSize;
    ...
    // 4. After a period of random sleep, hang up the leader and simulate the
    // leader hang during installation snapshot
    int sleepMs = butil::fast_rand_less_than(maxWaitInstallSnapshotMs) + 1;
    ::usleep(1000 * sleepMs);
 ...
 * 2. Hang up the follower
 * 3. Restart to recover the follower (additional assurance is required to
 * recover through snapshot), and during the recovery process, the leader will
 * hang for a while
 */
TEST_F(
    RaftConfigChangeTest,
    ThreeNodeRecoverFollowerFromInstallSnapshotButLeaderHangMoment) {  // NOLINT
    LogicPoolID logicPoolId = 2;
    CopysetID copysetId = 100001;
    uint64_t chunkId = 1;
    int length = kOpRequestAlignSize;
 ...
 * 2. Hang up the follower,
 * 3. Restart to recover the follower (additional assurance is required to
 * recover through snapshot), but the follower hung during the recovery process
 * 4. After a period of time, pull it up
 */
TEST_F(
    RaftConfigChangeTest,
    ThreeNodeRecoverFollowerFromInstallSnapshotButFollowerShutdown) {  // NOLINT
    LogicPoolID logicPoolId = 2;
    CopysetID copysetId = 100001;
    uint64_t chunkId = 1;
    int length = kOpRequestAlignSize;
    ...
    // 4. After a random period of sleep, hang up the follower and simulate the
    // installation snapshot Problem with follower
    int sleepMs1 = butil::fast_rand_less_than(maxWaitInstallSnapshotMs) + 1;
    ::usleep(1000 * sleepMs1);
    ASSERT_EQ(0, cluster.ShutdownPeer(shutdownPeer));
 ...
 * 2. Hang up the follower,
 * 3. Restart to recover the follower (additional assurance is required to
 * recover through snapshot). During the recovery process, the follower
 * restarted
 */
TEST_F(
    RaftConfigChangeTest,
    ThreeNodeRecoverFollowerFromInstallSnapshotButFollowerRestart) {  // NOLINT
    LogicPoolID logicPoolId = 2;
    CopysetID copysetId = 100001;
    uint64_t chunkId = 1;
    int length = kOpRequestAlignSize;
 ...
 * 2. Hang up the follower,
 * 3. Restart to recover the follower (additional assurance is required to
 * recover through snapshot), and the follower has changed during the recovery
 * process
 * 4. Restore after a period of time
 */
TEST_F(RaftConfigChangeTest,
       ThreeNodeRecoverFollowerFromInstallSnapshotButFollowerHang) {  // NOLINT
    LogicPoolID logicPoolId = 2;
    CopysetID copysetId = 100001;
    uint64_t chunkId = 1;
    int length = kOpRequestAlignSize;
    ...
    // 4. After a period of random sleep, hang the follower and simulate the
    // installation snapshot Problem with follower
    int sleepMs1 = butil::fast_rand_less_than(maxWaitInstallSnapshotMs) + 1;
    ::usleep(1000 * sleepMs1);
    ASSERT_EQ(0, cluster.HangPeer(shutdownPeer));
...
/**
 * 1. Create a replication group of 5 members, wait for the leader to generate,
 * write the data, and then read it out for verification
...
TEST_F(RaftConfigChangeTest, ThreeNodeKillFollowerThenChangePeers) {
    ...
    butil::Status st =
        ChangePeers(logicPoolId, copysetId, conf, newConf, options);
    ASSERT_TRUE(st.ok()) << st.error_str();

    ASSERT_EQ(0, cluster.WaitLeader(&leaderPeer));

    // Verification of data written before read
    ReadVerify(leaderPeer, logicPoolId, copysetId, chunkId, length,
               ch - 1,  // b
               loop);
