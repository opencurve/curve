...
static const char* raftVoteParam[4][16] = {
    {"chunkserver", "-chunkServerIp=127.0.0.1", "-chunkServerPort=9321",
     ...
     "-conf=./9321/chunkserver.conf", "-raft_sync_segments=true",
     "-raftLogUri=curve://./9321/copysets",
     "-walFilePoolDir=./9321/walfilepool/",
     "-walFilePoolMetaPath=./9321/walfilepool.meta", NULL},
    {"chunkserver", "-chunkServerIp=127.0.0.1", "-chunkServerPort=9322",
     ...
     "-conf=./9322/chunkserver.conf", "-raft_sync_segments=true",
     "-raftLogUri=curve://./9322/copysets",
     "-walFilePoolDir=./9322/walfilepool/",
     "-walFilePoolMetaPath=./9322/walfilepool.meta", NULL},
    {"chunkserver", "-chunkServerIp=127.0.0.1", "-chunkServerPort=9323",
     ...
     "-conf=./9323/chunkserver.conf", "-raft_sync_segments=true",
     "-raftLogUri=curve://./9323/copysets",
     "-walFilePoolDir=./9323/walfilepool/",
     "-walFilePoolMetaPath=./9323/walfilepool.meta", NULL},
    {"chunkserver", "-chunkServerIp=127.0.0.1", "-chunkServerPort=9324",
     ...
     "-conf=./9324/chunkserver.conf", "-raft_sync_segments=true",
     "-raftLogUri=curve://./9324/copysets",
     "-walFilePoolDir=./9324/walfilepool/",
     "-walFilePoolMetaPath=./9324/walfilepool.meta", NULL},
};
 ...
 *5. Transfer the leader to the newly started follower and read the data for
 *verification
...
TEST_F(RaftSnapshotTest, AddPeerRecoverFromSnapshot) {
    ...
    PeerCluster cluster("ThreeNode-cluster", logicPoolId, copysetId, peers,
                        params_, paramsIndexs_);
    ...
    WriteThenReadVerify(leaderPeer, logicPoolId, copysetId, chunkId, length, ch,
                        loop, initsn);
    ...
    ReadVerify(leaderPeer, logicPoolId, copysetId, chunkId, length, ch, loop);
    ...
    // Add a new peer
    ASSERT_EQ(0, cluster.StartPeer(peer4_, PeerCluster::PeerToId(peer4_)));
...
/**
 *Verify the shutdown of non leader nodes on three nodes, restart, and control
 *the recovery from install snapshot
 * 1. Create a replication group of 3 replicas
 * 2. Wait for the leader to generate, write the data, and then read it out for
 *verification
 * 3. Shutdown non leader
 * 4. Then sleep exceeds one snapshot interval, write read data,
 * 5. Then sleep for more than one snapshot interval and write read data; 4,5
 *two-step It is to ensure that at least two snapshots are taken, so that when
 *the node restarts again, it must pass the install snapshot
 * 6. Wait for the leader to be generated, and then verify the data written
 *before the read
...
TEST_F(RaftSnapshotTest, ShutdownOnePeerRestartFromInstallSnapshot) {
    ...
    // wait snapshot to ensure that it can trigger a snapshot
    // In addition, by increasing the chunk version number, trigger the chunk
    // file to generate a snapshot file
    ::sleep(1.5 * snapshotIntervalS_);
    ...
    ReadSnapshotVerify(leaderPeer, logicPoolId, copysetId, chunkId, length, ch,
                       loop);
    ...
    TransferLeaderAssertSuccess(&cluster, shutdownPeer, defaultCliOpt_);
    leaderPeer = shutdownPeer;
    ReadVerify(leaderPeer, logicPoolId, copysetId, chunkId, length, ch + 2,
               loop);
    ReadSnapshotVerify(leaderPeer, logicPoolId, copysetId, chunkId, length, ch,
                       loop);
}
 ...
 *the recovery from install snapshot
 * 1. Create a replication group of 3 replicas
 * 2. Wait for the leader to generate, write the data, and update the write
 *version to generate a chunk snapshot
 * 3. Shutdown non leader
 * 4. Then the sleep exceeds one snapshot interval,
 * 5. Delete the chunk snapshot and write the data again with a new version to
 *generate a new chunk snapshot
 * 6. Then sleep more than one snapshot interval; 4,5 two-step
 *    It is to ensure that at least two snapshots are taken, so that when the
 *node restarts again, it must pass the install snapshot
 * 7. Wait for the leader to be generated, and then verify the data written
 *before the read
...
TEST_F(RaftSnapshotTest, DoCurveSnapshotAfterShutdownPeerThenRestart) {
    ...
    WriteThenReadVerify(leaderPeer, logicPoolId, copysetId, chunkId, length,
                        ...
                        initsn + 1);  // sn = 2
    // Verify the correctness of chunk snapshot data
    ReadSnapshotVerify(leaderPeer, logicPoolId, copysetId, chunkId, length,
                       ch - 1,  // a
                       loop);
    ...
    DeleteSnapshotVerify(leaderPeer, logicPoolId, copysetId, chunkId,
                         initsn + 1);  // csn = 2
...
/**
 * During the process of verifying the curve snapshot dump, if there are
 * multiple copysets in the chunkserver,
 * 1. Create a replication group of 3 replicas
 * 2. Generate a new copyset for each replication group's chunkserver and use it
 * as a subsequent operation object
 * 3. Wait for the leader to generate and write data
 * 4. If the sleep exceeds one snapshot interval, ensure that a raft snapshot is
 * generated
 * 5. Update the write version to generate a chunk snapshot
 * 6. Then the sleep exceeds one snapshot interval to ensure that a raft
 * snapshot is generated
 * 7. Shutdown non leader
 * 8. Add a new node to AddPeer and restore it by loading a snapshot, then
 * remove the shutdown peer
 * 9. Switch the leader to the newly added peer
 * 10. Wait for the leader to be generated, then read the data and chunk
 * snapshot generated before validation
 */
TEST_F(RaftSnapshotTest, AddPeerWhenDoingCurveSnapshotWithMultiCopyset) {
    ...
    // Wait snapshot to ensure that it can trigger a snapshot
    // Ensure that the newly added peer is installed by downloading the snapshot
    // by taking at least two snapshots
    ::sleep(1.5 * snapshotIntervalS_);
