...
class MDSModuleException : public ::testing::Test {
 public:
    ...
    * Monitor whether client io can be issued normally within the expected time
     ...
     * @return: If the io is issued normally within the expected time, return true; otherwise, return false
     */
    ...
    bool SendAioWriteRequest(uint64_t offset, uint64_t size) {
        ...
        auto writeCallBack = [](CurveAioContext* context) {
            // Regardless of whether IO is successful or not, as long as it returns, it is set to true
...
TEST_F(MDSModuleException, MDSExceptionTest) {
    ...
    // 2. Expectations:
    //    a. When the cluster is in a normal state, client read and write requests can be issued normally.
    //    b. When shutting down an MDS, before the MDS service switches to another MDS,
    //       new write IO from clients will hang, and mount/unmount services will behave abnormally.
    //    c. After the MDS service switches, it is expected that client IO will be unaffected, and mount/unmount services will be normal.
    //    d. When bringing the MDS back up, client IO will be unaffected.
    // 1. In the initial state of the cluster, IO can be issued normally.
    ASSERT_TRUE(MonitorResume(0, 4096, 1));

    // 2. Kill an MDS that is currently in service, and when it is started, the first MDS is selected as the leader
    int serviceMDSID = 0;
    cluster->CurrentServiceMDS(&serviceMDSID);
    ASSERT_EQ(0, cluster->StopMDS(serviceMDSID));

    // 3. Start the background suspend and unload thread, and expect the suspend and unload to fail
    CreateOpenFileBackend();

    // 4. Start background IO monitoring and start writing from the next segment to trigger the getorallocate logic
    //    follower mds cluster normal service after renewing session expiration (20s renewal)
    ASSERT_TRUE(MonitorResume(segment_size, 4096, 25));

    // 5. Waiting for the end of backend suspension and uninstallation monitoring
    ...
    //      b. Turn off an MDS that is not in service, expect no impact on client IO, and suspend and uninstall the service normally
    // 1. The initial state of the cluster, IO is issued normally
    ASSERT_TRUE(MonitorResume(0, 4096, 1));

    // 2. Kill an MDS that is not in service. When starting, the first MDS is selected as the leader, and kill the second MDS
    serviceMDSID = 0;
    cluster->CurrentServiceMDS(&serviceMDSID);
    int killid = (serviceMDSID + 1) % 3;
    ASSERT_EQ(0, cluster->StopMDS(killid));

    // 3. Start the backend suspend and uninstall thread, and it is expected that the suspend and uninstall service will not be affected
    CreateOpenFileBackend();
    ...
    // 2. Expectations:
    //    a. When the cluster is in a normal state, client read and write requests can be issued normally.
    //    b. During the MDS hang period and before the lease renewal with etcd times out, new write IO will fail.
    //       This is because a new write triggers getorallocate, and the RPC sent to the MDS will keep timing out, leading to retries
    //       that eventually fail.
    //    c. The client session renewal duration is longer than the lease renewal duration between MDS and etcd.
    //       So, MDS is expected to complete the switch before session renewal failure occurs.
    //       Therefore, the client's session will not expire, and overwrite writes will not result in exceptions.
    //    d. When the hung MDS is restored, it is expected to have no impact on client IO.
    // 0. First, sleep for a period of time to allow the MDS cluster to elect a leader.
    std::this_thread::sleep_for(std::chrono::seconds(10));

    // 1. The initial state of the cluster, IO is issued normally
    ASSERT_TRUE(MonitorResume(0, 4096, 1));

    // 2. Hang an MDS that is currently in service, and when it is started, the first MDS is selected as the leader
    serviceMDSID = 0;
    cluster->CurrentServiceMDS(&serviceMDSID);
    ...
    // 2. Expectations
    //      a. When the cluster status is normal: client read and write requests can be issued normally
    //      b. Hang an MDS that is not in service, expecting no impact on client IO, and suspending and uninstalling the service is normal
    // 1. The initial state of the cluster, IO is issued normally
    ASSERT_TRUE(MonitorResume(0, 4096, 1));

    // 2. Hang an MDS that is not in service. When starting, the first MDS is selected as the leader, and hang the second MDS
    serviceMDSID = 0;
    cluster->CurrentServiceMDS(&serviceMDSID);
    ...
    // 4. Start backend iops monitoring and start writing from the next segment to trigger getorallocate logic
    //    follower mds cluster normal service after renewing session expiration (20s renewal)
    ...
    // 2. Expectations:
    //    a. When the cluster is in a normal state, client read and write requests can be issued normally.
    //    b. When shutting down two MDS nodes, before the MDS service switches to another MDS,
    //       new write IO from clients will fail, and mount/unmount services will behave abnormally.
    //    c. After the MDS service switches, it is expected that client IO will recover, and mount/unmount services will be normal.
    //    d. When bringing the MDS nodes back up, client IO will be unaffected.
    // 1. In the initial state of the cluster, IO can be issued normally.
    ASSERT_TRUE(MonitorResume(0, 4096, 1));

    // 2. Kill two MDSs. When starting, the first MDS is selected as the leader, and kill the first two MDSs
    serviceMDSID = 0;
    cluster->CurrentServiceMDS(&serviceMDSID);
    ...
    // 3. Starting the backend suspend and uninstall thread, it is expected that the suspend and uninstall service will be affected
    CreateOpenFileBackend();
    ...
    // 2. Expectations:
    //    a. When the cluster is in a normal state, client read and write requests can be issued normally.
    //    b. When shutting down two MDS nodes, it is expected that client IO will be unaffected, and mount/unmount services will be normal.
    //    c. When restarting these two MDS nodes, it is expected that client IO will be unaffected.
    // 1. In the initial state of the cluster, IO can be issued normally.
    ASSERT_TRUE(MonitorResume(0, 4096, 1));

    // 2. Starting the backend suspend and uninstall thread, it is expected that the suspend and uninstall service will be affected
    CreateOpenFileBackend();

    // 3. Kill two MDSs. When starting, the first MDS is selected as the leader, and kill the second two MDSs
    serviceMDSID = 0;
    cluster->CurrentServiceMDS(&serviceMDSID);
    ...
    // 1. Hang two MDS nodes, one of which is currently serving, and then recover them.
    // 2. Expectations:
    //    a. When the cluster is in a normal state, client read and write requests can be issued normally.
    //    b. During the MDS hang period and before the lease renewal with etcd times out, new write IO will fail.
    //       This is because a new write triggers getorallocate, and the RPC sent to the MDS will keep timing out, leading to retries
    ...
    //       So, MDS is expected to complete the switch before session renewal failure occurs.
    //       Therefore, the client's session will not expire, and overwrite writes will not result in exceptions.
    //    d. When the hung MDS nodes are recovered, it is expected to have no impact on client IO.
    // 1. Hang two MDS nodes, with the first MDS being elected as leader during startup, and both being hung before the process.
    serviceMDSID = 0;
    cluster->CurrentServiceMDS(&serviceMDSID);
    ...
    // 1. Hang two MDS nodes, neither of which is currently serving, and then recover them.
    // 2. Expectations:
    //    a. When the cluster is in a normal state, client read and write requests can be issued normally.
    //    b. Hang one MDS node that is not currently serving. It is expected that client IO will be unaffected, and mount/unmount services will behave normally.
    //    c. When these two MDS nodes are recovered, client IO is expected to be unaffected.
    // 1. In the initial state of the cluster, IO can be issued normally.
    ASSERT_TRUE(MonitorResume(0, 4096, 1));

    // 2. Hang two mds, the first mds is selected as the leader when starting, and kill the second two mds
    serviceMDSID = 0;
    cluster->CurrentServiceMDS(&serviceMDSID);
    ...
    // 2. Expectations:
    //    a. When the cluster is in a normal state, client read and write requests can be issued normally.
    //    b. Kill all three MDS nodes: Client IO failures occur after session expiration.
    //    c. During the period before the client session expires, new writes will fail, but overwrite writes will not be affected.
    //    d. Recover one of the hung MDS nodes: Client session renewal succeeds, and IO returns to normal.
    //    e. Recover the other two hung MDS nodes: Client IO remains unaffected.
    ...
    // 3. Send an IO and sleep for a period of time to determine whether to return
    //    Due to writing from the next segment, it triggers the getorallocate logic
    //    MDS is no longer in service, write requests are constantly hanging, unable to return
    ASSERT_TRUE(SendAioWriteRequest(9 * segment_size, 4096));
    std::this_thread::sleep_for(std::chrono::seconds(30));
    ASSERT_FALSE(writeIOReturnFlag);

    // 4. Waiting for the end of backend suspension and uninstallation monitoring
    WaitBackendCreateDone();
    ...
    // 2. Expectations:
    //    a. When the cluster is in a normal state, client read and write requests can be issued normally.
    //    b. Hang three MDS nodes: Client IO hangs after the session expires.
    //    c. During the period before the client session expires, new writes will hang continuously, but overwrite writes will not be affected.
    //    e. Recover one of the hung MDS nodes: Client session renewal succeeds, and IO returns to normal.
    //    f. Recover the other two hung MDS nodes: Client IO remains unaffected.
    // 1. In the initial state of the cluster, IO can be issued normally.
    ASSERT_TRUE(MonitorResume(0, 4096, 1));
    ...
    // 4. Send an IO and sleep for a period of time to determine whether to return
    //      Due to writing from the next segment, it triggers the getorallocate logic
    //      MDS is no longer in service, write requests are constantly hanging, unable to return
    ...
    // 7. Pulling up the process being hung may result in the process not shaking hands with ETCD for a long time,
    //    After it was pulled up, it exited, so the mds was restarted after recover,
    //    This ensures that at least one mds in the cluster is providing services
    ASSERT_EQ(0, cluster->RecoverHangMDS(1));
