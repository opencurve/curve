...
class MDSModuleException : public ::testing::Test {
 public:
     ...
     * @return: Is IO successfully distributed
     */
...
TEST_F(MDSModuleException, MDSExceptionTest) {
    ...
    //1 Restarting a serving MDS
    //2. Expectations
    //      a. When the cluster status is normal: client read and write requests can be issued normally
    //      b. Turn off one MDS, and before switching the MDS service to another MDS,
    //          When a client writes new IO, it will hang, and when the service is suspended or uninstalled, it will be abnormal
    //          After switching the mds service, it is expected that the client IO will not be affected, and the suspended and uninstalled services will be normal
    //      d. Pull up the mds again, client IO has no impact
    ...
    //Follow mds cluster normal service after renewing session expiration (20s renewal)
    ...
    //1 Hang an MDS that is currently serving
    //2. Expectations
    //      a. When the cluster status is normal: client read and write requests can be issued normally
    //      b. During the mds hang period and before the renewal timeout with etcd, the new write IO will fail,
    //         Because a new write triggers getorallocate, this RPC will continue to timeout when sent to mds, and then try again
    //         The final retry failed.
    //      c. The duration of client session renewal is always longer than when renewing between mds and etcd, so
    //          Mds expects to complete the switch before the session renewal fails, so the client's session
    //          It will not expire, and there will be no exceptions when overwriting.
    //      d. Restoring the hung mds is expected to have no impact on client io
    //0 Sleep for a period of time and wait for the mds cluster to select the leader
    ...
    //1 Restart two mds, one of which is currently serving the mds
    //2. Expectations
    //      a. When the cluster status is normal: client read and write requests can be issued normally
    //      b. Turn off both mds, and before switching the mds service to the other mds,
    //          The client's new IO write may fail, and suspending or uninstalling the service may cause exceptions
    //      c. After switching the mds service, it is expected that the client IO will resume and the suspended and uninstalled services will be normal
    //      d. Pull up the mds again, client IO has no impact
    //1 The initial state of the cluster, IO is issued normally
    ASSERT_TRUE(MonitorResume(0, 4096, 1));
    ...
    //1 Restart two MDSs, both of which are not in service
    //2. Expectations
    //      a. When the cluster status is normal: client read and write requests can be issued normally
    //      b. Turn off both mds, expect no impact on client IO, and suspend and uninstall services are normal
    //      c. Restarting these two mds does not affect client IO
    //1 The initial state of the cluster, IO is issued normally
    ASSERT_TRUE(MonitorResume(0, 4096, 1));
    ...
    //1 Hang two MDSs, including one that is currently in service, and then restore
    //2. Expectations
    //      a. When the cluster status is normal: client read and write requests can be issued normally
    //      b. During the mds hang period and before the renewal timeout with etcd, the new write IO will fail,
    //          Because a new write triggers getorallocate, this RPC will continue to timeout when sent to mds, and then try again
    ...
    //          It will not expire, and there will be no exceptions when overwriting.
    //      d. Restoring the hung mds is expected to have no impact on client io
    //1 Hang two MDSs, the first MDS is selected as the leader when starting, and hang the first two MDSs
    serviceMDSID = 0;
    cluster->CurrentServiceMDS(&serviceMDSID);
    ...
    //1 Hang two mds, which do not include the mds being serviced, and then restore
    //2. Expectations
    //      a. When the cluster status is normal: client read and write requests can be issued normally
    //      b. Hang an MDS that is not in service, expecting no impact on client IO, and suspending and uninstalling the service is normal
    //      c. Restore these two mds without affecting client IO
    //1 The initial state of the cluster, IO is issued normally
    ASSERT_TRUE(MonitorResume(0, 4096, 1));
    ...
    //1 Restart three MDSs
    //2. Expectations
    //  a. Cluster status is normal: client read and write requests can be issued normally
    //  b. Kill three mds: client experienced IO failure after session expiration
    //  c. New writes during the period before the client session expires will fail, and overwriting writes will not affect it
    //  d. Successfully renewed one of the mds: client sessions for which hang was restored, and the io returned to normal
    //  e. Restore the mds of the other two hang machines, with no impact on client io
    ...
    //1 Hang three MDSs and then restore
    //2. Expectations
    //  a. Cluster status is normal: client read and write requests can be issued normally
    //  b. Hang three mds: client has IO hang after session expiration
    //  c. New writes during the period before the client session expires will continue to hang, and overwriting writes will not affect it
    //  e. Successfully renewed one of the mds: client sessions for which hang was restored, and the io returned to normal
    //  f. Restore the mds of the other two hang machines, with no impact on client io
    //1 The initial state of the cluster, IO is issued normally
    ASSERT_TRUE(MonitorResume(0, 4096, 1));
