...
const std::vector<std::string> mdsConfigOpts{std::string("mds.etcd.endpoint=") +
                                             std::string(kEtcdClientIpPort)};
...
off_t RandomWriteOffset() { return rand() % 32 * (16 * 1024 * 1024); }

size_t RandomWriteLength() { return rand() % 32 * 4096; }
...
protected:
    static void SetUpTestCase()
    {
        ...
        ASSERT_EQ(0, cluster->PreparePhysicalPool(
                         1,
        ...
        ASSERT_EQ(
            ...
            cluster->PrepareLogicalPool(
                1,
                "test/integration/client/config/unstable/topo_unstable.json"));
        ...
        // 7. Sleep for 10 seconds first and let chunkserver select the leader
        std::this_thread::sleep_for(std::chrono::seconds(10));
    }
...
std::unordered_map<int, ChunkserverParam>
    UnstableCSModuleException::chunkServers; // NOLINT
...
//     1 ETCD
//     1 mds
//     3 zones, each with 2 chunkservers inside
TEST_F(UnstableCSModuleException, HangOneZone)
{
    srand(time(nullptr));

    // Enable multiple threads to write files
    ...
    // Write normally for 60 seconds and record the IOPS for the next 30 seconds
    ...
    // Print IOPS per second
    for (int i = 1; i <= 10; ++i)
    {
        std::this_thread::sleep_for(std::chrono::seconds(1));
        auto tmp = iops.value.get_value(1);
        LOG(INFO) << "after " << i << "s, iops: " << tmp;
        // Record the iops value for 5 seconds after recording
        if (i >= 5)
        {
