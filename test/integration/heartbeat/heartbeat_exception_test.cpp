...
/*
 * Bug Description: In a stability testing environment, when one machine
 crashes, it is set to "pending," and during the replica recovery process, there
 is MDS switching. Eventually, it was found that there were 5 "pending" chunk
 servers that did not complete migration.
 ...
 * 2. Copyset-1 completes the configuration change. At this point, the
 configuration on the leader is updated to epoch=2/{A, B, C, D},
 *    the configuration on the candidate is epoch=1/{A, B, C}, and the
 configuration recorded in MDS1 is epoch=1/{A, B, C}.
 * 3. MDS1 crashes, and MDS2 takes over the service. MDS2 loads copysets from
 the database, and the configuration for copyset-1 in MDS2 is epoch=1/{A, B, C}.
 * 4. Candidate-D reports a heartbeat, and the configuration for copyset-1 is
 epoch=1/{A, B, C}.
 *    MDS2 finds that the epoch reported by D matches the one recorded in MDS2,
 but D is not in the replication group recorded by MDS2,
 *    and there is no corresponding operator in the scheduling module. As a
 result, a command is issued to delete copyset-1 on D, leading to an accidental
 deletion of D.
 *
 *  Solution:
 *  Under normal circumstances, the heartbeat module should wait for a certain
 period (currently configured as 20 minutes) after MDS starts before issuing a
 command to delete a copyset.
 *  This greatly ensures that during this time, the configuration on the
 copyset-leader is updated in MDS,
 *  preventing the accidental deletion of data on replicas that have just joined
 the replication group.
 *
 * The starting point for this time should be when MDS officially starts
 providing external services,
 * rather than the MDS startup time. If it is set based on the MDS startup time,
 then if the standby MDS starts much later but can still provide services, it
 could be deleted immediately, leading to the bug.
 */
TEST_F(HeartbeatExceptionTest, test_mdsRestart_opLost) {
    ...
    // 4. The candidate reports the outdated configuration compared to MDS (the
    // candidate replays logs one by one to apply the old configuration):
    //    copyset-1(epoch=1, peers={1,2,3}, leader=1)
    //    Because mds.heartbeat.clean_follower_afterMs time has not yet elapsed,
    //    MDS cannot issue deletion commands. MDS issues no commands, so the
    //    data on the candidate will not be accidentally deleted.
    rep.Clear();
    req.Clear();
    ...
    //    mds issues a deletion configuration, and the data on the candidate
    ...
    // 6. leader reports the latest configuration copyset-1(epoch=3,
    // peers={1,2,3,4}, leader=1)
    auto newPeers = peers;
    ...
    hbtest_->AddCopySetToRequest(&req, csInfo);
    hbtest_->SendHeartbeat(req, SENDHBOK, &rep);
    // Check the response and issue the copyset current configuration guide
    // candidate to delete data
    ASSERT_EQ(0, rep.needupdatecopysets_size());
}
