 ...
 * 1. Write chunk1
 * 2. Simulate the first snapshot taken, write chunk1 during the dump process and generate a snapshot, but chunk2 does not have data write
 * 3. Delete the snapshot and write data to chunk2
 * 4. Simulate taking a second snapshot, writing chunk1 during the dump process, but not chunk2
...
TEST_F(SnapshotTestSuit, SnapshotTest) {
    ...
    /******************Scene 1: Take the first snapshot of the file******************/
    ...
    errorCode = dataStore_->WriteChunk(id1,  // id
                                       ...
                                       nullptr);
    ASSERT_EQ(errorCode, CSErrorCode::Success);
    // Information on chunk1 can be obtained, and all information meets expectations
    ...
    // Read the [0, 12KB) area of the chunk1 snapshot file, and the data read should all be '1'
    ...
    // Repeat write, verify that there will be no duplicate rows, and when reading the snapshot, the data in the [4KB, 8KB] area should be '1'
    errorCode = dataStore_->WriteChunk(id1,  // id
                                       fileSn,
                                       buf1_2,
                                       offset,
    ...
    // Write the area that has been partially cowed, and write the [4kb, 12kb] area
    offset = PAGE_SIZE;
    length = 2 * PAGE_SIZE;
    errorCode = dataStore_->WriteChunk(id1,  // id
                                       fileSn,
    ...
    // At this point, the data content returned by reading chunk1 should be [0,12KB]:2
    // The data content returned from reading chunk1 snapshot should be [0, 12KB):1
    // The data in other address spaces can be guaranteed without any need
    ...
    // When reading the [0, 12KB) area of the chunk1 snapshot file, the read data should still be '1'
    readSize = 3 * PAGE_SIZE;
    memset(readbuf, 0, sizeof(readbuf));
    errorCode = dataStore_->ReadSnapshotChunk(id1,  // chunk id
                                              1,  // snap sn
    ...
    /******************Scene 2: First snapshot completes, delete snapshot******************/

    // Request to delete the snapshot of chunk1, return success, and delete the snapshot
    ...
    // At this point, the data content returned by reading chunk1 should be [0,8KB]:3, [8KB, 12KB]:2
    // The data content returned from reading chunk1 snapshot should be [0, 12KB]:2
    // The data in other address spaces can be guaranteed without any need
    readSize = 3 * PAGE_SIZE;
    memset(readbuf, 0, sizeof(readbuf));
    errorCode = dataStore_->ReadChunk(id1,    // chunk id
    ...
    // Read the [0, 12KB) area of the chunk1 snapshot file, with data content of '2'
    readSize = 3 * PAGE_SIZE;
    memset(readbuf, 0, sizeof(readbuf));
    errorCode = dataStore_->ReadSnapshotChunk(id1,  // chunk id
                                              2,  // snap sn
    ...
    // The data content returned by reading the chunk2 snapshot should be [0, 8KB): a, and the data in the other address spaces can be guaranteed without any need to
    readSize = 2 * PAGE_SIZE;
    memset(readbuf, 0, sizeof(readbuf));
    errorCode = dataStore_->ReadSnapshotChunk(id2,  // chunk id
                                              2,  // snap sn
    ...
    /******************Scene 4: Second snapshot completes, delete snapshot******************/
    ...
    errorCode = dataStore_->WriteChunk(id2,  // id
                                       ...
                                       length,
                                       nullptr);
    // Check chunk2 information, as expected, curSn becomes 3, no snapshot will be generated
    errorCode = dataStore_->GetChunkInfo(id2, &chunk2Info);
    ASSERT_EQ(errorCode, CSErrorCode::Success);
    ASSERT_EQ(fileSn, chunk2Info.curSn);
    ASSERT_EQ(0, chunk2Info.snapSn);
    ...
    errorCode = dataStore_->WriteChunk(id2,  // id
                                       ...
                                       length,
                                       nullptr);
    // Check chunk2 information, chunk information remains unchanged and no snapshot will be generated
    errorCode = dataStore_->GetChunkInfo(id2, &chunk2Info);
    ASSERT_EQ(errorCode, CSErrorCode::Success);
    ASSERT_EQ(fileSn, chunk2Info.curSn);
    ASSERT_EQ(0, chunk2Info.snapSn);
