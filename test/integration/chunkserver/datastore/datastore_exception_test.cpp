...
/**
 * Exception Test 2
 * Scenario: Chunk's metapage data is corrupt, then the metapage is updated, and then the DataStore is restarted
...
/**
 * Exception Test 4
 * Scenario: Chunk snapshot's metapage data is corrupt, but the metapage is updated, and then restart the DataStore
...
/**
 * Exception Test 6
 * Scenario: WriteChunk updates the metapage and restarts, sn>chunk.sn,sn==chunk.correctedSn
 * Expected: Successful restart, successful re execution of the previous operation
 */
...
/**
 * Exception Test 7
 * Scenario: WriteChunk generates a snapshot and restarts, restoring historical and current operations
...
/**
 * Exception Test 9
 * Scenario: WriteChunk generates a snapshot and updates the metapage before restarting, restoring historical and current operations
 *      sn>chunk.sn, sn>chunk.correctedSn
 * Expected: Reboot successful
 */
...
/**
 * Exception Test 10
 * Scenario: WriteChunk restarts before updating the snapshot metapage to restore historical and current operations
 *      sn>chunk.sn, sn>chunk.correctedSn
 * Expected: Reboot successful
 */
...
/**
 * Exception Test 11
 * Scenario: WriteChunk updates snapshot metadata and restarts to restore historical and current operations
 *      sn>chunk.sn, sn>chunk.correctedSn
 * Expected: Reboot successful
 */
...
/**
 * Exception Test 12
 * Scenario: PasteChunk, when data is written halfway and the metapage has not been updated, restart/crash
