...
int SnapshotCloneServerModule::Start(const SnapshotCloneServerOptions& option) {
    ...
    auto core = std::make_shared<SnapshotCoreImpl>(
        client_, metaStore_, dataStore_, snapshotRef_, serverOption_);
    ...
    snapshotServiceManager_ =
        std::make_shared<SnapshotServiceManager>(taskMgr, core);
    ...
    auto cloneCore =
        std::make_shared<CloneCoreImpl>(client_, metaStore_, dataStore_,
                                        snapshotRef_, cloneRef_, serverOption_);
    if (cloneCore->Init() < 0) {
    ...
    cloneServiceManager_ = std::make_shared<CloneServiceManager>(
        cloneTaskMgr, cloneCore, cloneServiceManagerBackend);
