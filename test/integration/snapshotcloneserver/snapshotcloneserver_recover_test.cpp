...
const std::vector<std::string> chunkserverConfigOptions{
    ...
    "chunkfilepool.allocated_by_percent=false",
...
const std::vector<std::string> chunkserverConf1{
    ...
    {"-walFilePoolMetaPath=./" + kTestPrefix + "1/walfilepool.meta"},
};
...
const std::vector<std::string> chunkserverConf2{
    ...
    {"-walFilePoolMetaPath=./" + kTestPrefix + "2/walfilepool.meta"},
};
...
const std::vector<std::string> chunkserverConf3{
    ...
    {"-walFilePoolMetaPath=./" + kTestPrefix + "3/walfilepool.meta"},
};
...
class SnapshotCloneServerTest : public ::testing::Test {
 public:
    static void SetUpTestCase() {
        ...
        threadpool[0] = std::thread(&CurveCluster::FormatFilePool, cluster_,
                                    "./" + kTestPrefix + "1/chunkfilepool/",
                                    "./" + kTestPrefix + "1/chunkfilepool.meta",
                                    "./" + kTestPrefix + "1/chunkfilepool/", 2);
        threadpool[1] = std::thread(&CurveCluster::FormatFilePool, cluster_,
                                    "./" + kTestPrefix + "2/chunkfilepool/",
                                    "./" + kTestPrefix + "2/chunkfilepool.meta",
                                    "./" + kTestPrefix + "2/chunkfilepool/", 2);
        threadpool[2] = std::thread(&CurveCluster::FormatFilePool, cluster_,
        ...
        // Create a logical pool and sleep for a period of time to let the
    ...
    int PrepareCreateCloneFile(const std::string& fileName, FInfo* fInfoOut,
        ...
        int ret = snapClient_->CreateCloneFile(
            testFile1_, fileName, UserInfo_t(mdsRootUser_, mdsRootPassword_),
            testFile1Length, seqNum, chunkSize, 0, 0, kDefaultPoolset,
            fInfoOut);
        return ret;
    }
    ...
    int PrepareCreateCloneChunk(const std::vector<SegmentInfo>& segInfoVec,
        ...
        if (IsRecover) {
            for (int i = 0; i < testFile1AllocSegmentNum; i++) {
                ...
                // Due to the fact that each segment in the test file only
                // writes the first chunk, Snapshots can only dump the currently
                // written chunks, So cloning each segment from the snapshot
                // only creates the first chunk. And when cloning from a file,
                // because mds doesn't know if chunk has been written or not, So
                // we need to create all chunks.
                ...
                int ret = snapClient_->CreateCloneChunk(
                    location, cidInfo,
                    1,  // Restore the version number of the chunk in the
                        // snapshot
                    2,  // Restore the version number of the new file, which is
                        // the original file version number+1
                    chunkSize, cb);
                if (ret != LIBCURVE_ERROR::OK) {
                    return ret;
                }
    ...
    int PrepareRecoverChunk(const std::vector<SegmentInfo>& segInfoVec,
        ...
        if (IsSnapshot) {
            for (int i = 0; i < testFile1AllocSegmentNum; i++) {
                // Due to the fact that each segment in the test file only
                // writes the first chunk, Snapshots can only dump the currently
                // written chunks, So clone each segment from the snapshot and
                // only recover the first chunk. And when cloning from a file,
                // because mds doesn't know if chunk has been written or not, So
                // we need to recover all chunks.
                ChunkIDInfo cidInfo = segInfoVec[i].chunkvec[0];
...
TEST_F(SnapshotCloneServerTest, TestRecoverSnapshotWhenNotCreateSnapOnCurvefs) {
    ...
    SnapshotInfo snapInfo(uuid1, testUser1_, testFile1_, "snapxxx", 0,
                          chunkSize, segmentSize, testFile1Length, 0, 0,
                          kDefaultPoolset, 0, Status::pending);
...
// A snapshot has been created in the curve, but the successful result has not
// been returned. Restart for recovery
...
// A snapshot has been created in the curve, and the results have been returned.
// Restart to recover
TEST_F(SnapshotCloneServerTest,
...
// The snapshot phase has been created in the curve. NOS uploads some snapshots
// and restarts for recovery
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneHasNotCreateCloneFile) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kClone, testFile1_,
                        dstFile, kDefaultPoolset, 0, 0, 0, CloneFileType::kFile,
                        false, CloneStep::kCreateCloneFile,
                        CloneStatus::cloning);
...
// Reboot during the CreateCloneFile phase, successful file creation on mds but
// not returned
TEST_F(SnapshotCloneServerTest,
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneHasNotCreateCloneMeta) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kClone, testFile1_,
                        ...
                        CloneStep::kCreateCloneMeta, CloneStatus::cloning);
...
// Reboot during the CreateCloneMeta phase, successfully creating segment on mds
// but not returning
TEST_F(SnapshotCloneServerTest,
...
// Reboot during the CreateCloneChunk phase, cloneChunk not created on
// chunkserver
TEST_F(SnapshotCloneServerTest, TestRecoverCloneHasNotCreateCloneChunk) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kClone, testFile1_,
                        dstFile, kDefaultPoolset, fInfoOut.id, fInfoOut.id, 0,
                        CloneFileType::kFile, false,
                        CloneStep::kCreateCloneChunk, CloneStatus::cloning);
...
// Restart the CreateCloneChunk phase and create a partial clone chunk on the
// chunkserver
TEST_F(SnapshotCloneServerTest,
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneHasNotCompleteCloneMeta) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kClone, testFile1_,
                        dstFile, kDefaultPoolset, fInfoOut.id, fInfoOut.id, 0,
                        CloneFileType::kFile, false,
                        CloneStep::kCompleteCloneMeta, CloneStatus::cloning);
...
// The CompleteCloneMeta phase was restarted, and the call to CompleteCloneMeta
// on mds was successful but did not return
TEST_F(SnapshotCloneServerTest,
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneHasNotRecoverChunk) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kClone, testFile1_,
                        dstFile, kDefaultPoolset, fInfoOut.id, fInfoOut.id, 0,
                        CloneFileType::kFile, false, CloneStep::kRecoverChunk,
                        CloneStatus::cloning);
...
// Restart the RecoverChunk phase and partially call RecoverChunk on the
// chunkserver
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneHasNotCompleteCloneFile) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kClone, testFile1_,
                        dstFile, kDefaultPoolset, fInfoOut.id, fInfoOut.id, 0,
                        CloneFileType::kFile, false,
                        CloneStep::kCompleteCloneFile, CloneStatus::cloning);
...
// CompleteCloneFile stage restarted, but CompleteCloneFile successfully did not
// return on mds
TEST_F(SnapshotCloneServerTest,
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneHasNotChangeOwner) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kClone, testFile1_,
                        dstFile, kDefaultPoolset, fInfoOut.id, fInfoOut.id, 0,
                        CloneFileType::kFile, false, CloneStep::kChangeOwner,
                        CloneStatus::cloning);
...
// The ChangeOwner phase restarts, but the ChangeOwner on mds successfully did
// not return
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneHasNotRenameCloneFile) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kClone, testFile1_,
                        dstFile, kDefaultPoolset, fInfoOut.id, fInfoOut.id, 0,
                        CloneFileType::kFile, false,
                        CloneStep::kRenameCloneFile, CloneStatus::cloning);
...
// RenameCloneFile stage restarted, but RenameCloneFile successfully did not
// return on mds
TEST_F(SnapshotCloneServerTest,
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneLazyHasNotCreateCloneFile) {
    ...
    std::string uuid1 = UUIDGenerator().GenerateUUID();
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kRecover, snapId,
                        testFile1_, kDefaultPoolset, 0, 0, 0,
                        CloneFileType::kSnapshot, true,
                        CloneStep::kCreateCloneFile, CloneStatus::recovering);
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneLazyHasNotCreateCloneMeta) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kRecover, snapId,
                        ...
                        CloneFileType::kSnapshot, true,
                        CloneStep::kCreateCloneMeta, CloneStatus::recovering);
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneLazyHasNotCreateCloneChunk) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kRecover, snapId,
                        testFile1_, kDefaultPoolset, fInfoOut.id, testFd1_, 0,
                        CloneFileType::kSnapshot, true,
                        CloneStep::kCreateCloneChunk, CloneStatus::recovering);
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneLazyHasNotCompleteCloneMeta) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kRecover, snapId,
                        testFile1_, kDefaultPoolset, fInfoOut.id, testFd1_, 0,
                        CloneFileType::kSnapshot, true,
                        CloneStep::kCompleteCloneMeta, CloneStatus::recovering);
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneLazyHasNotChangeOwner) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kRecover, snapId,
                        testFile1_, kDefaultPoolset, fInfoOut.id, testFd1_, 0,
                        CloneFileType::kSnapshot, true, CloneStep::kChangeOwner,
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneLazyHasNotRenameCloneFile) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kRecover, snapId,
                        testFile1_, kDefaultPoolset, fInfoOut.id, testFd1_, 0,
                        CloneFileType::kSnapshot, true,
                        CloneStep::kRenameCloneFile, CloneStatus::recovering);
...
TEST_F(SnapshotCloneServerTest, TestRecoverCloneLazyHasNotCompleteCloneFile) {
    ...
    CloneInfo cloneInfo(uuid1, testUser1_, CloneTaskType::kRecover, snapId,
                        testFile1_, kDefaultPoolset, fInfoOut.id, testFd1_, 0,
                        CloneFileType::kSnapshot, true,
                        CloneStep::kCompleteCloneFile, CloneStatus::recovering);
