...
const std::vector<std::string> mdsConf1{
    ...
    {" --confPath=./test/integration/snapshotcloneserver/config/mds.conf.1"},
    {" --log_dir=./runlog/SCSTestLog"},
};
...
const std::vector<std::string> chunkserverConf1{
    ...
    {" -conf=./test/integration/snapshotcloneserver/config/chunkserver.conf.1"},
...
const std::vector<std::string> snapshotcloneConf{
    {" --conf=./test/integration/snapshotcloneserver/config/snapshot_clone_server.conf"},  //NOLINT
    {" --log_dir=./runlog/SCSTestLog"},
};
...
class SnapshotCloneServerTest : public ::testing::Test {
 public:
    static void SetUpTestCase() {
        system("mkdir -p ./runlog/SCSTestLog");

        cluster_ = new CurveCluster();
        ASSERT_NE(nullptr, cluster_);

        // 初始化db
        cluster_->InitDB("SCSTestDB");
        //在一开始清理数据库和文件
        ...
        fileClient_->Init("./test/integration/snapshotcloneserver/config/client.conf");  // NOLINT
...
TEST_F(SnapshotCloneServerTest, TestSnapLazyClone) {
    ...
    // 预期7：返回用户认证失败
    ret = CleanCloneTask(testUser2_, uuid1);
    ASSERT_EQ(kErrCodeInvalidUser, ret);

    // 操作8： ItUser1 CleanCloneTask UUID为clone1的UUID
    // 预期8：返回执行成功
    ret = CleanCloneTask(testUser1_, uuid1);
    ASSERT_EQ(0, ret);
    ...
    // 预期10：返回空
    TaskCloneInfo info;
    ret = GetCloneTaskInfo(testUser1_, uuid1, &info);
    ASSERT_EQ(kErrCodeFileNotExist, ret);
...
TEST_F(SnapshotCloneServerTest, TestImageLazyClone) {
    ...
    // 预期4：返回文件状态异常
    std::string uuid2;
    ret = MakeSnapshot(testUser1_, testFile1_, "snap1", &uuid2);
    ASSERT_EQ(kErrCodeFileStatusInvalid, ret);
    FileSnapshotInfo info2;
    int retCode = GetSnapshotInfo(
        testUser1_, testFile1_, uuid2, &info2);
