...
class CurveCluster {
 public:
     ...
     *If you need chunkservers with different IPs, please set the ipPort to 192.168.200.1:XXXX
     *
     * @param[in] id mdsId
    ...
    /**
     * @brief Start a snapshot clone server
     *
    ...
    /**
     * @brief Stop the snapshot clone server for the specified Id
     *
     * @param id The ID of the snapshot clone server
    ...
    /**
     * @brief: Restart the snapshot clone server with the specified Id
     *
     * @param id The ID of the snapshot clone server
     * @param force Use kill -9 when it is true
     * @return success returns pid; Failure returns -1
    ...
    /**
     * @brief Stop all snapshots clone servers
     * @return returns 0 for success, -1 for failure
     */
    ...
    /**
     *StopAllChunkServer Stop all chunkservers
     * @return 0.Success; -1.Failure
     */
    ...
    /**
     *Create File in Curve
     *
     * @param[in] user
     * @param[in] pwd password
     * @param[in] fileName File name
     * @param[in] fileSize File size
     * @param[in] normalFile Is a normal file
     * @return 0.Success; -1.Failure
     */
 ...
 private:
    /**
     *ProbePort detects whether the specified ipPort is in a listening state
     *
     * @param[in] ipPort The specified ipPort value
     * @param[in] timeoutMs The timeout time detected, in milliseconds
     * @param[in] expectOpen: Do you want it to be in a listening state
     *
     * @return 0 indicates that the detection within the specified time meets expectations- 1 indicates that the detection within the specified time does not meet expectations
     */
 ...
 private:
    ...
    //The pid corresponding to the snapshot clone server id
    std::map<int, pid_t> snapPidMap_;

    //The ipPort corresponding to the snapshot clone server ID
    std::map<int, std::string> snapIpPort_;

    //Conf corresponding to snapshot clone server id
