...
class CopysetEpochTest : public testing::Test {
 protected:
    ...
    virtual void TearDown() { Exec(("rm -fr " + dir1).c_str()); }
...
TEST_F(CopysetEpochTest, DISABLED_basic) {
    ...
    butil::string_appendf(&confEpochPath1, "/" BRAFT_SNAPSHOT_PATTERN,
                          ++lastIncludeIndex);
    ...
    // When the node is shut down and restarted, a load snapshot will be
    // executed to load the epoch from the snapshot
    node->Fini();
    node->Run();
    {
        butil::Status status = WaitLeader(logicPoolId, copysetId, conf, &leader,
    ...
    butil::string_appendf(&confEpochPath2, "/" BRAFT_SNAPSHOT_PATTERN,
                          ++lastIncludeIndex);
    ...
    butil::string_appendf(&confEpochPath4, "/" BRAFT_SNAPSHOT_PATTERN,
                          ++lastIncludeIndex);
    ...
    butil::string_appendf(&confEpochPath5, "/" BRAFT_SNAPSHOT_PATTERN,
                          ++lastIncludeIndex);
