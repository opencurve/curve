...
/**
 * Test CHUNK_OP_READ type request, requesting to read a chunk that is not a
 * clone chunk Result: Will not copy data from the remote end, directly read
 * data from the local, and the result is returned as successful
 */
 ...
 * Case3: The requested read area has been partially written and partially
 * unwritten Result3: Read from the local chunk for regions that have been
 * written, and read from the source for regions that have not been written,
 * resulting in a pass request Case4: The requested read area has been partially
 * written, and the requested offset is not aligned with pagesize Result4: Error
 * returned
 */
TEST_P(CloneCoreTest, ReadChunkTest2) {
    ...
    {
        info.bitmap->Set();
        // After each call to HandleReadRequest, it will be released by the
        // closure
        std::shared_ptr<ReadChunkRequest> readRequest =
            GenerateReadRequest(CHUNK_OP_TYPE::CHUNK_OP_READ, offset, length);
        EXPECT_CALL(*copyer_, DownloadAsync(_)).Times(0);
    ...
    // case2
    {
        ...
        // After a normal proposal, the closure will be handed over to the
        // concurrency layer for processing, Since the node here is mock, it is
        // necessary to proactively execute task.done.Run to release resources
    ...
    // case3
    {
        ...
        ASSERT_EQ(memcmp(cloneData,
                         closure->resContent_.attachment.to_string().c_str() +
                             3 * blocksize_,  // NOLINT
                         2 * blocksize_),
                  0);
    }
...
/**
 * Test CHUNK_OP_READ type request, the chunk requested for reading does not
 * exist, but the request contains the source data address Expected result:
 * Download data from the source and generate a pass request
 */
 ...
 * Result1: Returns -1, and the response status changes to
 * CHUNK_OP_STATUS_FAILURE_UNKNOWN Case2: Error downloading Result2: Returns -1,
 * and the response status changes to CHUNK_OP_STATUS_FAILURE_UNKNOWN Case3:
 * Error in ReadChunk Result3: Returns -1, and the response status changes to
 * CHUNK_OP_STATUS_FAILURE_UNKNOWN
 */
 ...
 *Result: Will not copy data remotely or read data locally, returns success
 *directly
 */
 ...
 * Case2: The requested area for recovery has not been written in whole or in
 * part Result2: Copy data from the remote end and generate a pass request
 */
TEST_P(CloneCoreTest, RecoverChunkTest2) {
    ...
    // case2
    {
        ...
        FakeChunkClosure* closure =
            reinterpret_cast<FakeChunkClosure*>(readRequest->Closure());
        // The closure has been forwarded to PasteRequest for processing, and
        // the closure has not been executed yet
...
// Case1: When reading a chunk, copy data from the remote end, but do not
// generate a pass request Case2: When recovering a chunk, copying data from the
// remote end will generate a pass request
