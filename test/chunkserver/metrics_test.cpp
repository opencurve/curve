...
TEST_F(CSMetricTest, OnRequestTest) {
    ...
    CopysetMetricPtr copysetMetric =
        metric_->GetCopysetMetric(logicId, copysetId);  // NOLINT
    ASSERT_NE(copysetMetric, nullptr);
...
TEST_F(CSMetricTest, OnResponseTest) {
    ...
    bool hasError = false;
    // Count the success of writing
    metric_->OnResponse(logicId, copysetId, CSIOMetricType::WRITE_CHUNK, size,
                        latUs, hasError);
    ...
    // Statistics on successful reads
    metric_->OnResponse(logicId, copysetId, CSIOMetricType::READ_CHUNK, size,
                        latUs, hasError);
    ...
    // Statistics on successful recovery
    metric_->OnResponse(logicId, copysetId, CSIOMetricType::RECOVER_CHUNK, size,
                        latUs, hasError);
    ...
    // Count the success of pass
    metric_->OnResponse(logicId, copysetId, CSIOMetricType::PASTE_CHUNK, size,
                        latUs, hasError);
    ...
    // Statistics on successful downloads
    metric_->OnResponse(logicId, copysetId, CSIOMetricType::DOWNLOAD, size,
                        latUs, hasError);
    ...
    hasError = true;
    // Count the number of write failures, increase the number of errors, and
    ...
    // Count the number of read failures, increase the number of errors, and
    // keep everything else unchanged
    metric_->OnResponse(logicId, copysetId, CSIOMetricType::READ_CHUNK, size,
                        latUs, hasError);
    ASSERT_EQ(0, serverReadMetric->reqNum_.get_value());
...
TEST_F(CSMetricTest, CountTest) {
    // In the initial state, there is no copyset and there are chunkNum chunks
    // in FilePool
    ...
    CopysetMetricPtr copysetMetric2 =
        metric_->GetCopysetMetric(logicId, copysetId2);  // NOLINT
    ...
    // The clone chunk is overwritten and written once, converting it to a
    // regular chunk
    ...
    ASSERT_EQ(CSErrorCode::Success, datastore->DeleteChunk(id2, 1));
    ASSERT_EQ(2, copysetMetric->GetChunkCount());
    ASSERT_EQ(0, copysetMetric->GetSnapshotCount());
    ASSERT_EQ(1, copysetMetric->GetCloneChunkCount());

    // Simulate copyset to reload the datastore, and after reinitialization, the
    // number of chunks remains unchanged for bug fix: CLDCFS-1473
...
TEST_F(CSMetricTest, OnOffTest) {
    ...
    // Creating the metric for the copyset returned success, but it was not
    // actually created
    {
        CopysetID copysetId = 1;
