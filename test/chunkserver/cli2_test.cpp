...
TEST_F(Cli2Test, basic) {
    ...
    /**
     * Set a larger default election timeout because the current CI environment
     * is prone to timeout
     */
    ...
    if (0 > pid4) {
        ...
        StartChunkserver(ip, port + 3, copysetdir.c_str(), confs,
                         snapshotInterval, electionTimeoutMs);
        return;
    }
    ...
    /**
     * The configuration change requires a longer timeout due to the completion
     * of copying a log entry Time to avoid occasional timeout errors in the CI
     * environment
     */
    ...
    /* remove peer */
    {
        ...
        butil::Status st = curve::chunkserver::RemovePeer(
            logicPoolId, copysetId, conf, peer, opt);
        LOG(INFO) << "remove peer: " << st.error_code() << ", "
                  << st.error_str();
        ASSERT_TRUE(st.ok());
        /* It is possible to remove a leader. If a leader is being removed, it
         * is necessary to wait until a new leader is generated, Otherwise, the
         * add peer test below will fail and wait for a long time to ensure
         * removal After the successful election of the new leader, switch to
         * the flush configuration of the become leader Complete*/
        ::usleep(1.5 * 1000 * electionTimeoutMs);
        butil::Status status = WaitLeader(logicPoolId, copysetId, conf, &leader,
                                          electionTimeoutMs);
        ASSERT_TRUE(status.ok());
    }
    /*  add peer */
    {
        ...
        Peer peer;
        peer.set_address("127.0.0.1:9035:0");
        butil::Status st = curve::chunkserver::AddPeer(logicPoolId, copysetId,
                                                       conf, peer, opt);
        LOG(INFO) << "add peer: " << st.error_code() << ", " << st.error_str();
        ASSERT_TRUE(st.ok());
    }
    /*Repeatedly add the same peer*/
    {
        ...
        butil::Status st = curve::chunkserver::AddPeer(logicPoolId, copysetId,
                                                       conf, peer, opt);
        LOG(INFO) << "add one peer repeat: " << st.error_code() << ", "
                  << st.error_str();
        ASSERT_TRUE(st.ok());
    }
    /*  transfer leader */
    {
        ...
        peer3.set_address("127.0.0.1:9035:0");
        {
            ...
            butil::Status st = curve::chunkserver::TransferLeader(
                logicPoolId, copysetId, conf, peer1, opt);
            LOG(INFO) << "transfer leader: " << st.error_code() << ", "
                      << st.error_str();
            ASSERT_TRUE(st.ok());
            /* The transfer leader only sends rpc to the leader and does not
             * wait for the leader to transfer We only return after success, so
             * we need to wait here. In addition, we cannot immediately check
             * the leader because After the leader transfer, the previous leader
             * may be returned, except for the transfer After the leader is
             * successful, when the benefit leader is in progress, the leader is
             * already visible, but The benefit leader will execute flush
             * current conf to act as the noop. If at this time Immediately
             * proceed to the next transfer leader, which will be organized
             * because there can only be one configuration at the same time
             * Changes in progress*/
            ::usleep(waitTransferLeader);
            butil::Status status = WaitLeader(logicPoolId, copysetId, conf,
                                              &leader, electionTimeoutMs);
            LOG(INFO) << "get leader: " << status.error_code() << ", "
                      << status.error_str();
            ASSERT_TRUE(status.ok());
        ...
        }
        {
            LOG(INFO) << "start transfer leader";
            butil::Status st = curve::chunkserver::TransferLeader(
                logicPoolId, copysetId, conf, peer2, opt);
            LOG(INFO) << "transfer leader: " << st.error_code() << ", "
                      << st.error_str();
            ASSERT_TRUE(st.ok());
            ::usleep(waitTransferLeader);
        ...
        }
        {
            LOG(INFO) << "start transfer leader";
            butil::Status st = curve::chunkserver::TransferLeader(
                logicPoolId, copysetId, conf, peer3, opt);
    ...
    /* change peers */
    {
        ...
        newConf.parse_from(
            "127.0.0.1:9033:0,127.0.0.1:9034:0,127.0.0.1:9036:0");  // NOLINT
        butil::Status st = curve::chunkserver::ChangePeers(
            logicPoolId, copysetId, conf, newConf, opt);
        LOG(INFO) << "change peers: " << st.error_code() << ", "
                  << st.error_str();
        ASSERT_TRUE(st.ok());
    }
    /* reset peer */
    {
        // Wait for the change peer to complete, otherwise the instance will
        // fail
        sleep(3);
        Peer peer;
        peer.set_address("127.0.0.1:9033:0");
        butil::Status status = curve::chunkserver::ResetPeer(
            logicPoolId, copysetId, conf, peer, opt);
    ...
    /*Get leader - illegal address*/
    {
        ...
        Peer leader;
        conf.parse_from(
            "127.0.0.1:65540:0,127.0.0.1:65541:0,127.0.0.1:65542:0");  // NOLINT
        butil::Status status = GetLeader(logicPoolId, copysetId, conf, &leader);
        ASSERT_FALSE(status.ok());
    ...
    /*Add peer - non-existent peer*/
    {
        ...
        butil::Status status = curve::chunkserver::AddPeer(
            logicPoolId, copysetId, conf, peer, opt);
        ASSERT_FALSE(status.ok());
    ...
    /*Change peers - non-existent peers*/
    {
        ...
        Configuration newConf;
        newConf.parse_from(
            "127.0.0.1:9033:0,127.0.0.1:9034:0,127.0.0.1:9039:0");  // NOLINT
        butil::Status status = curve::chunkserver::ChangePeers(
            logicPoolId, copysetId, conf, newConf, opt);
        ASSERT_FALSE(status.ok());
    ...
    /*Illegal snapshot peer address*/
    {
        ...
        peer.set_address("127.0.0.1:65540:0");
        butil::Status status =
            curve::chunkserver::Snapshot(logicPoolId, copysetId, peer, opt);
        LOG(INFO) << "snapshot: " << status.error_code() << ", "
                  << status.error_str();
        ASSERT_EQ(-1, status.error_code());
    }
