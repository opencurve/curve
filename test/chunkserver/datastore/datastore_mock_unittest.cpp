...
const char chunk1snap1Path[] = "/home/chunkserver/copyset/data/chunk_1_snap_1";
const char chunk1snap2[] = "chunk_1_snap_2";
const char chunk1snap2Path[] = "/home/chunkserver/copyset/data/chunk_1_snap_2";
const char chunk2[] = "chunk_2";
const char chunk2Path[] = "/home/chunkserver/copyset/data/chunk_2";
...
const char temp1Path[] = "/home/chunkserver/copyset/data/chunk_1_tmp";
const char location[] = "/file1/0@curve";
const int UT_ERRNO = 1234;

bool hasCreatFlag(int flag) { return flag & O_CREAT; }

ACTION_TEMPLATE(SetVoidArrayArgument, HAS_1_TEMPLATE_PARAMS(int, k),
...
class CSDataStore_test
 ...
 public:
    void SetUp() {
        ...
        dataStore = std::make_shared<CSDataStore>(lfs_, fpool_, options);
        fdMock = 100;
    ...
    inline void FakeEncodeChunk(char* buf, SequenceNum correctedSn,
                                SequenceNum sn,
    ...
    inline void FakeEncodeSnapshot(char* buf, SequenceNum sn) {
    ...
    void FakeEnv() {
        // fake DirExists
        EXPECT_CALL(*lfs_, DirExists(baseDir)).WillRepeatedly(Return(true));
        // fake List
        ...
        // fake FileExists
        ON_CALL(*lfs_, FileExists(_)).WillByDefault(Return(false));
        EXPECT_CALL(*lfs_, FileExists(chunk1Path)).WillRepeatedly(Return(true));
        EXPECT_CALL(*lfs_, FileExists(chunk2Path)).WillRepeatedly(Return(true));
        // fake Open
        ON_CALL(*lfs_, Open(_, _)).WillByDefault(Return(fdMock++));
        EXPECT_CALL(*lfs_, Open(_, Truly(hasCreatFlag))).Times(0);
        EXPECT_CALL(*lfs_, Open(chunk1Path, _)).WillRepeatedly(Return(1));
        EXPECT_CALL(*lfs_, Open(chunk1Path, Truly(hasCreatFlag))).Times(0);
        EXPECT_CALL(*lfs_, Open(chunk1snap1Path, _)).WillRepeatedly(Return(2));
        EXPECT_CALL(*lfs_, Open(chunk1snap1Path, Truly(hasCreatFlag))).Times(0);
        EXPECT_CALL(*lfs_, Open(chunk2Path, _)).WillRepeatedly(Return(3));
        EXPECT_CALL(*lfs_, Open(chunk2Path, Truly(hasCreatFlag))).Times(0);
        // fake fpool->GetFile()
        ON_CALL(*fpool_, GetFileImpl(_, NotNull())).WillByDefault(Return(0));
        EXPECT_CALL(*fpool_, RecycleFile(_)).WillRepeatedly(Return(0));
        // fake Close
        ON_CALL(*lfs_, Close(_)).WillByDefault(Return(0));
        // fake Delete
        ON_CALL(*lfs_, Delete(_)).WillByDefault(Return(0));
        // fake Fsync
        ON_CALL(*lfs_, Fsync(_)).WillByDefault(Return(0));
        // fake Fstat
        ...
        EXPECT_CALL(*lfs_, Read(2, NotNull(), 0, metapagesize_))
            .WillRepeatedly(
                DoAll(SetArrayArgument<1>(chunk1SnapMetaPage,
...
TEST_P(CSDataStore_test, ConstructorTest) {
    ...
    options.blockSize = blocksize_;
    options.metaPageSize = metapagesize_;
    ASSERT_DEATH(std::make_shared<CSDataStore>(nullptr, fpool_, options), "");
    ASSERT_DEATH(std::make_shared<CSDataStore>(lfs_, nullptr, options), "");
    options.baseDir = "";
    ASSERT_DEATH(std::make_shared<CSDataStore>(lfs_, fpool_, options), "");
}
...
TEST_P(CSDataStore_test, InitializeTest1) {
    // test unknown file
    EXPECT_CALL(*lfs_, DirExists(baseDir)).Times(1).WillOnce(Return(true));
    EXPECT_CALL(*lfs_, Mkdir(baseDir)).Times(0);
    vector<string> fileNames;
...
/**
 * InitializeTest
 * Case: There is a snapshot file, but the snapshot file does not have a
 * corresponding chunk Expected result: Delete the snapshot file and return true
 */
TEST_P(CSDataStore_test, InitializeTest2) {
    ...
    EXPECT_CALL(*lfs_, List(baseDir, NotNull()))
        .Times(1)
        .WillOnce(DoAll(SetArgPointee<1>(fileNames), Return(0)));
    EXPECT_CALL(*lfs_, FileExists(chunk2Path)).WillRepeatedly(Return(false));
    EXPECT_TRUE(dataStore->Initialize());
}
...
TEST_P(CSDataStore_test, InitializeTest3) {
    ...
    EXPECT_CALL(*lfs_, Close(3)).Times(1);
}
...
/**
 * InitializeTest
 * Case: There is a chunk file, and there are two conflicting snapshot files in
 * the chunk file Expected result: returns false
 */
...
TEST_P(CSDataStore_test, InitializeErrorTest1) {
    ...
    EXPECT_CALL(*lfs_, DirExists(baseDir)).Times(1).WillOnce(Return(false));
    EXPECT_CALL(*lfs_, Mkdir(baseDir)).Times(1).WillOnce(Return(-UT_ERRNO));
    // List should not be called
    EXPECT_CALL(*lfs_, List(baseDir, _)).Times(0);
    EXPECT_FALSE(dataStore->Initialize());
}
...
TEST_P(CSDataStore_test, InitializeErrorTest2) {
    // List dir failed
    EXPECT_CALL(*lfs_, DirExists(baseDir)).Times(1).WillOnce(Return(false));
    EXPECT_CALL(*lfs_, Mkdir(baseDir)).Times(1).WillOnce(Return(0));
    // List failed
    EXPECT_CALL(*lfs_, List(baseDir, NotNull()))
        .Times(1)
...
TEST_P(CSDataStore_test, InitializeErrorTest3) {
    ...
    EXPECT_CALL(*lfs_, Open(chunk1Path, _)).WillOnce(Return(-UT_ERRNO));
    EXPECT_FALSE(dataStore->Initialize());

    // open success
    EXPECT_CALL(*lfs_, Open(chunk1Path, _)).WillOnce(Return(1));
    // expect call close
    EXPECT_CALL(*lfs_, Close(1)).Times(1);
    // stat failed
    EXPECT_CALL(*lfs_, Fstat(1, NotNull())).WillOnce(Return(-UT_ERRNO));
    EXPECT_FALSE(dataStore->Initialize());
    ...
    EXPECT_CALL(*lfs_, Read(1, NotNull(), 0, metapagesize_))
        .WillOnce(DoAll(
            SetArrayArgument<1>(chunk1MetaPage, chunk1MetaPage + metapagesize_),
            Return(metapagesize_)));
    EXPECT_FALSE(dataStore->Initialize());
...
TEST_P(CSDataStore_test, InitializeErrorTest4) {
    ...
    EXPECT_CALL(*lfs_, Open(chunk1snap1Path, _)).WillOnce(Return(-UT_ERRNO));
    EXPECT_FALSE(dataStore->Initialize());

    // Each reinitialization will release the original resources and reload them
    EXPECT_CALL(*lfs_, Close(1)).WillOnce(Return(0));
    // open success
    EXPECT_CALL(*lfs_, Open(chunk1snap1Path, _)).WillOnce(Return(2));
    // expect call close
    EXPECT_CALL(*lfs_, Close(2)).Times(1);
    // stat failed
    EXPECT_CALL(*lfs_, Fstat(2, NotNull())).WillOnce(Return(-UT_ERRNO));
    EXPECT_FALSE(dataStore->Initialize());
...
TEST_P(CSDataStore_test, InitializeErrorTest5) {
    ...
    EXPECT_CALL(*lfs_, List(baseDir, NotNull()))
        ...
        .WillOnce(DoAll(SetArgPointee<1>(fileNames), Return(0)));
    // set open chunk file failed
    EXPECT_CALL(*lfs_, Open(chunk1Path, _)).WillRepeatedly(Return(-UT_ERRNO));
    EXPECT_FALSE(dataStore->Initialize());
}
...
TEST_P(CSDataStore_test, WriteChunkTest1) {
    ...
    string chunk3Path =
        string(baseDir) + "/" + FileNameOperator::GenerateChunkFileName(id);

    // If sn is 0, returns InvalidArgError
    EXPECT_EQ(CSErrorCode::InvalidArgError,
              dataStore->WriteChunk(id, 0, buf, offset, length, nullptr));
    // expect call chunkfile pool GetFile
    EXPECT_CALL(*lfs_, FileExists(chunk3Path)).WillOnce(Return(false));
    EXPECT_CALL(*fpool_, GetFileImpl(chunk3Path, NotNull()))
        .WillOnce(Return(0));
    EXPECT_CALL(*lfs_, Open(chunk3Path, _)).Times(1).WillOnce(Return(4));
    // will read metapage
    ...
    EXPECT_CALL(*lfs_, Read(4, NotNull(), 0, metapagesize_))
        .WillOnce(DoAll(
            SetArrayArgument<1>(chunk3MetaPage, chunk3MetaPage + metapagesize_),
            Return(metapagesize_)));
    // will write data
    ...
    EXPECT_CALL(*lfs_, Sync(4)).WillOnce(Return(0)).WillOnce(Return(-1));
    ...
    EXPECT_CALL(*lfs_, Close(2)).Times(1);
    EXPECT_CALL(*lfs_, Close(3)).Times(1);
    EXPECT_CALL(*lfs_, Close(4)).Times(1);
    delete[] buf;
}
...
TEST_P(CSDataStore_test, WriteChunkTest3) {
    ...
    EXPECT_CALL(*lfs_, Read(3, NotNull(), 0, metapagesize_))
        .WillRepeatedly(DoAll(
            SetArrayArgument<1>(chunk2MetaPage, chunk2MetaPage + metapagesize_),
            Return(metapagesize_)));
    EXPECT_TRUE(dataStore->Initialize());
...
/**
 * WriteChunkTest
 * Case: chunk exists, request sn to be equal to the SN of the chunk and not
 * less than correctSn chunk does not have a snapshot Expected result: Directly
...
/**
 * WriteChunkTest
 * Case: chunk exists, request sn is greater than the sn of the chunk, equal to
 * correctSn, chunk does not have a snapshot Expected result: Metapage will be
 * updated and data will be written to the chunk file
 */
 ...
 * Expected result: A snapshot file will be created, and the metapage will be
 * updated, When writing data, first perform a Copy-On-Write operation to the
 * snapshot, and then write to the chunk file
 */
TEST_P(CSDataStore_test, WriteChunkTest7) {
    ...
    string snapPath =
        string(baseDir) + "/" + FileNameOperator::GenerateSnapshotName(id, 2);
    // snapshot not exists
    EXPECT_CALL(*lfs_, FileExists(snapPath)).WillOnce(Return(false));
    // expect call chunkfile pool GetFile
    EXPECT_CALL(*fpool_, GetFileImpl(snapPath, NotNull())).WillOnce(Return(0));
    EXPECT_CALL(*lfs_, Open(snapPath, _)).WillOnce(Return(4));
    ...
    EXPECT_CALL(*lfs_, Read(4, NotNull(), 0, metapagesize_))
        .WillOnce(DoAll(SetArrayArgument<1>(metapage, metapage + metapagesize_),
                        Return(metapagesize_)));
    // will update metapage
    EXPECT_CALL(*lfs_,
                Write(3, Matcher<const char*>(NotNull()), 0, metapagesize_))
    ...
    // Write data for the same block again, no longer co w, but directly write
    // the data
    EXPECT_CALL(*lfs_, Write(3, Matcher<butil::IOBuf>(_),
                             metapagesize_ + offset, length))
        .Times(1);
    ...
    EXPECT_EQ(CSErrorCode::BackwardRequestError,
              dataStore->WriteChunk(id, sn - 1, buf, offset, length, nullptr));
 ...
 * WriteChunkTest
 * Case: chunk exists, request sn to be equal to the SN of the chunk and not
 * less than correctSn chunk has a snapshot Expected result: When writing data,
 * first perform a Copy-On-Write operation to the snapshot, and then write to
 * the chunk file
 */
 ...
 * WriteChunkTest
 * Case: chunk exists, request sn is greater than the sn of the chunk, equal to
 * correctSn chunk has a snapshot Expected result: Update the metapage and write
 * the chunk file
 */
 ...
 * Expected result: There are historical snapshots that have not been deleted,
 * write failed, and a SnapshotConflictError is returned
 */
 ...
 * Case1: clone chunk exists and has not been written before writing to the
 * region Expected result 1: Write data and update bitmap Case2: clone chunk
 * exists and has been written before writing to the region Expected result 2:
 * Write data but not update bitmap Case3: chunk exists and is a clone chunk.
 * Some areas have been written, while others have not Expected result 3: Write
 * data and update bitmap Case4: Overwrite the entire chunk Expected result 4:
 * Write data, and then the clone chunk will be converted to a regular chunk
 */
TEST_P(CSDataStore_test, WriteChunkTest13) {
    ...
    // Create clone chunk
    {
        ...
        EXPECT_CALL(*lfs_, Read(4, NotNull(), 0, metapagesize_))
            .WillOnce(DoAll(SetArrayArgument<1>(chunk3MetaPage,
                            ...
                            Return(metapagesize_)));
        EXPECT_EQ(CSErrorCode::Success,
                  dataStore->CreateCloneChunk(id, sn, correctedSn, chunksize_,
                                              location));
    }

    // Case1: chunk exists and is a clone chunk, which has not been written
    // before writing to the region
    {
        LOG(INFO) << "case 2";
        id = 3;  // not exist
        offset = blocksize_;
    ...
    // Case2: chunk exists and is a clone chunk, which has been written before
    ...
    // Case3: chunk exists and is a clone chunk. Some areas have been written,
    // while others have not
    {
        ...
        ASSERT_EQ(
            CSErrorCode::Success,
            dataStore->WriteChunk(id, sn, buf.get(), offset, length, nullptr));
        // After paste, the state of the chunk remains unchanged
        ASSERT_EQ(CSErrorCode::Success, dataStore->GetChunkInfo(id, &info));
        ASSERT_EQ(true, info.isClone);
 ...
 * Expected result 1: Write data, and then the clone chunk will be converted to
 * a regular chunk Case2: clone chunk exists, sn>chunk.sn, sn==chunk.correctedsn
 ...
 * Expected result: When writing data, first perform a Copy-On-Write operation
 * to the snapshot, and then write to the chunk file
 */
...
TEST_P(CSDataStore_test, WriteChunkErrorTest1) {
    ...
    EXPECT_CALL(*lfs_, Open(snapPath, _)).WillOnce(Return(-UT_ERRNO));
    EXPECT_EQ(CSErrorCode::InternalError,
              dataStore->WriteChunk(id, sn, buf, offset, length, nullptr));
    dataStore->GetChunkInfo(id, &info);
    ASSERT_EQ(2, info.curSn);
    ...
    EXPECT_CALL(*lfs_, FileExists(snapPath)).WillOnce(Return(true));
    EXPECT_CALL(*lfs_, Open(snapPath, _)).WillOnce(Return(4));
    EXPECT_CALL(*lfs_, Read(4, NotNull(), 0, metapagesize_))
        .WillOnce(Return(-UT_ERRNO));
    EXPECT_CALL(*lfs_, Close(4)).Times(1);
 ...
 * Expected result: Write failed, resulting in a snapshot file, but the chunk
 * version number will not change Write again without generating a new snapshot
 * file
 */
 ...
 * Expected result: Write failed, snapshot file generated, chunk version number
 * changed, The bitmap of the snapshot has not changed. If written again, it
 * will still be cowed
 */
...
/**
 * WriteChunkTest exception test
 * Case: Successfully created snapshot file, updated metapage, row, and write
 * data failed Expected result: Write failed, snapshot file generated, chunk
 * version number changed, The bitmap of the snapshot has changed, write it
 * again and directly write to the chunk file
 */
...
TEST_P(CSDataStore_test, WriteChunkErrorTest5) {
    ...
    EXPECT_CALL(*fpool_, GetFileImpl(chunk3Path, NotNull()))
        .WillOnce(Return(0));
    // set open chunk file failed
    EXPECT_CALL(*lfs_, Open(chunk3Path, _)).WillOnce(Return(-UT_ERRNO));
    EXPECT_EQ(CSErrorCode::InternalError,
              dataStore->WriteChunk(id, sn, buf, offset, length, nullptr));

    EXPECT_CALL(*lfs_, FileExists(chunk3Path)).WillOnce(Return(true));
    // open success
    EXPECT_CALL(*lfs_, Open(chunk3Path, _)).WillOnce(Return(4));
    // expect call close
    EXPECT_CALL(*lfs_, Close(4)).Times(1);
    // stat failed
    EXPECT_CALL(*lfs_, Fstat(4, NotNull())).WillOnce(Return(-UT_ERRNO));
    EXPECT_EQ(CSErrorCode::InternalError,
              dataStore->WriteChunk(id, sn, buf, offset, length, nullptr));
    ...
    EXPECT_CALL(*lfs_, Fstat(4, NotNull()))
        .WillOnce(DoAll(SetArgPointee<1>(fileInfo), Return(0)));
    EXPECT_EQ(CSErrorCode::FileFormatError,
              dataStore->WriteChunk(id, sn, buf, offset, length, nullptr));
 ...
 * Case1: The request location is too long, causing the metapage size to exceed
 * the page size Expected result 1: Create clone chunk failed Case2: Failed to
 * write data Expected result 2: InternalError returned, chunk status remains
 * unchanged Case3: Failed to update metapage Expected result 3: InternalError
 * returned, chunk status remains unchanged
 */
...
/**
 * ReadChunkTest
 * Case: chunk exists, reading area exceeds chunk size or offset and length are
 * not aligned Expected result: InvalidArgError error code returned
 */
...
TEST_P(CSDataStore_test, ReadChunkTest4) {
    ...
    EXPECT_EQ(
        CSErrorCode::Success,
        dataStore->CreateCloneChunk(id, sn, correctedSn, chunksize_, location));
...
TEST_P(CSDataStore_test, ReadSnapshotChunkTest3) {
    ...
    EXPECT_CALL(*lfs_, Write(1, Matcher<butil::IOBuf>(_),
        ...
        .Times(1);
    EXPECT_EQ(CSErrorCode::Success,
              dataStore->WriteChunk(id, sn, writeBuf, offset, length, nullptr));
    ...
    EXPECT_CALL(*lfs_, Read(1, NotNull(), metapagesize_, blocksize_)).Times(1);
    EXPECT_CALL(*lfs_,
...
/**
 * ReadSnapshotChunkErrorTest
 * Case: chunk exists, request version number is equal to Chunk version number,
 * failed while reading data Expected result: InternalError returned
 */
...
TEST_P(CSDataStore_test, ReadChunkMetaDataTest2) {
    ...
    memset(buf, 0, blocksize_);
    // test chunk exists
    EXPECT_CALL(*lfs_, Read(1, NotNull(), 0, metapagesize_)).Times(1);
    EXPECT_EQ(CSErrorCode::Success, dataStore->ReadChunkMetaPage(id, sn, buf));
...
TEST_P(CSDataStore_test, DeleteChunkTest1) {
    ...
    // test chunk not exists
    EXPECT_EQ(CSErrorCode::Success, dataStore->DeleteChunk(id, sn));
...
TEST_P(CSDataStore_test, DeleteChunkTest4) {
    ...
    {
        // chunk will be closed
        EXPECT_CALL(*lfs_, Close(3)).Times(0);
        // expect to call FilePool RecycleFile
        EXPECT_CALL(*fpool_, RecycleFile(chunk2Path)).Times(0);
        EXPECT_EQ(CSErrorCode::BackwardRequestError,
    ...
    // case2
    {
        ...
        // expect to call FilePool RecycleFile
        EXPECT_CALL(*fpool_, RecycleFile(chunk2Path)).WillOnce(Return(0));
        EXPECT_EQ(CSErrorCode::Success, dataStore->DeleteChunk(id, 3));
    }
 ...
 * Case: chunk exists, snapshot file does not exist, error occurred during
 * recyclechunk Expected result: returned successfully
 */
TEST_P(CSDataStore_test, DeleteChunkErrorTest1) {
    ...
    EXPECT_CALL(*lfs_, Close(3)).Times(1);
    // expect to call FilePool RecycleFile
    EXPECT_CALL(*fpool_, RecycleFile(chunk2Path)).WillOnce(Return(-1));
    EXPECT_EQ(CSErrorCode::InternalError, dataStore->DeleteChunk(id, sn));
...
// When there is a snapshot file, the sn of fileSn>=chunk is the only condition
// to determine whether to delete the snapshot For correctedSn, if fileSn is
// greater than chunk's sn and correctedSn is the judgment Do you want to modify
// the unique condition for correctedSn
 ...
 *       chunk.sn>snap.sn
 * Expected result: Delete snapshot without modifying correctedSn, return
 * success
 */
TEST_P(CSDataStore_test, DeleteSnapshotChunkOrCorrectSnTest2) {
    ...
    EXPECT_CALL(*lfs_, Close(2)).Times(1);
    // expect to call FilePool RecycleFile
    EXPECT_CALL(*fpool_, RecycleFile(chunk1snap1Path)).Times(1);
 ...
 * Case: chunk exists, snapshot exists
 *       fileSn < chunk's sn
 *       At this point, regardless of the value of correctSn, correctedSn will
 * not be modified Expected result: Success returned, snapshot will not be
 * deleted, correctedSn will not be modified
 */
TEST_P(CSDataStore_test, DeleteSnapshotChunkOrCorrectSnTest3) {
    ...
    EXPECT_CALL(*lfs_, Close(2)).Times(0);
    // chunk's metapage should not be updated
    EXPECT_CALL(*lfs_,
                Write(3, Matcher<const char*>(NotNull()), 0, metapagesize_))
        .Times(0);
    ...
    // The following use case is used to supplement the
    // DeleteSnapshotChunkOrCorrectSnTest2 use case Boundary situation when
    // fileSn == sn fileSn == sn fileSn > correctedSn
    fileSn = 3;
    // snapshot will be closed
    EXPECT_CALL(*lfs_, Close(2)).Times(1);
    // expect to call FilePool RecycleFile
 ...
 * Expected result: Delete the snapshot and modify correctedSn, returning
 * success
 */
...
TEST_P(CSDataStore_test, DeleteSnapshotChunkOrCorrectSnTest8) {
    ...
    EXPECT_CALL(*lfs_, Close(2)).Times(0);
    // expect to call FilePool RecycleFile
    EXPECT_CALL(*fpool_, RecycleFile(chunk1snap1Path)).Times(0);
...
TEST_P(CSDataStore_test, DeleteSnapshotChunkOrCorrectSnErrorTest2) {
    ...
    EXPECT_CALL(*lfs_, Close(2)).Times(1);
    // expect to call FilePool RecycleFile
    EXPECT_CALL(*fpool_, RecycleFile(chunk1snap1Path)).WillOnce(Return(-1));
    // chunk's metapage will be updated
    EXPECT_CALL(*lfs_,
                Write(1, Matcher<const char*>(NotNull()), 0, metapagesize_))
        .Times(0);
 ...
 * Case2: The specified chunk does not exist, the specified chunksize is
 * consistent with the configuration Expected result 2: Creation successful
 * Case3: The specified chunk exists, and the parameters are consistent with the
 * original chunk Expected result 3: Success returned Case4: The specified chunk
 * exists, and the parameters are inconsistent with the original chunk Expected
 * result 4: ChunkConflictError returned without changing the original chunk
 * information Case5: The specified chunk exists, but the specified chunk size
 * is inconsistent with the configuration Expected result 5: InvalidArgError
 * returned without changing the original chunk information Case6: The specified
 * chunk exists, but the chunk is not a clone chunk. The parameters are
 * consistent with the chunk information Expected result: ChunkConflictError
 * returned without changing the original chunk information
 */
TEST_P(CSDataStore_test, CreateCloneChunkTest) {
    ...
    // Case1: Input incorrect parameters
    {
        // size != chunksize
        EXPECT_EQ(CSErrorCode::InvalidArgError,
                  dataStore->CreateCloneChunk(id, sn, correctedSn, blocksize_,
                                              location));

        // sn == 0
        EXPECT_EQ(CSErrorCode::InvalidArgError,
                  dataStore->CreateCloneChunk(id, 0, correctedSn, chunksize_,
                                              location));

        // location is empty
        EXPECT_EQ(
            CSErrorCode::InvalidArgError,
            dataStore->CreateCloneChunk(id, sn, correctedSn, chunksize_, ""));
    }
    ...
    // Case3: The specified chunk exists, and the parameters are consistent with
    // the original chunk
    {
        EXPECT_EQ(CSErrorCode::Success,
                  dataStore->CreateCloneChunk(id, sn, correctedSn, chunksize_,
    ...
    // Case4: The specified chunk exists, and the parameters are inconsistent
    // with the original chunk Returns ChunkConflictError, but does not change
    // the original chunk information
    {
        // Version inconsistency
        EXPECT_EQ(CSErrorCode::ChunkConflictError,
                  dataStore->CreateCloneChunk(id, sn + 1, correctedSn,
                                              chunksize_, location));
        // Inconsistent correctedSn
        EXPECT_EQ(CSErrorCode::ChunkConflictError,
                  dataStore->CreateCloneChunk(id, sn, correctedSn + 1,
                                              chunksize_, location));
    ...
    // Case5: The specified chunk exists, but the specified chunksize is
    // inconsistent with the configuration Returns InvalidArgError, but does not
    // change the original chunk information
    {
        EXPECT_EQ(
            CSErrorCode::InvalidArgError,
            dataStore->CreateCloneChunk(id, sn, correctedSn,
                                        chunksize_ + metapagesize_, location));
        // Check the generated clone chunk information
        ASSERT_EQ(CSErrorCode::Success, dataStore->GetChunkInfo(id, &info));
        ASSERT_EQ(id, info.chunkId);
        ASSERT_EQ(sn, info.curSn);
 ...
 * Case2: chunk exists, requested offset exceeds chunk file size or offset
 * length is not aligned Expected result 2: InvalidArgError returned Case3:
 * chunk exists, but not clone chunk Expected result 3: Success returned Case4:
 * chunk exists and is a clone chunk, which has not been written before writing
 * to the region Expected result 4: Write data and update bitmap Case5: chunk
 * exists and is a clone chunk, which has been written before writing to the
 * region Expected result 5: No data written and Bitmap will not be updated
 * Case6: chunk exists and is a clone chunk. Some areas have been written, while
 * others have not Expected result 6: Only write unwritten data and update
 * bitmap Case7: Overwrite the entire chunk Expected result 7: Data is written
 * to an unwritten area, and then the clone chunk will be converted to a regular
 * chunk
 */
TEST_P(CSDataStore_test, PasteChunkTest1) {
    ...
    // Case4: chunk exists and is a clone chunk, which has not been written
    // before writing to the region
    {
        id = 3;  // not exist
        offset = blocksize_;
    ...
    // Case5: chunk exists and is a clone chunk, which has been written before
    // writing to the region
    {
        id = 3;  // not exist
        offset = blocksize_;
    ...
    // Case6: chunk exists and is a clone chunk. Some areas have been written,
    // while others have not
    {
        id = 3;  // not exist
        offset = 0;
        length = 4 * blocksize_;
        // [2 * blocksize_, 4 * blocksize_) area has been written, [0,
        // blocksize_) is a metapage
        EXPECT_CALL(*lfs_, Write(4, Matcher<const char*>(NotNull()),
    ...
    // Case7: Overwrite the entire chunk
    {
        ...
        offset = 0;
        length = chunksize_;
        // [blocksize_, 4 * blocksize_) area has been written, [0, blocksize_)
        // is a metapage
        EXPECT_CALL(*lfs_, Write(4, Matcher<const char*>(NotNull()),
...
TEST_P(CSDataStore_test, GetHashErrorTest1) {
    ...
    // test chunk not exists
    EXPECT_EQ(CSErrorCode::ChunkNotExistError,
              dataStore->GetChunkHash(id, 0, 4096, &hash));
...
TEST_P(CSDataStore_test, GetHashErrorTest2) {
    ...
    // test read chunk failed
    EXPECT_CALL(*lfs_, Read(1, NotNull(), 0, 4096)).WillOnce(Return(-UT_ERRNO));
    EXPECT_EQ(CSErrorCode::InternalError,
              dataStore->GetChunkHash(id, 0, 4096, &hash));
    EXPECT_CALL(*lfs_, Close(1)).Times(1);
    EXPECT_CALL(*lfs_, Close(2)).Times(1);
