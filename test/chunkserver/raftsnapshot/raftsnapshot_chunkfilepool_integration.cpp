...
/**
 * Verify the shutdown and restart of non-leader nodes in a cluster of 3 nodes, and control them to recover from installing snapshots.
 ...
 * 4. Sleep for a duration longer than a snapshot interval, then write and read data.
 * 5. Sleep for a duration longer than a snapshot interval again, then write and read data.
 *    Steps 4 and 5 are to ensure that at least two snapshots are taken. Therefore,
 *    when the node restarts, it must recover via an install snapshot because the log has already been deleted. The data for the install snapshot is retrieved from the FilePool.
 * 6. Wait for the leader to emerge, then read the previously written data for verification.
...
TEST_F(RaftSnapFilePoolTest, ShutdownOnePeerRestartFromInstallSnapshot) {
    ...
    // In this snapshot creation, Raft will retrieve a file from the FilePool as the snapshot file,
    // and it will delete the previous snapshot file. The deleted file will be reclaimed into the FilePool.
    // So overall, this snapshot creation will only result in the datastore retrieving a file from the FilePool,
    // but a snapshot is taken and then returned.
    ...
    // In this snapshot creation, Raft will retrieve a file from the FilePool as the snapshot file.
    // Then, it will delete the previous snapshot file, and the deleted file will be reclaimed into the FilePool.
    // So, overall, this snapshot creation will only result in the datastore retrieving a file from the FilePool,
    // but it involves taking one snapshot and returning another to the FilePool.
    ::sleep(1.5*snapshotTimeoutS);
    // Initiate read/write again
