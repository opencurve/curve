...
/**
 *The current FilePool needs to be formatted in advance before it can be used. This function is used to format the FilePool in advance
 * @param fsptr: This article's file system pointer
 * @param chunkfileSize: Chunk file size
 * @param metaPageSize: Chunk The metapage size of the file
 * @param poolpath: The path to the file pool, for example/ Chunkfilepool/
 * @param metaPath: Meta file path, for example/ Chunkfilepool/chunkfilepool.meta
 * @return successfully initializes and returns the FilePool pointer. Otherwise, it returns null
 */
...
class TestCluster {
 ...
 public:
     ...
     * @param: createFilePool: create a FilePool? It is not necessary to restart it
