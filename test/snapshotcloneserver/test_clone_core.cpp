...
class TestCloneCoreImpl : public ::testing::Test {
 public:
    ...
    virtual void SetUp() {
        ...
        core_ = std::make_shared<CloneCoreImpl>(
            client_, metaStore_, dataStore_, snapshotRef_, cloneRef_, option);
        EXPECT_CALL(*client_, Mkdir(_, _)).WillOnce(Return(LIBCURVE_ERROR::OK));
 ...
 protected:
    ...
    void MockBuildFileInfoFromFileSuccess(std::shared_ptr<CloneTaskInfo> task);

    void MockCreateCloneFileSuccess(std::shared_ptr<CloneTaskInfo> task);

    void MockCloneMetaSuccess(std::shared_ptr<CloneTaskInfo> task);

    void MockCreateCloneChunkSuccess(std::shared_ptr<CloneTaskInfo> task);

    void MockCompleteCloneMetaSuccess(std::shared_ptr<CloneTaskInfo> task);

    void MockRecoverChunkSuccess(std::shared_ptr<CloneTaskInfo> task);

    void MockChangeOwnerSuccess(std::shared_ptr<CloneTaskInfo> task);

    void MockRenameCloneFileSuccess(std::shared_ptr<CloneTaskInfo> task);

    void MockCompleteCloneFileSuccess(std::shared_ptr<CloneTaskInfo> task);

    void MockBuildFileInfoFromSnapshotFail(std::shared_ptr<CloneTaskInfo> task);

    void MockBuildFileInfoFromFileFail(std::shared_ptr<CloneTaskInfo> task);

    void MockCreateCloneFileFail(std::shared_ptr<CloneTaskInfo> task);

    void MockCloneMetaFail(std::shared_ptr<CloneTaskInfo> task);

    void MockCreateCloneChunkFail(std::shared_ptr<CloneTaskInfo> task);

    void MockCompleteCloneMetaFail(std::shared_ptr<CloneTaskInfo> task);

    void MockRecoverChunkFail(std::shared_ptr<CloneTaskInfo> task);

    void MockChangeOwnerFail(std::shared_ptr<CloneTaskInfo> task);

    void MockRenameCloneFileFail(std::shared_ptr<CloneTaskInfo> task);

    void MockCompleteCloneFileFail(std::shared_ptr<CloneTaskInfo> task);
...
TEST_F(TestCloneCoreImpl, TestClonePreForSnapSuccess) {
    ...
    EXPECT_CALL(*metaStore_, GetSnapshotInfo(source, _))
        .WillOnce(DoAll(SetArgPointee<1>(snap), Return(kErrCodeSuccess)));

    EXPECT_CALL(*metaStore_, AddCloneInfo(_)).WillOnce(Return(kErrCodeSuccess));

    int ret = core_->CloneOrRecoverPre(source, user, destination, lazyFlag,
                                       CloneTaskType::kClone, kDefaultPoolset,
                                       &cloneInfoOut);
...
TEST_F(TestCloneCoreImpl, TestClonePreForSnapTaskExist) {
    ...
    CloneInfo info1("taskid1", user, CloneTaskType::kClone, source, destination,
                    kDefaultPoolset, 100, 101, 0, CloneFileType::kSnapshot,
                    lazyFlag, CloneStep::kCreateCloneFile,
                    CloneStatus::cloning);
    cloneInfoList.push_back(info1);
    EXPECT_CALL(*metaStore_, GetCloneInfoByFileName(destination, _))
        .WillOnce(
            DoAll(SetArgPointee<1>(cloneInfoList), Return(kErrCodeSuccess)));
...
TEST_F(TestCloneCoreImpl, TestClonePreForSnapFailOnFileExist) {
    ...
    std::vector<CloneInfo> cloneInfoList;
    CloneInfo info1("taskid1", user, CloneTaskType::kRecover, source,
                    destination, "", 100, 101, 0, CloneFileType::kSnapshot,
                    lazyFlag, CloneStep::kCreateCloneFile,
                    CloneStatus::cloning);
    cloneInfoList.push_back(info1);
    EXPECT_CALL(*metaStore_, GetCloneInfoByFileName(destination, _))
...
TEST_F(TestCloneCoreImpl, TestClonePreDestinationAndTaskExist) {
    ...
    std::vector<CloneInfo> cloneInfoList;
    CloneInfo info1("taskid1", user, CloneTaskType::kClone, source, destination,
                    kDefaultPoolset, 100, destId, 0, CloneFileType::kFile,
                    lazyFlag, CloneStep::kRecoverChunk,
...
TEST_F(TestCloneCoreImpl, TestRecoverPreDestinationNotExist) {
    ...
    int ret =
        core_->CloneOrRecoverPre(source, user, destination, lazyFlag,
...
TEST_F(TestCloneCoreImpl, TestClonePreForFileSetCloneFileStatusReturnNotExist) {
    ...
    EXPECT_CALL(*client_, SetCloneFileStatus(source, FileStatus::BeingCloned,
                                             option.mdsRootUser))
        .WillOnce(Return(-LIBCURVE_ERROR::NOTEXIST));
...
TEST_F(TestCloneCoreImpl,
    ...
    CloneInfo info("id1", "user1", CloneTaskType::kClone, "snapid1", "file1",
                   kDefaultPoolset, CloneFileType::kSnapshot, true);
...
TEST_F(TestCloneCoreImpl, HandleCloneOrRecoverTaskFailOnGetSnapshotInfo) {
    CloneInfo cinfo("id1", "user1", CloneTaskType::kClone, "snapid1", "file1",
                    kDefaultPoolset, CloneFileType::kSnapshot, true);
    ...
    SnapshotInfo info(uuid, user, fileName, desc, seqnum, chunksize,
                      segmentsize, filelength, 0, 0, "default", time, status);

    EXPECT_CALL(*metaStore_, GetSnapshotInfo(_, _))
        .WillRepeatedly(
            DoAll(SetArgPointee<1>(info), Return(kErrCodeInternalError)));
...
TEST_F(TestCloneCoreImpl, HandleCloneOrRecoverTaskFailOnCreateCloneFile) {
    CloneInfo info("id1", "user1", CloneTaskType::kClone, "snapid1", "file1",
                   kDefaultPoolset, CloneFileType::kSnapshot, true);
    info.SetStatus(CloneStatus::cloning);
    auto cloneMetric = std::make_shared<CloneInfoMetric>("id1");
...
TEST_F(TestCloneCoreImpl, HandleCloneOrRecoverTaskStage2SuccessForCloneByFile) {
    CloneInfo info("id1", "user1", CloneTaskType::kClone, "snapid1", "file1",
                   kDefaultPoolset, 1, 2, 100, CloneFileType::kFile, true,
                   CloneStep::kRecoverChunk, CloneStatus::cloning);
    info.SetStatus(CloneStatus::cloning);
    auto cloneMetric = std::make_shared<CloneInfoMetric>("id1");
    auto cloneClosure = std::make_shared<CloneClosure>();
...
TEST_F(TestCloneCoreImpl,
    ...
    EXPECT_CALL(*client_, GetFileInfo(_, _, _))
        .WillRepeatedly(
            DoAll(SetArgPointee<2>(fInfo), Return(LIBCURVE_ERROR::OK)));
...
TEST_F(TestCloneCoreImpl, HandleCloneOrRecoverTaskStepUnknown) {
    CloneInfo info("id1", "user1", CloneTaskType::kClone, "snapid1", "file1",
                   kDefaultPoolset, CloneFileType::kSnapshot, true);
    info.SetStatus(CloneStatus::cloning);
...
void TestCloneCoreImpl::MockBuildFileInfoFromSnapshotSuccess(
    ...
    Status status = Status::done;
    SnapshotInfo info(uuid, user, fileName, desc, seqnum, chunksize,
                      segmentsize, filelength, 0, 0, kDefaultPoolset, time,
                      status);

    EXPECT_CALL(*metaStore_, GetSnapshotInfo(_, _))
        .WillRepeatedly(DoAll(SetArgPointee<1>(info), Return(kErrCodeSuccess)));
    ...
    EXPECT_CALL(*dataStore_, GetChunkIndexData(_, _))
        .WillOnce(DoAll(SetArgPointee<1>(snapMeta), Return(kErrCodeSuccess)));
...
void TestCloneCoreImpl::MockCreateCloneFileSuccess(
    ...
    EXPECT_CALL(*client_, CreateCloneFile(_, _, _, _, _, _, _, _, _, _))
        .WillOnce(
            DoAll(SetArgPointee<9>(fInfoOut), Return(LIBCURVE_ERROR::OK)));
}
...
void TestCloneCoreImpl::MockCreateCloneChunkSuccess(
    ...
    if (CloneFileType::kSnapshot == task->GetCloneInfo().GetFileType()) {
        location1 = LocationOperator::GenerateS3Location("file1-0-1");
        location2 = LocationOperator::GenerateS3Location("file1-1-1");
    } else {
        location1 = LocationOperator::GenerateCurveLocation(
            task->GetCloneInfo().GetSrc(), std::stoull("0"));
        location2 = LocationOperator::GenerateCurveLocation(
            task->GetCloneInfo().GetSrc(), std::stoull("1048576"));
    }
    ...
    EXPECT_CALL(*client_, CreateCloneChunk(AnyOf(location1, location2), _, _,
                                           correctSn, _, _))
        .WillRepeatedly(DoAll(
            Invoke([](const std::string& location,
                      const ChunkIDInfo& chunkidinfo, uint64_t sn, uint64_t csn,
...
void TestCloneCoreImpl::MockRecoverChunkSuccess(
    ...
    EXPECT_CALL(*client_, RecoverChunk(_, _, _, _))
        .WillRepeatedly(
            DoAll(Invoke([](const ChunkIDInfo& chunkidinfo, uint64_t offset,
                            uint64_t len, SnapCloneClosure* scc) {
                      scc->SetRetCode(LIBCURVE_ERROR::OK), scc->Run();
                  }),
                  Return(LIBCURVE_ERROR::OK)));
}
...
void TestCloneCoreImpl::MockBuildFileInfoFromSnapshotFail(
    ...
    EXPECT_CALL(*client_, GetFileInfo(_, _, _))
        .WillRepeatedly(
            DoAll(SetArgPointee<2>(fInfo), Return(-LIBCURVE_ERROR::FAILED)));
}
...
void TestCloneCoreImpl::MockCreateCloneFileFail(
    ...
    EXPECT_CALL(*client_, CreateCloneFile(_, _, _, _, _, _, _, _, _, _))
        .WillOnce(
            DoAll(SetArgPointee<9>(fInfoOut), Return(-LIBCURVE_ERROR::FAILED)));
}

void TestCloneCoreImpl::MockCloneMetaFail(std::shared_ptr<CloneTaskInfo> task) {
...
void TestCloneCoreImpl::MockRecoverChunkFail(
    ...
    EXPECT_CALL(*client_, RecoverChunk(_, _, _, _))
        .WillRepeatedly(DoAll(
            Invoke([](const ChunkIDInfo& chunkidinfo, uint64_t offset,
                      uint64_t len, SnapCloneClosure* scc) { scc->Run(); }),
            Return(-LIBCURVE_ERROR::FAILED)));
}
...
TEST_F(TestCloneCoreImpl, TestCleanOrRecoverTaskPreSuccess) {
    ...
    int ret = core_->CleanCloneOrRecoverTaskPre(user, taskId, &cloneInfoOut);
    ASSERT_EQ(kErrCodeSuccess, ret);
}
...
TEST_F(TestCloneCoreImpl, TestCleanOrRecoverTaskPreGetCloneInfoFail) {
    ...
    EXPECT_CALL(*metaStore_, GetCloneInfo(_, _))
        .WillOnce(DoAll(SetArgPointee<1>(cinfo), Return(-1)));
...
TEST_F(TestCloneCoreImpl, TestHandleCleanCloneOrRecoverTaskSuccess) {
    ...
    EXPECT_CALL(*metaStore_, DeleteCloneInfo(_)).WillOnce(Return(0));
...
TEST_F(TestCloneCoreImpl, TestCheckFileExists) {
    FInfo fInfo;
    fInfo.id = 100;
    EXPECT_CALL(*client_, GetFileInfo(_, _, _))
        .WillOnce(DoAll(SetArgPointee<2>(fInfo), Return(LIBCURVE_ERROR::OK)));
...
TEST_F(TestCloneCoreImpl, TestHandleDeleteCloneInfoSnapDeleteCloneInfoFail) {
    CloneInfo info("id1", "user1", CloneTaskType::kClone, "snapid1", "file1",
                   kDefaultPoolset, CloneFileType::kSnapshot, false);
    EXPECT_CALL(*metaStore_, DeleteCloneInfo(_)).WillOnce(Return(-1));
...
TEST_F(TestCloneCoreImpl, TestHandleDeleteCloneInfoSnapSuccess) {
    CloneInfo info("id1", "user1", CloneTaskType::kClone, "snapid1", "file1",
                   kDefaultPoolset, CloneFileType::kSnapshot, false);
...
TEST_F(TestCloneCoreImpl,
    ...
    CloneInfo info("id1", "user1", CloneTaskType::kClone, "source1", "file1",
                   kDefaultPoolset, CloneFileType::kFile, false);
    info.SetStatus(CloneStatus::metaInstalled);
    EXPECT_CALL(*metaStore_, DeleteCloneInfo(_)).WillOnce(Return(0));
...
TEST_F(TestCloneCoreImpl, TestHandleDeleteCloneInfoFileSetStatusFail) {
    CloneInfo info("id1", "user1", CloneTaskType::kClone, "source1", "file1",
                   kDefaultPoolset, CloneFileType::kFile, false);
    info.SetStatus(CloneStatus::metaInstalled);
    cloneRef_->IncrementRef("source1");
    ASSERT_EQ(cloneRef_->GetRef("source1"), 1);
    EXPECT_CALL(*client_, SetCloneFileStatus(_, _, _)).WillOnce(Return(-1));
    ASSERT_EQ(core_->HandleDeleteCloneInfo(info), kErrCodeInternalError);
    ASSERT_EQ(cloneRef_->GetRef("source1"), 1);
}

TEST_F(TestCloneCoreImpl, TestHandleDeleteCloneInfoFileDeleteCloneInfoFail) {
    CloneInfo info("id1", "user1", CloneTaskType::kClone, "source1", "file1",
                   kDefaultPoolset, CloneFileType::kFile, false);
    info.SetStatus(CloneStatus::metaInstalled);
    EXPECT_CALL(*metaStore_, DeleteCloneInfo(_)).WillOnce(Return(-1));
...
TEST_F(TestCloneCoreImpl, TestHandleDeleteCloneInfoFileSuccess) {
    CloneInfo info("id1", "user1", CloneTaskType::kClone, "source1", "file1",
                   kDefaultPoolset, CloneFileType::kFile, false);
    info.SetStatus(CloneStatus::metaInstalled);
    EXPECT_CALL(*metaStore_, DeleteCloneInfo(_)).WillOnce(Return(0));
