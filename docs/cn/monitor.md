...
`bvar::Maxer<T>` : 求最大值，默认std::numeric_limits::max()，varname << N相当于varname = max(varname, N)。

`bvar::Miner<T>` : 求最小值，默认std::numeric_limits::min()，varname << N相当于varname = min(varname, N)。
