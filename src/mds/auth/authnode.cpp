...
bool AuthNodeImpl::Init(const AuthOption &option) {
    ...
    if (keyMap_.find(curve::common::AUTH_ROLE) == keyMap_.end()) {
        Key key;
...
AuthStatusCode AuthNodeImpl::AddKey(const std::string &encKey) {
    ...
    if (!key.ParseFromString(decKeyStr)) {
        LOG(ERROR) << "Add key failed, parse key failed, decrypt key = "
                   << decKeyStr;
        return AuthStatusCode::AUTH_DECRYPT_FAILED;
    }

    WriteLockGuard keyMapLG(keyMapMutex_);
    if (keyMap_.find(key.id()) != keyMap_.end()) {
        LOG(WARNING) << "Add key but key exist and will update it, oldkey = "
                     << keyMap_[key.id()].DebugString() << ", newkey = "
                     << key.DebugString();
    } else {
        LOG(INFO) << "Add key, key = " << key.DebugString();
    }
    keyMap_[key.id()] = key;
    if (!storage_->StorageKey(key)) {
...
AuthStatusCode AuthNodeImpl::UpdateKey(const std::string &encKey) {
    std::string decKeyStr;
    auto ret = DecryptByAuthKey(encKey, &decKeyStr);
    if (AuthStatusCode::AUTH_OK != ret) {
        return ret;
    ...
    WriteLockGuard keyMapLG(keyMapMutex_);
    if (keyMap_.find(key.id()) == keyMap_.end()) {
        LOG(WARNING) << "Update auth key not exist, will add it, key = "
                     << key.DebugString();
    } else {
