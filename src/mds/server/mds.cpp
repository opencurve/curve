...
void MDS::InitMdsOptions(std::shared_ptr<Configuration> conf) {
    ...
    InitAuthClientOption(&options_.authClientOption);
...
void MDS::Init() {
    ...
    InitAuthServiceManager(options_.authOption, options_.authClientOption);
    // init auth client
    curve::client::MetaServerOption metaServerOption;
    metaServerOption.rpcRetryOpt.addrs = {options_.mdsListenAddr};
    curve::client::AuthClient::GetInstance().Init(
        metaServerOption, options_.authClientOption);
    fileLockManager_ =
...
void MDS::InitAuthClientOption(curve::common::AuthClientOption *option) {
    conf_->GetBoolValue("auth.client.enable", &option->enable);
    if (option->enable) {
        conf_->GetValueFatalIfFail("auth.client.id", &option->clientId);
        conf_->GetValueFatalIfFail("auth.key.current", &option->key);
        LOG_IF(FATAL, !common::Encryptor::AESKeyValid(option->key))
            << "auth.key.current length is invalid"
            << ", key length: " << option->key.length();
        ...
        conf_->GetUInt32Value("auth.client.ticket.refresh.intervalSec",
            &option->ticketRefreshIntervalSec);
        conf_->GetUInt32Value("auth.client.ticket.refresh.threshold",
            &option->ticketRefreshThresholdSec);
    }
}
...
void MDS::InitAuthServiceManager(const AuthOption &option,
    ...
    if (!authNode->Init(option)) {
        LOG(ERROR) << "InitAuthServiceManager failed.";
        return;
    }
