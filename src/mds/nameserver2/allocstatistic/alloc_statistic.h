...
class AllocStatistic {
 ...
 private:
     /**
     * @brief CalculateSegmentAlloc Get all the segment records of the specified
     *                              revision from Etcd
     */
 ...
 private:
    ...
    // When stop_ is true, stop the persistent thread and the statistical thread
    // that counts the segment allocation in Etcd
    Atomic<bool> stop_;
