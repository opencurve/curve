...
class CopysetConfGenerator {
 ...
 private:
    ...
    *
    * @return ::curve::mds::topology::UNINTIALIZE_ID if there isn't any configuration to dispatch，
    *         otherwise other value apart from UNINTIALIZE_ID
    */
    ...
    * @param[out] copysetConf configuration instruction generated by the scheduler
    *
 ...
 private:
    ...
    // MDS will start cleaning copysets some time after it started rather than start immediately (delay cleaning), 
    // this can avoid error in cases like the example below:
    // 1. An operator: ABC+D(epoch: 8) generated by MDS, and has been dispatched to leader
    // 2. MDS restart and the operator has lost, the configuration recorded by MDS is ABC(epoch: 8)
    ...
    //    In this case instruction would have been dispatched to clean copyset on, but it shouldn't
    //    since D is already a member of the copyset at this moment
    // Why delay cleaning?
    // In normal cases the leader will report candidates within a heartbeat, and candidates
    // willnot be cleaned even if they report expired config after that.
    steady_clock::time_point mdsStartTime_;
    // after started for this peroid of time, clean date on followers is enable
