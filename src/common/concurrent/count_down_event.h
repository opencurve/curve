...
/**
 *Used for inter thread synchronization, CountDownEvent is implemented through a counter, which
 *The initial value initCnt is the total number of events that need to be waited for through the interface Wait. Whenever a
 *When an event occurs, the Signal interface will be called, causing the counter value to decrease by 1. When the counter value reaches
 *When it reaches 0, the Wait wait will end. Usually used to wait for some events to occur
 */
