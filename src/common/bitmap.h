...
class Bitmap {
 public:
     ...
     *The constructor will create a new bitmap internally, and then use the bitmap memcpy in the parameters
     ...
     * @param startIndex: The starting position of the range, including this position
     ...
     * @param index: Refers to the location of the positioning
     * @return: true indicates that the current bit status is 1, while false indicates that it is 0
     */
    bool Test(uint32_t index) const;
    /**
     *Obtain the specified position and the position after which the first bit is 1
     * @param index: Refers to the location of the positioning, including this location
     * @return: The position where the first bit is 1. If it does not exist, return NO_POS
     */
    uint32_t NextSetBit(uint32_t index) const;
    /**
     *Gets the position where the first bit between the specified start position and end position is 1
     * @param startIndex: The starting position, including this position
     * @param endIndex: End position, including this position
     * @return: The position where the first bit is 1. If it does not exist within the specified range, return NO_POS
     */
    uint32_t NextSetBit(uint32_t startIndex, uint32_t endIndex) const;
    /**
     *Obtain the specified position and the position after which the first bit is 0
     * @param index: Refers to the location of the positioning, including this location
     * @return: The position where the first bit is 0. If it does not exist, return NO_POS
     */
    uint32_t NextClearBit(uint32_t index) const;
    /**
     *Gets the position where the first bit between the specified start position and end position is 0
     * @param startIndex: The starting position, including this position
     * @param endIndex: End position, including this position
     * @return: The position where the first bit is 0. If it does not exist within the specified range, return NO_POS
     */
    uint32_t NextClearBit(uint32_t startIndex, uint32_t endIndex) const;
    /**
     *Divide the designated area of the bitmap into several continuous areas based on bit states, with consistent bit states within the continuous areas
     *For example, 00011100 will be divided into three regions: [0,2], [3,5], [6,7]
     * @param startIndex: The starting index of the specified region
     * @param endIndex: The end index of the specified range
     * @param clearRanges: A vector that stores a continuous region with a bit state of 0, which can be specified as nullptr
     * @param setRanges: A vector that stores a continuous region with a bit state of 1, which can be specified as nullptr
     */
 ...
 public:
    //Represents a non-existent position, with a value of 0xffffff
