...
bool Configuration::LoadConfig() {
    ...
    if (cFile.is_open()) {
        ...
        while (getline(cFile, line)) {
            ...
            std::string value =
                line.substr(delimiterPos + 1, commentPos - delimiterPos - 1);
            SetValue(key, value);
        }
    } else {
...
void Configuration::SetUInt64Value(const std::string& key,
...
double Configuration::GetFloatValue(const std::string& key,
                                    float defaultvalue) {
    std::string value = GetValue(key);
