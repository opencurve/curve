...
std::string Encryptor::Base64Decode(const std::string& src) {
    ...
    int decodedLength = BIO_read(bio, buffer, length);

    if (decodedLength > 0) {
        decodedString.assign(buffer, decodedLength);
    }
...
int Encryptor::AESEncrypt(const std::string &key,
    ...
    for (auto i = 0; i < pad_len; i++) {
        data += static_cast<char>(pad_len);
    }

    int ret = AES_set_encrypt_key(
        reinterpret_cast<const unsigned char*>(key.c_str()), keyBits, &enc_key);
    if (ret != 0) {
        LOG(ERROR) << "AES_set_encrypt_key failed, error = " << ret;
        return ret;
    }
    unsigned char* enc_out = new unsigned char[encLength];
    memset(enc_out, 0, sizeof(unsigned char) * encLength);
    AES_cbc_encrypt(reinterpret_cast<const unsigned char*>(data.c_str()),
        enc_out, data.size(), &enc_key, uiv, AES_ENCRYPT);
    *out = std::string(reinterpret_cast<const char*>(enc_out), encLength);
    delete[] enc_out;
    return 0;
}
...
int Encryptor::AESDecrypt(const std::string &key,
    ...
    if (ret != 0) {
        LOG(ERROR) << "AES_set_decrypt_key failed, error = " << ret;
        return ret;
    }
    unsigned char *dec_out = new unsigned char[in.size()];
    memset(dec_out, 0, sizeof(unsigned char) * in.size());
    AES_cbc_encrypt(reinterpret_cast<const unsigned char*>(in.c_str()),
        dec_out, in.size(), &dec_key, uiv, AES_DECRYPT);
    ...
    auto pad_len = std::min<std::uint8_t>(dec_out[in.size() - 1],
        AES_BLOCK_SIZE);
    if (pad_len > AES_BLOCK_SIZE || pad_len >= in.size()) {
        return -1;
    }
    *out = std::string(reinterpret_cast<const char*>(dec_out),
        in.size() - pad_len);
    delete[] dec_out;
    return 0;
}
...
bool Authenticator::VerifyCredential(
    ...
    if (token.encticket().empty() || token.encclientidentity().empty()) {
        LOG(ERROR) << "token info is not enough";
        return false;
    }
    // decrypt ticket
    Ticket ticket;
    ...
    if (needRedecByLastKey) {
        ...
        if (options_.lastKey.size() == 0 || LastKeyExpired()) {
            LOG(ERROR) << "aes decrypt ticket failed with current key and last"
                       << " key is empty or expiration when VerifyCredential"
                       << ", lastKey size = " << options_.lastKey.size()
            ...
            if (ret != 0) {
                LOG(ERROR) << "aes decrypt ticket failed with last key"
    ...
    if (ticket.expiration() < now) {
        LOG(ERROR) << "ticket is expired, now = " << now
    ...
    if (ret != 0) {
        LOG(ERROR) << "aes decrypt cInfo failed when VerifyCredential";
        return false;
    }
    ...
    if (cInfo.timestamp() + options_.requestTTL < now ||
        ...
        LOG(ERROR) << "cid in ticket is not equal to decrypted from encId"
                   << " or request expiration, cid in ticket = " << ticket.cid()
