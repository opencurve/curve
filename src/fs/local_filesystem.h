...
class LocalFileSystem {
 public:
     ...
     * If the file system has not been formatted yet, it will be formatted
     * first, Then mount the file system, Formatted or mounted file systems will
     * not be repeatedly formatted or mounted
     * @param option: initialization parameters
     */
    virtual int Init(const LocalFileSystemOption& option) = 0;

    /**
     * Obtain the file system status information where the file or directory is
     * located
     ...
     * @return successfully returns the file handle id, while failure returns a
     * negative value
     */
     ...
     * If the deleted object is a directory, the files or subdirectories under
     ...
     * Renaming or moving files or directories to a different path will not
     * overwrite existing files
     * @param oldPath: Path to the original file or directory
     * @param newPath: New file or directory path
     * The new file or directory does not exist before renaming, otherwise an
     ...
     * @return Successfully returned 0
     */
    virtual int Rename(const string& oldPath, const string& newPath,
     ...
     * @return returns the length of the data successfully read, while failure
     * returns -1
     */
     ...
     * @return returns the length of successfully written data, while failure
     * returns -1
     */
     ...
     * @return returns the length of successfully added data, while failure
     * returns -1
     */
...
class LocalFsFactory {
 public:
     ...
     * The factory method of the local file system creates corresponding objects
     * based on the type passed in The file system created by this interface
     ...
     */
    static std::shared_ptr<LocalFileSystem> CreateFs(
        FileSystemType type, const std::string& deviceID);
};
