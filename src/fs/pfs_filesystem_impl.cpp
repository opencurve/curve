...
ssize_t preadv_dispatch(int fd, const struct iovec *vector,
    ...
    if (PFS_FD_ISVALID(fd)) {
        if (butil::iobuf::iobuf_is_user())
            return pfs_preadv(fd, vector, count, offset);
        else
            return pfs_preadv_dma(fd, vector, count, offset);
    }
...
ssize_t pwritev_dispatch(int fd, const struct iovec *vector,
    ...
    if (PFS_FD_ISVALID(fd)) {
        if (butil::iobuf::iobuf_is_user())
            return pfs_pwritev(fd, vector, count, offset);
        else
            return pfs_pwritev_dma(fd, vector, count, offset);
    }
    return pwritev(fd, vector, count, offset);
}

ssize_t readv_dispatch(int fd, const struct iovec *vector, int count) {
    if (PFS_FD_ISVALID(fd)) {
        if (butil::iobuf::iobuf_is_user())
            return pfs_readv(fd, vector, count);
        else
            return pfs_readv_dma(fd, vector, count);
    }
    return readv(fd, vector, count);
}

ssize_t writev_dispatch(int fd, const struct iovec *vector, int count) {
    if (PFS_FD_ISVALID(fd)) {
        if (butil::iobuf::iobuf_is_user())
            return pfs_writev(fd, vector, count);
        else
            return pfs_writev_dma(fd, vector, count);
    }
...
int PfsFileSystemImpl::Sync(int fd) {
    return 0;
}
