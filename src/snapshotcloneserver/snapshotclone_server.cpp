...
void InitClientOption(std::shared_ptr<Configuration> conf,
    ...
    conf->GetValueFatalIfFail("client.config_path", &clientOption->configPath);
    conf->GetValueFatalIfFail("mds.rootUser", &clientOption->mdsRootUser);
...
void InitSnapshotCloneServerOptions(std::shared_ptr<Configuration> conf,
    ...
    conf->GetValueFatalIfFail("server.address", &serverOption->addr);
    conf->GetValueFatalIfFail("server.clientAsyncMethodRetryTimeSec",
                              &serverOption->clientAsyncMethodRetryTimeSec);
    conf->GetValueFatalIfFail("server.clientAsyncMethodRetryIntervalMs",
    ...
    conf->GetValueFatalIfFail("server.snapshotTaskManagerScanIntervalMs",
                              &serverOption->snapshotTaskManagerScanIntervalMs);
    conf->GetValueFatalIfFail("server.chunkSplitSize",
                              &serverOption->chunkSplitSize);
    conf->GetValueFatalIfFail("server.checkSnapshotStatusIntervalMs",
    ...
    conf->GetValueFatalIfFail("server.cloneTaskManagerScanIntervalMs",
    ...
    conf->GetValueFatalIfFail("mds.rootUser", &serverOption->mdsRootUser);
    ...
    conf->GetValueFatalIfFail(
        "server.backEndReferenceRecordScanIntervalMs",
        &serverOption->backEndReferenceRecordScanIntervalMs);
    conf->GetValueFatalIfFail(
        "server.backEndReferenceFuncScanIntervalMs",
...
bool SnapShotCloneServer::InitEtcdClient(void) {
    ...
    if (res != EtcdErrCode::EtcdOK) {
        LOG(ERROR) << "init etcd client err! "
                   << "etcdaddr: "
                   << snapshotCloneServerOptions_.etcdConf.Endpoints
                   << ", etcdaddr len: "
                   << snapshotCloneServerOptions_.etcdConf.len
                   << ", etcdtimeout: "
    ...
    if (res != EtcdErrCode::EtcdOK && res != EtcdErrCode::EtcdKeyNotExist) {
        LOG(ERROR) << "Run snapsthotcloneserver err. Check if etcd is running.";
        return false;
    }
...
void SnapShotCloneServer::StartCompaginLeader(void) {
    ...
    while (0 != leaderElection_->CampaignLeader()) {
        LOG(INFO) << option.leaderUniqueName << " campaign for leader again";
    }
...
bool SnapShotCloneServer::Init() {
    ...
    metaStore_ =
        std::make_shared<SnapshotCloneMetaStoreEtcd>(etcdClient_, codec);
    ...
    snapshotCore_ = std::make_shared<SnapshotCoreImpl>(
        client_, metaStore_, dataStore_, snapshotRef_,
        snapshotCloneServerOptions_.serverOption);
    ...
    snapshotTaskManager_ =
        std::make_shared<SnapshotTaskManager>(snapshotCore_, snapshotMetric_);
    snapshotServiceManager_ = std::make_shared<SnapshotServiceManager>(
        snapshotTaskManager_, snapshotCore_);
    ...
    cloneCore_ = std::make_shared<CloneCoreImpl>(
        client_, metaStore_, dataStore_, snapshotRef_, cloneRef_,
        snapshotCloneServerOptions_.serverOption);
    ...
    if (cloneServiceManager_->Init(snapshotCloneServerOptions_.serverOption) <
        0) {
        LOG(ERROR) << "CloneServiceManager init fail.";
        return false;
    }
    service_ = std::make_shared<SnapshotCloneServiceImpl>(
        snapshotServiceManager_, cloneServiceManager_);
...
bool SnapShotCloneServer::Start(void) {
    // Start the clone service first and then the snapshot service, because
    // there is a clone reference when deleting snapshot dependencies
...
void SnapShotCloneServer::RunUntilQuit(void) { server_->RunUntilAskedToQuit(); }
