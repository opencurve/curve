...
struct FileSnapMap {
    ...
    /**
     * @brief to obtain whether the current chunk data exists in the current mapping table
     *
...
class SnapshotCore {
 public:
     ...
     *The first step is to build a snapshot file mapping and put MateObj
...
class SnapshotCoreImpl : public SnapshotCore {
 public:
    ...
    //Public interface definition can be found in the SnapshotCore interface annotation
 ...
 private:
    ...
    /**
     * @brief Attempt to clean up failed snapshots before creating them, otherwise they may fail again
     *
     * @param task snapshot task information
     * @return error code
     */
 ...
 private:
    ...
    //Lock the file name of the snapshot to prevent concurrent snapshots. Snapshots of the same file need to be queued
