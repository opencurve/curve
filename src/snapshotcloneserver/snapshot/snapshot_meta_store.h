...
#ifndef SRC_SNAPSHOTCLONESERVER_SNAPSHOT_SNAPSHOT_META_STORE_H_
#define SRC_SNAPSHOTCLONESERVER_SNAPSHOT_SNAPSHOT_META_STORE_H_
...
class DBSnapshotMetaStore : public SnapshotMetaStore{
 public:
    explicit DBSnapshotMetaStore(
        std::shared_ptr<curve::snapshotcloneserver::SnapshotRepo> repo)
        :repo_(repo) {}
    ~DBSnapshotMetaStore() {}
    int Init() override;
 ...
 private:
    ...
    // db metastore的repo实现
    std::shared_ptr<curve::snapshotcloneserver::SnapshotRepo> repo_;
...
#endif  // SRC_SNAPSHOTCLONESERVER_SNAPSHOT_SNAPSHOT_META_STORE_H_
