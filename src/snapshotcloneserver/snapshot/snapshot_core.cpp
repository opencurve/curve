...
/**
 * @brief Asynchronous execution of snapshot creation task and update of task
 * progress
 *
 * The snapshot schedule is planned as follows:
 *
 *  |CreateSnapshotOnCurvefs| BuildChunkIndexData | BuildSnapshotMap |
 * TransferSnapshotData | UpdateSnapshot | //NOLINT | 5%                    | 6%
 * | 10%              | 10%~99%              | 100%           | //NOLINT
 *
 *
 * Explanation of errors and cancellations during asynchronous execution:
 * 1. The occurrence of an error will cause the entire asynchronous task to be
 * directly interrupted without any cleaning action: When an error occurs, there
 * is usually an abnormality in the system, and the cleaning action may not be
 * completed, Therefore, no cleaning will be carried out, only the status will
 * be set, and after manual intervention to eliminate anomalies, Use the
 * DeleteSnapshot function to manually delete snapshots with error status.
 * 2. When a cancel occurs, the cleaning actions are carried out in reverse
 * order of creating functions, If an error occurs during the cleaning process,
 * it will be immediately interrupted, followed by the same error process.
 *
...
void SnapshotCoreImpl::HandleCreateSnapshotTask(
    ...
    // If there are currently failed snapshots, it is necessary to clean up the
    // failed snapshots first, otherwise the snapshots will fail again
    ...
    // To support task restart, there are three situations that need to be
    // addressed
    // 1. I haven't taken a snapshot, there's no seqNum, and there's no snapshot
    // on the curve
    // 2. I have taken a snapshot, and there is seqNum and a snapshot on the
    // curve
    // 3. I have taken a snapshot and have completed the dump to delete it.
    // There is seqNum, but there is no snapshot on the curve
    ...
    if (kUnInitializeSeqNum == seqNum) {
        ...
        FInfo snapInfo;
        ret =
            client_->GetSnapshot(fileName, info->GetUser(), seqNum, &snapInfo);
    ...
    ret = BuildSnapshotMap(fileName, seqNum, &fileSnapshotMap);
    if (ret < 0) {
        LOG(ERROR) << "BuildSnapshotMap error, "
                   << " fileName = " << task->GetFileName()
                   << ", seqNum = " << seqNum << ", uuid = " << task->GetUuid();
        HandleCreateSnapshotError(task);
        return;
    }
    ...
    if (existIndexData) {
        ret = TransferSnapshotData(
            indexData, *info, segInfos,
    ...
    if (task->IsCanceled()) {
        return CancelAfterTransferSnapshotData(task, indexData,
                                               fileSnapshotMap);
    }
...
int SnapshotCoreImpl::StartCancel(std::shared_ptr<SnapshotTaskInfo> task) {
...
void SnapshotCoreImpl::CancelAfterTransferSnapshotData(
    std::shared_ptr<SnapshotTaskInfo> task, const ChunkIndexData& indexData,
...
void SnapshotCoreImpl::CancelAfterCreateChunkIndexData(
    ...
    UUID uuid = task->GetUuid();
    uint64_t seqNum = info.GetSeqNum();
    ChunkIndexDataName name(task->GetFileName(), seqNum);
    int ret = dataStore_->DeleteChunkIndexData(name);
    if (ret < 0) {
        LOG(ERROR) << "DeleteChunkIndexData error "
                   << "while canceling CreateSnapshot, "
                   << " ret = " << ret << ", fileName = " << task->GetFileName()
                   << ", seqNum = " << seqNum << ", uuid = " << task->GetUuid();
        HandleCreateSnapshotError(task);
        return;
    }
...
int SnapshotCoreImpl::CreateSnapshotOnCurvefs(
    const std::string& fileName, SnapshotInfo* info,
    std::shared_ptr<SnapshotTaskInfo> task) {
    uint64_t seqNum = 0;
    int ret = client_->CreateSnapshot(fileName, info->GetUser(), &seqNum);
    if (LIBCURVE_ERROR::OK == ret || -LIBCURVE_ERROR::UNDER_SNAPSHOT == ret) {
        // ok
    ...
    FInfo snapInfo;
    ret = client_->GetSnapshot(fileName, info->GetUser(), seqNum, &snapInfo);
    if (ret != LIBCURVE_ERROR::OK) {
        LOG(ERROR) << "GetSnapShot on curvefs fail, "
                   << " ret = " << ret << ", fileName = " << fileName
                   << ", user = " << info->GetUser() << ", seqNum = " << seqNum
                   << ", uuid = " << task->GetUuid();
        return kErrCodeInternalError;
    }
    ...
    // After taking a snapshot, you need to wait for 2 sessions to ensure that
    // the seq is synchronized to all clients
...
int SnapshotCoreImpl::DeleteSnapshotOnCurvefs(const SnapshotInfo& info) {
    ...
    uint64_t seqNum = info.GetSeqNum();
    int ret = client_->DeleteSnapshot(fileName, user, seqNum);
    if (ret != LIBCURVE_ERROR::OK && ret != -LIBCURVE_ERROR::NOTEXIST &&
        ret != -LIBCURVE_ERROR::DELETING) {
        LOG(ERROR) << "DeleteSnapshot error, "
                   << " ret = " << ret << ", fileName = " << fileName
                   << ", user = " << user << ", seqNum = " << seqNum
                   << ", uuid = " << info.GetUuid();
        return kErrCodeInternalError;
    }
    do {
        FileStatus status;
        ret = client_->CheckSnapShotStatus(info.GetFileName(), info.GetUser(),
                                           seqNum, &status);
        LOG(INFO) << "Doing CheckSnapShotStatus, fileName = "
                  << info.GetFileName() << ", user = " << info.GetUser()
                  << ", seqNum = " << seqNum
...
int SnapshotCoreImpl::BuildChunkIndexData(
    const SnapshotInfo& info, ChunkIndexData* indexData,
    std::map<uint64_t, SegmentInfo>* segInfos,
    std::shared_ptr<SnapshotTaskInfo> task) {
    std::string fileName = info.GetFileName();
    std::string user = info.GetUser();
    ...
    for (uint64_t i = 0; i < fileLength / segmentSize; i++) {
        ...
        if (LIBCURVE_ERROR::OK == ret) {
            ...
            for (std::vector<uint64_t>::size_type j = 0;
                ...
                // 2 Sns, the smaller one is the snap snap snap, and the larger
                // one is the write after the snapshot 1 SN, there are two
                // situations:
                //      When it is less than or equal to seqNum, it is a snap
                //      snap and has not been written since the snapshot; When
                //      greater than, it indicates that it was blank when taking
                //      a snapshot, and is the version written for the first
                //      time after the snapshot (seqNum+1)
                // No sn, never written before
                // Greater than 2 sns, error, error reported
                if (chunkInfo.chunkSn.size() == 2) {
                    uint64_t seq =
                        std::min(chunkInfo.chunkSn[0], chunkInfo.chunkSn[1]);
                    ...
                    LOG(ERROR)
                        << "GetChunkInfo return chunkInfo.chunkSn.size()"
                        << " invalid, size = " << chunkInfo.chunkSn.size()
                        << ", uuid = " << task->GetUuid();
                    return kErrCodeInternalError;
                }
                if (task->IsCanceled()) {
...
int SnapshotCoreImpl::BuildSegmentInfo(
    const SnapshotInfo& info, std::map<uint64_t, SegmentInfo>* segInfos) {
    ...
    for (uint64_t i = 0; i < fileLength / segmentSize; i++) {
        uint64_t offset = i * segmentSize;
        SegmentInfo segInfo;
        ret = client_->GetSnapshotSegmentInfo(fileName, user, seq, offset,
                                              &segInfo);
...
int SnapshotCoreImpl::TransferSnapshotData(
    const ChunkIndexData indexData, const SnapshotInfo& info,
    ...
    for (auto& chunkIndex : chunkIndexVec) {
        ...
        if (chunkIndexInSegment >= it->second.chunkvec.size()) {
            LOG(ERROR) << "TransferSnapshotData, "
                       ...
                       << ", chunkIndexInSegment = " << chunkIndexInSegment
                       << ", size = " << it->second.chunkvec.size()
                       << ", uuid = " << task->GetUuid();
            return kErrCodeInternalError;
        }
    }
    ...
    for (auto& chunkIndex : chunkIndexVec) {
        ...
        auto it = segInfos.find(segNum);
        if (it != segInfos.end()) {
            ChunkIDInfo cidInfo = it->second.chunkvec[chunkIndexInSegment];
...
int SnapshotCoreImpl::DeleteSnapshotPre(UUID uuid, const std::string& user,
                                        const std::string& fileName,
                                        SnapshotInfo* snapInfo) {
    NameLockGuard lockSnapGuard(snapshotRef_->GetSnapshotLock(), uuid);
    int ret = metaStore_->GetSnapshotInfo(uuid, snapInfo);
    if (ret < 0) {
        // When the snapshot does not exist, it directly returns deletion
        // success, making the interface idempotent
        return kErrCodeSuccess;
    }
    ...
    if ((!fileName.empty()) && (fileName != snapInfo->GetFileName())) {
...
/**
 * @brief Asynchronous execution of delete snapshot task and update task
 * progress
 *
...
void SnapshotCoreImpl::HandleDeleteSnapshotTask(
    ...
    if (dataStore_->ChunkIndexDataExist(name)) {
        ...
        double progressPerData =
            static_cast<double>(totalProgress) / chunkDataNum;
        uint32_t index = 0;
...
int SnapshotCoreImpl::GetSnapshotInfo(const UUID uuid, SnapshotInfo* info) {
    ...
    for (auto& snap : snapInfos) {
        if (snap.GetSeqNum() != seqNum &&
            ...
            if (ret < 0) {
                ...
                // Otherwise, once a failed snapshot does not have indexdata,
                // all snapshots cannot be deleted
            } else {
