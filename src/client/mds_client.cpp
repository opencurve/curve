...
int RPCExcutorRetryPolicy::PreProcessBeforeRetry(int status, bool retryUnlimit,
    ...
    if (retryUnlimit) {
        ...
        // 1. Access to an existing IP address, but no one is listening: ECONNREFUSED
        // 2. In the normal RPC scenario, the remote process has crashed: EHOSTDOWN
        ...
        // When health checks are enabled, in the event of a failure in the underlying TCP connection,
        // RPC requests will directly return EHOSTDOWN locally. In this situation,
        // add some sleep time to avoid a large number of retry requests overwhelming bthread.
...
/**
 * Obtain the next MDS index to retry based on the input state. The MDS switching logic is as follows:
 ...
 * 2. If an RPC fails, it triggers a switch to curRetryMDSIndex. If at this point, lastWorkingMDSIndex is equal to 
 *    currentWorkingMDSIndex, then sequentially switch to the next MDS index.
 *    If lastWorkingMDSIndex is not equal to currentWorkingMDSIndex, 
 *    it means that another interface has updated currentWorkingMDSAddrIndex_,
 *    so this time, switch directly to currentWorkingMDSAddrIndex_.
 */
