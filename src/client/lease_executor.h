...
/**
 * Please refresh the result. If the session does not exist, there is no need to
 * renew it If the session exists but the lease renewal fails, continue to renew
 * the contract Successfully renewed the contract, FInfo_ Only in t will there
 * be corresponding file information
 */
struct LeaseRefreshResult {
    enum class Status { OK, FAILED, NOT_EXIST };
    Status status;
    FInfo_t finfo;
};
...
/**
 * The fileinstance corresponding to each vdisk will maintain heartbeat with the
 * mds Heartbeat is achieved through LeaseExecutor, which periodically Go to MDS
 * to renew the contract and bring back the latest version information of the
 * current file on the MDS side Then check if the version information has
 * changed, and if so, notify the iomanager Updated version. If the renewal
 * fails, the user's newly sent IO needs to be returned in error
 */
class LeaseExecutor : public LeaseExecutorBase {
 public:
    /**
     * Constructor
     * @param: leaseopt is the option configuration for the current lease
     * renewal
     * @param: mdsclient is a renewed client with mds
     * @param: iomanager will schedule IO in case of contract renewal failure or
     * version change
     */
     ...
     * @param: fi is the current version information of the file that needs to
     * be renewed
 ...
 private:
    /**
     * During a lease period, rfreshTimesPerLease will be renewed times,
     * increasing every time the renewal fails When consecutive renewals of
     * rfreshTimesPerLease fail times, disable IO
     */
 ...
 private:
    ...
    // IO manager, calls its interface when a file needs to update version
    // information or disable IO
    IOManager4File* iomanager_;

    // Configuration information for the current lease execution
    LeaseOption leaseoption_;

    // The lease information transmitted from the mds end, including the lease
    // duration of the current file and the sessionid
...
// Call OnTriggeringTask when the timer is triggered, and decide whether to
// continue timing triggering based on the return value If no longer triggered,
// call OnDestroyingTask for cleaning operation
