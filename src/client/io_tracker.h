...
// IOTracker is used to track a user's IO, as a user's IO may cross
// chunkservers, Therefore, when it is actually distributed, it will be split
// into multiple small IOs and sent down concurrently. Therefore, we need to
...
class CURVE_CACHELINE_ALIGNMENT IOTracker {
 ...
 public:
     ...
     * @param mdsclient transparently transmits to the splitter for
     * communication with mds
     * @param fileInfo Basic information of the file corresponding to the
     * current io
     */
    ...
    /**
     * The chunk-related interfaces are intended for use by snapshots. The
     * upper-level snapshot and file interfaces are separate. However, in the
     * IOTracker, they are unified so that the lower levels do not need to be
     * aware of the upper-level interface category.
    ...
    /**
     * Wait is used for synchronous interface waiting. When the user's IO is
     * taken over by client internal threads, the call can return to the upper
     * layer. However, the user's synchronous IO semantics require waiting for
     * the result to return before returning to the upper layer, so Wait here
     * will make the user thread wait.
     * @return: Returns read/write information. For asynchronous IO, it returns
     * 0 or -1. 0 means success, -1 means failure. For synchronous IO, it
     * returns the length or -1. 'length' represents the actual read/write
     * length, and -1 represents read/write failure.
     */
    int Wait();

    /**
     * Each request must have its own OP type, and an interface is provided here
     * to obtain the type during IO splitting
     */
    ...
    /**
     * Because client IOs are all sent asynchronously, and a single IO is split
     * into multiple Requests, after asynchronous IO returns, it should inform
     * the IOTracker that the current request has returned. This way, the
     * tracker can handle the returned request.
 ...
 private:
    ...
    /**
     * When IO splitting or IO distribution fails, it needs to be called, set
     * the return status, and return upwards
     */
    void ReturnOnFail();
    /**
     * The user's incoming large IO will be split into multiple sub IOs, and the
     * sub IO resources will be reclaimed before returning here
     */
 ...
 private:
    ...
    // The current IO data content, where data is the buffer for reading and
    // writing data
    off_t offset_;
    uint64_t length_;
    ...
    // When a user sends synchronous IO, they need to wait in the upper layer
    // because the client's IO sending process is all asynchronous, so here we
    // need to use a conditional variable to wait for asynchronous IO to return
    // Afterwards, the waiting condition variable is awakened and then returned
    // upwards.
    IOConditionVariable iocv_;

    // The context of asynchronous IO is called aioctx when asynchronous IO
    // returns Asynchronous callback for return.
    ...
    // The large IO is split into multiple requests, which are stored in the
    // reqlist in China
    ...
    // The scheduler is used to separate user threads from the client's own
    // threads After the large IO is split, the split reqlist is passed to the
    // scheduler and sent downwards
    RequestScheduler* scheduler_;

    // For asynchronous IO, the Tracker needs to notify the upper level that the
    // current IO processing has ended The iomanager can release the tracker
