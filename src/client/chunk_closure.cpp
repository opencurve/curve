...
void ClientClosure::PreProcessBeforeRetry(int rpcstatus, int cntlstatus) {
    ...
    // There will be a delay of 1-2 seconds when migrating from copysetleader to
    // client GetLeader to obtain a new leader For a request, GetLeader may
    // still return the old Leader The rpc timeout time may be set to 2s/4s, and
    // the leader information will be obtained after the timeout To retry the
    // request on the new Leader as soon as possible, set the rpc timeout time
...
// Perform corresponding processing for different request types and return
// status codes Each subclass needs to implement SendRetryRequest for retry
// requests
