...
class IOConditionVariable {
 public:
    ...
    /**
     *Conditional variable wake-up function, because the underlying RPC request is asynchronous, users need to issue synchronous IO
     *Pause waiting for IO return while sending read/write requests.
    ...
    /**
     *It is a function called when user IO needs to wait, and this function will return when Complete is called
     */
    int Wait() {
        std::unique_lock<std::mutex> lk(mtx_);
