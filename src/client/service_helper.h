...
// The copyset information and rpc related parameter information corresponding
// to the GetLeader request
struct GetLeaderInfo {
    ...
    GetLeaderInfo(const LogicPoolID& logicPoolId, const CopysetID& copysetId,
                  const std::vector<CopysetPeerInfo<ChunkServerID>>&
                      copysetPeerInfo,  // NOLINT
                  int16_t currentLeaderIndex,
...
class ServiceHelper {
 public:
     ...
     * @param[out]: leaderAddr is the output parameter that returns the leader
     * information of the current copyset
     * @param[out]: leaderId is the output parameter, returning the ID
     ...
     * The user information needs to be included in the file name, such as the
     * file name being temp and the username being user, So the complete file
     * information is: temp_user_. If the file name is: /temp_temp_, So the
     * complete file name is /temp_temp__user_.
