...
class IOManager4File : public IOManager {
 public:
     ...
     * @param: mdsclient transparently transmits to the underlying layer and
     * communicates with mds when necessary
     * @return: Successfully returned reading the true length, -1 indicates
     * failure
     */
    ...
    /**
     * When the lease excutor detects a version update, it needs to notify the
     * iomanager to update the file version information
 ...
 private:
    ...
    /**
     * lease related interface, when LeaseExecutor contract renewal fails, calls
     * LeaseTimeoutDisableIO Failed to return all newly issued IOs
     */
    void LeaseTimeoutBlockIO();

    /**
     * When the lease is successfully renewed, the LeaseExecutor calls the
     * interface to restore IO
     */
    void ResumeIO();

    /**
     * When the lesaeexcutor detects a version change, it calls the interface
     * and waits for inflight to return. During this period, IO is hanging
     */
    void BlockIO();
 ...
 private:
    ...
    // The task thread pool is used to isolate the QEMU thread from the curve
    // thread
    ...
    // When QEMU calls close, it closes the iomanager and its corresponding
    // resources. The lease renewal thread notifies the iomanager's scheduler
    // thread when renewal succeeds or fails, indicating whether it needs to
    // block or resume IO. Therefore, if the lease renewal thread needs to
    // notify the iomanager at this point, and if the iomanager's scheduler
    // resources have already been released, it may lead to a crash. So, it's
    // necessary to add a lock to protect this resource, ensuring that there is
    // no concurrency when exiting. This ensures that the lease renewal thread
    // won't use these resources when they are being destructed.
    std::mutex exitMtx_;
