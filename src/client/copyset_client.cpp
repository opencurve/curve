...
bool CopysetClient::FetchLeader(LogicPoolID lpid, CopysetID cpid,
    ...
    // 2. If the pull of leader information in the metacache fails, send an RPC
    // request to obtain new leader information
...
// Both of these situations will call the interface, as retrying RPCs may
// require re pushing to the queue If non retrying RPC is pushed back into the
// queue, it will cause a deadlock.
int CopysetClient::ReadChunk(const ChunkIDInfo& idinfo, uint64_t sn,
    ...
    // 1. During the normal retry process, if the file is not in a closed state,
    // RPC will directly re push to the scheduler queue header
    //    The retry call is in the brpc thread, so there will be no blocking of
    //    the retry RPC here Will not block the brpc thread as it is common to
    //    all files. Avoid affecting other files Because the session renewal
    //    failure may only be a network issue, IO is actually still possible
    //    after the renewal is successful Normal distribution, so failure cannot
    //    be directly returned upwards. Hang on at the bottom and continue
    //    sending after the renewal is successful
    // 2. exitFlag_=true during file closing, retrying rpc will directly return
    // to the user through closure
    //    After the return call, doneguard will call the run of the closure,
    //    releasing the inflight rpc count, Then the closure is returned to the
    //    user upwards.
    ...
    auto task = [&](Closure* done, std::shared_ptr<RequestSender> senderPtr) {
        ...
        senderPtr->ReadChunk(idinfo, sn, offset, length, sourceInfo, readDone);
    };
...
int CopysetClient::DeleteChunkSnapshotOrCorrectSn(const ChunkIDInfo& idinfo,
    ...
    auto task = [&](Closure* done, std::shared_ptr<RequestSender> senderPtr) {
        DeleteChunkSnapClosure* deleteDone =
            new DeleteChunkSnapClosure(this, done);
        senderPtr->DeleteChunkSnapshotOrCorrectSn(idinfo, correctedSn,
                                                  deleteDone);
    };
