...
// MDSClientBase abstracts all RPC interfaces with the MDS, decoupling them from
// business logic. Here, it is responsible only for sending RPC requests, while
// the specific business logic processing is returned to the caller through
// responses and controllers, which are handled by the caller.
class MDSClientBase {
 public:
    /**
     * Open File
     ...
     * @param[out]: response is the response of the rpc, provided for external
     * processing
     * @param[in|out]: cntl is both an input and output parameter, returning RPC
     * status
     * @param[in]: channel is the current channel established with MDS
     */
    void OpenFile(const std::string& filename, const UserInfo_t& userinfo,
                  OpenFileResponse* response, brpc::Controller* cntl,
     ...
     * @param: normalFile indicates whether the created file is a regular file
     * or a directory file. If it is a directory, size is ignored
     * @param[out]: response is the response of the rpc, provided for external
     * processing
     * @param[in|out]: cntl is both an input and output parameter, returning RPC
     * status
    ...
    void CreateFile(const CreateFileContext& context,
                    CreateFileResponse* response, brpc::Controller* cntl,
                    brpc::Channel* channel);
    /**
     * To close the file, it is necessary to carry the sessionid, so that the
     ...
     * @param[in]: channel is the current channel established with MDS
     */
    void CloseFile(const std::string& filename, const UserInfo_t& userinfo,
                   const std::string& sessionid, CloseFileResponse* response,
     ...
     * @param[in]: channel is the current channel established with MDS
     */
    void GetFileInfo(const std::string& filename, const UserInfo_t& userinfo,
                     GetFileInfoResponse* response, brpc::Controller* cntl,
                     brpc::Channel* channel);
     ...
     *
     */
    void IncreaseEpoch(const std::string& filename, const UserInfo_t& userinfo,
     ...
     * @param[in]: channel is the current channel established with MDS
     */
    void CreateSnapShot(const std::string& filename, const UserInfo_t& userinfo,
     ...
     * @param[in]: channel is the current channel established with MDS
     */
    void DeleteSnapShot(const std::string& filename, const UserInfo_t& userinfo,
                        uint64_t seq, DeleteSnapShotResponse* response,
                        brpc::Controller* cntl, brpc::Channel* channel);
    /**
     ...
     * @param[in]: channel is the current channel established with MDS
     */
    void ListSnapShot(const std::string& filename, const UserInfo_t& userinfo,
     ...
     * @param[in]: channel is the current channel established with MDS
     */
    void RefreshSession(const std::string& filename, const UserInfo_t& userinfo,
    ...
    void ListPoolset(ListPoolsetResponse* response, brpc::Controller* cntl,
                     brpc::Channel* channel);
     ...
     * @param[in]:channel  rpc channel
     */
    void GetOrAllocateSegment(bool allocate, uint64_t offset, const FInfo_t* fi,
     ...
     * @param[in]: channel is the current channel established with MDS
     */
    void RenameFile(const UserInfo_t& userinfo, const std::string& origin,
                    const std::string& destination, uint64_t originId,
                    uint64_t destinationId, RenameFileResponse* response,
                    brpc::Controller* cntl, brpc::Channel* channel);
    /**
     * Extension file
     * @param: userinfo is the user information
     ...
     * @param[in]: channel is the current channel established with MDS
     */
    void Extend(const std::string& filename, const UserInfo_t& userinfo,
                uint64_t newsize, ExtendFileResponse* response,
                brpc::Controller* cntl, brpc::Channel* channel);
    /**
     * Delete files
     * @param: userinfo is the user information
     * @param: filename The file name to be deleted
     * @param: Does deleteforce force deletion without placing it in the garbage
     * bin
     ...
     * @param[in]: channel is the current channel established with MDS
     */
    void DeleteFile(const std::string& filename, const UserInfo_t& userinfo,
                    bool deleteforce, uint64_t fileid,
                    DeleteFileResponse* response, brpc::Controller* cntl,
                    brpc::Channel* channel);
     ...
     * @param[in]:channel
     */
    void RecoverFile(const std::string& filename, const UserInfo_t& userinfo,
                     uint64_t fileid, RecoverFileResponse* response,
                     brpc::Controller* cntl, brpc::Channel* channel);
     ...
     * @param[in]: channel is the current channel established with MDS
     */
    void ChangeOwner(const std::string& filename, const std::string& newOwner,
                     const UserInfo_t& userinfo, ChangeOwnerResponse* response,
                     brpc::Controller* cntl, brpc::Channel* channel);
    /**
     ...
     * @param[in]: channel is the current channel established with MDS
     */
    void Listdir(const std::string& dirpath, const UserInfo_t& userinfo,
                 ListDirResponse* response, brpc::Controller* cntl,
                 brpc::Channel* channel);
    /**
     ...
     * @param[in]: channel is the current channel established with MDS
     */
    void GetChunkServerInfo(const std::string& ip, uint16_t port,
     ...
     * @param[out]: response is the response of the current rpc call, returned
