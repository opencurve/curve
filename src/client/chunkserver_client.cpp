...
int ChunkServerClient::UpdateFileEpoch(const CopysetPeerInfo<ChunkServerID> &cs,
    ...
    google::protobuf::Closure *rpcDone =
        brpc::NewCallback(OnUpdateFileEpochReturned, ctx.get());

    ChunkService_Stub stub(&ctx->channel);
    stub.UpdateEpoch(&ctx->cntl, &ctx->request, &ctx->response, rpcDone);
    doneGuard.release();
    ctx.release();
    return LIBCURVE_ERROR::OK;
}

void ChunkServerClient::OnUpdateFileEpochReturned(UpdateEpochContext *ctx) {
    ...
    if (ctx->cntl.Failed()) {
        if (ctx->cntl.ErrorCode() == brpc::ELOGOFF ||
            ...
            ctx->response.Clear();
            google::protobuf::Closure *rpcDone =
                brpc::NewCallback(OnUpdateFileEpochReturned, ctx_guard.get());

            ChunkService_Stub stub(&ctx->channel);
            stub.UpdateEpoch(&ctx->cntl, &ctx->request, &ctx->response,
                             rpcDone);
            doneGuard.release();
            ctx_guard.release();
        } else {
            LOG(WARNING) << "UpdateEpoch failed, cntl.errotText: "
                         << ctx->cntl.ErrorText();
            ctx->done->SetErrCode(-::FAILED);
        }
    } else {
        switch (ctx->response.status()) {
        case CHUNK_OP_STATUS::CHUNK_OP_STATUS_SUCCESS:
            LOG(INFO) << "Received UpdateEpoch Response [log_id="
                      << ctx->cntl.log_id() << "] from "
                      << ctx->cntl.remote_side() << " to "
                      << ctx->cntl.local_side()
                      << ", Message: " << ctx->response.DebugString();
            ctx->done->SetErrCode(::OK);
            break;
        case CHUNK_OP_STATUS::CHUNK_OP_STATUS_EPOCH_TOO_OLD:
            LOG(WARNING) << "Received UpdateEpoch Response [log_id="
                         ...
                         << ctx->cntl.local_side()
                         << ", Message: " << ctx->response.DebugString();
            ctx->done->SetErrCode(-::EPOCH_TOO_OLD);
            break;
        default:
            LOG(ERROR) << "can't reach here!";
            ctx->done->SetErrCode(-::UNKNOWN);
            break;
        }
    }
}
