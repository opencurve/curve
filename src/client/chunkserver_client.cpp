...
int ChunkServerClient::UpdateFileEpoch(
    ...
    std::unique_ptr<UpdateEpochRequest> request(new UpdateEpochRequest);
    std::unique_ptr<UpdateEpochResponse> response(new UpdateEpochResponse);

    int ret = 0;
    if (!cs.externalAddr.IsEmpty()) {
        ret = channel->Init(cs.externalAddr.addr_, NULL);
        if (ret != 0) {
        ...
        ret = channel->Init(cs.internalAddr.addr_, NULL);
        if (ret != 0) {
            LOG(ERROR) << "failed to init channel to chunkserver, "
                       << "id: " << cs.chunkserverID
    ...
    ctx->cntl = cntl.get();
    ctx->request = request.get();
    ctx->response = response.get();
    ctx->done = done;
    ctx->curTry = 1;  // 1 for current try
    ctx->retryOps_ = retryOps_;

    google::protobuf::Closure* rpcDone = brpc::NewCallback(
                OnUpdateFileEpochReturned, ctx.release());

    ChunkService_Stub stub(channel.get());
    stub.UpdateEpoch(cntl.release(), request.release(),
                     response.release(), rpcDone);
    channel.release();
    doneGuard.release();
...
void ChunkServerClient::OnUpdateFileEpochReturned(
    ...
    std::unique_ptr<brpc::Channel> channel_guard(ctx->channel);
    std::unique_ptr<brpc::Controller> cntl_guard(ctx->cntl);
    std::unique_ptr<UpdateEpochRequest>  req_guard(ctx->request);
    std::unique_ptr<UpdateEpochResponse> res_guard(ctx->response);
    if (ctx->cntl->Failed()) {
        if (ctx->cntl->ErrorCode() == brpc::ELOGOFF ||
            ...
            ctx->response->Clear();
            google::protobuf::Closure* rpcDone = brpc::NewCallback(
                        OnUpdateFileEpochReturned, ctx_guard.release());

            ChunkService_Stub stub(ctx->channel);
            stub.UpdateEpoch(cntl_guard.release(), req_guard.release(),
                             res_guard.release(), rpcDone);
