...
class GetLeaderProxy : public std::enable_shared_from_this<GetLeaderProxy> {
    ...
    void HandleResponse(brpc::CallId callId, bool success,
        ...
        if (success) {
            for (auto id : callIds_) {
                ...
                // After canceling, subsequent rpc request callbacks will still
                // be executed, but will be marked as failed
                brpc::StartCancel(id);
            }
            ...
            // If it is empty, it indicates that it is the last rpc returned,
            // and the request needs to be marked as failed and returned upwards
            if (callIds_.empty()) {
                std::lock_guard<bthread::Mutex> ulk(finishMtx_);
                finish_ = true;
                success_ = false;
 ...
 private:
    ...
    // Protect callIds_/success_, Avoiding asynchronous rpc callbacks from
...
int ServiceHelper::GetLeader(const GetLeaderInfo& getLeaderInfo,
                             PeerAddr* leaderAddr, ChunkServerID* leaderId,
                             FileMetric* fileMetric) {
...
int ServiceHelper::CheckChunkServerHealth(const butil::EndPoint& endPoint,
