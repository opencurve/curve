...
class RPCExcutorRetryPolicy {
 ...
 private:
    /**
     * When an RPC fails, it needs to be retried, and based on different statuses returned by `cntl`,
     * determine what kind of preprocessing should be done. The main tasks performed are as follows:
     * 1. If the last RPC timed out, execute RPC timeout exponential backoff logic.
     * 2. If the last RPC returned values like "not connect," it will actively trigger MDS address switching and retry.
     ...
     * @param[in][out]: curMDSRetryCount is the number of retries on the current MDS node. If MDS switching occurs,
     *                  this value will be reset to 1.
     * @param[in]: curRetryMDSIndex represents the current MDS index being retried.
     * @param[out]: lastWorkingMDSIndex is the index of the MDS that was providing service in the last attempt.
     * @param[out]: timeOutMS is adjusted based on the status to control the RPC timeout.
     *
    ...
    /**
     * Get the next MDS index to retry based on the input state. MDS switching logic:
     * Record three states: curRetryMDSIndex, lastWorkingMDSIndex, currentWorkingMDSIndex.
     * 1. At the beginning, curRetryMDSIndex = currentWorkingMDSIndex,
     *    lastWorkingMDSIndex = currentWorkingMDSIndex.
     * 2. If an RPC fails, it will trigger a switch of curRetryMDSIndex. If at this point,
     *    lastWorkingMDSIndex is equal to currentWorkingMDSIndex, it will sequentially switch
     *    to the next MDS index. If lastWorkingMDSIndex is not equal to currentWorkingMDSIndex,
     *    it means that another interface has updated currentWorkingMDSAddrIndex_. In this case,
     *    the switch will directly go to currentWorkingMDSAddrIndex_.
     * @param[in]: needChangeMDS indicates whether the current peripheral needs to switch MDS.
     *              This value is determined by the PreProcessBeforeRetry function.
     * @param[in]: currentRetryIndex is the current MDS index being retried.
     * @param[in][out]: lastWorkingIndex is the index of the last MDS being served in the last retry.
     *    The MDS being retried and the MDS being served may be different.
...
class MDSClient : public MDSClientBase,
 ...
 public:
     ...
     * @param[out]: The errcode of the output parameter is the error code on the side of libcurve
     */
