...
class MDSClient : public std::enable_shared_from_this<MDSClient> {
 public:
    ...
    LIBCURVE_ERROR Initialize(const MetaServerOption &metaopt,
        std::shared_ptr<AuthClient> authClient = nullptr);
 ...
 private:
     ...
     */
    template <typename T>
    static void FillUserInfo(T* request, const UserInfo_t& userinfo) {
    ...
    static bool IsRootUserAndHasPassword(const UserInfo& userinfo) {
        return userinfo.owner == kRootUserName && !userinfo.password.empty();
    }

    static std::string CalcSignature(const UserInfo& userinfo, uint64_t date);
