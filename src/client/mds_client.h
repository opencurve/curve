...
class RPCExcutorRetryPolicy {
 ...
 private:
    /**
     * When an RPC fails, it needs to be retried, and based on different
     * statuses returned by `cntl`, determine what kind of preprocessing should
     * be done. The main tasks performed are as follows:
     * 1. If the last RPC timed out, execute RPC timeout exponential backoff
     * logic.
     * 2. If the last RPC returned values like "not connect," it will actively
     * trigger MDS address switching and retry.
     * 3. Update retry information, such as the number of consecutive retries on
     * the current MDS.
     * @param[in]: status is the status of the current RPC failure.
     * @param[in]: normalRetryCount is the total count of normal retries.
     * @param[in][out]: curMDSRetryCount is the number of retries on the current
     * MDS node. If MDS switching occurs, this value will be reset to 1.
     * @param[in]: curRetryMDSIndex represents the current MDS index being
     * retried.
     * @param[out]: lastWorkingMDSIndex is the index of the MDS that was
     * providing service in the last attempt.
     * @param[out]: timeOutMS is adjusted based on the status to control the RPC
     * timeout.
     *
     * @return: Returns the next MDS index for the next retry.
     */
     ...
     * @return: If the channel is successfully obtained, 0 will be returned.
     * Otherwise, -1
     */
    int ExcuteTask(int mdsindex, uint64_t rpcTimeOutMS,
                   RPCExcutorRetryPolicy::RPCFunc task);
    /**
     * Get the next MDS index to retry based on the input state. MDS switching
     * logic: Record three states: curRetryMDSIndex, lastWorkingMDSIndex,
     ...
     * 2. If an RPC fails, it will trigger a switch of curRetryMDSIndex. If at
     * this point, lastWorkingMDSIndex is equal to currentWorkingMDSIndex, it
     * will sequentially switch to the next MDS index. If lastWorkingMDSIndex is
     * not equal to currentWorkingMDSIndex, it means that another interface has
     * updated currentWorkingMDSAddrIndex_. In this case, the switch will
     * directly go to currentWorkingMDSAddrIndex_.
     * @param[in]: needChangeMDS indicates whether the current peripheral needs
     * to switch MDS. This value is determined by the PreProcessBeforeRetry
     * function.
     * @param[in]: currentRetryIndex is the current MDS index being retried.
     * @param[in][out]: lastWorkingIndex is the index of the last MDS being
     * served in the last retry. The MDS being retried and the MDS being served
     * may be different.
     * @return: Returns the next MDS index to retry.
     */
    int GetNextMDSIndex(bool needChangeMDS, int currentRetryIndex,
                        int* lastWorkingindex);
    /**
     * Based on the input parameters, decide whether to continue retry. The
     * condition for retry exit is that the retry time exceeds the maximum
     * allowed time The retry time on IO paths is different from that on non IO
     * paths, and the retry time on non IO paths is determined by the
     * configuration file The mdsMaxRetryMS parameter specifies that the IO path
     * is an infinite loop retry.
     * @param[in]: startTimeMS
     * @param[in]: maxRetryTimeMS is the maximum retry time
     * @return: Need to continue retrying and return true, otherwise return
     * false
     */
...
class MDSClient : public MDSClientBase,
 ...
 public:
    ...
    /**
     * Obtain the serverlist information corresponding to the copysetid and
     * update it to the metacache
     ...
     * @return: Successfully returned LIBCURVE_ERROR::OK, otherwise will be
     * returned LIBCURVE_ERROR::FAILED
     */
    LIBCURVE_ERROR
     ...
     * @param: deleteforce  Does it force deletion without placing it in the
     * garbage bin
     * @param: id is the file id, with a default value of 0. If the user does
     * not specify this value, the id will not be passed to mds
     */
     ...
     * @param: seq is an output parameter that returns the version information
     * of the file when creating the snapshot
     * @return:
     * Successfully returned LIBCURVE_ERROR::OK, if authentication fails, return
     * LIBCURVE_ERROR::AUTHFAIL, Otherwise, return LIBCURVE_ERROR::FAILED
     */
     ...
     * @param: seq is the version information of the file when creating the
     * snapshot
     * @return:
     * Successfully returned LIBCURVE_ERROR::OK, if authentication fails, return
     * LIBCURVE_ERROR::AUTHFAIL, Otherwise, return LIBCURVE_ERROR::FAILED
    ...
    /**
     * Obtain snapshot file information with version number seq in the form of a
     * list, where snapif is the output parameter
     * @param: filename is the file name to be snapshot
     * @param: userinfo is the user information
     * @param: seq is the version information of the file when creating the
     * snapshot
     * @param: snapif is a parameter that saves the basic information of the
     * file
     * @return:
     * Successfully returned LIBCURVE_ERROR::OK, if authentication fails, return
     * LIBCURVE_ERROR::AUTHFAIL, Otherwise, return LIBCURVE_ERROR::FAILED
    ...
    /**
     * Obtain the chunk information of the snapshot and update it to the
     * metacache, where segInfo is the output parameter
     * @param: filename is the file name to be snapshot
     * @param: userinfo is the user information
     * @param: seq is the version information of the file when creating the
     * snapshot
    ...
    /**
     * The file interface needs to maintain a heartbeat with MDS when opening
     * files, and refresh is used to renew the contract The renewal result will
     * be returned to the calling layer through LeaseRefreshResult* resp
    ...
    /**
     * To close the file, it is necessary to carry the session ID, so that the
     * mds side will delete the session information in the database
     * @param: filename is the file name to be renewed
     * @param: sessionid is the session information of the file
     * @return:
     * Successfully returned LIBCURVE_ERROR::OK, if authentication fails, return
    ...
    LIBCURVE_ERROR CreateCloneFile(const std::string& source,
                                   ...
                                   const std::string& poolset, FInfo* fileinfo);
     ...
     * @return: Successfully returned LIBCURVE_ERROR::OK, failure returns
     ...
     * @param[out]: The errcode of the output parameter is the error code on the
     * side of libcurve
     */
