...
// rpc发送和mds地址切换状态机
int RPCExecutorRetryPolicy::DoRPCTask(RPCFunc rpctask,
...
bool RPCExecutorRetryPolicy::GoOnRetry(uint64_t startTimeMS,
                                      uint64_t maxRetryTimeMS) {
    if (maxRetryTimeMS == 0) {
        return true;
    }
...
int RPCExecutorRetryPolicy::PreProcessBeforeRetry(int status, bool retryUnlimit,
 ...
 */
int RPCExecutorRetryPolicy::GetNextMDSIndex(bool needChangeMDS,
...
int RPCExecutorRetryPolicy::ExcuteTask(int mdsindex, uint64_t rpcTimeOutMS,
                                      RPCFunc task) {
    assert(mdsindex >= 0 &&
