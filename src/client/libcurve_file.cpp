...
int FileClient::Open(const std::string& filename,
    ...
    bool isSnapshot = curve::client::ServiceHelper::GetSnapSeqFromFilename(
        filename, sn, &realfilename);
    if (isSnapshot) {
        LOG(INFO) << "Opening Snapshot sn: "
            << sn << ", realfilename: " << realfilename;
        fileserv = FileInstance::Open4Readonly(
            clientConfig.GetFileServiceOption(), mdsClient,
            realfilename, userinfo, openflags);
        if (fileserv == nullptr) {
            LOG(ERROR) << "Open4Readonly snapshot file failed, filename = "
                << filename;
            return -1;
        }
...
int FileClient::Open4ReadOnly(const std::string& filename,
    ...
    bool isSnapshot = curve::client::ServiceHelper::GetSnapSeqFromFilename(
        filename, sn, &realfilename);
    if (isSnapshot) {
        LOG(INFO) << "Open4ReadOnly Snapshot not allowed, sn: " << sn <<
...
int FileClient::IncreaseEpoch(const std::string& filename,
    ...
    bool isSnapshot = curve::client::ServiceHelper::GetSnapSeqFromFilename(
        filename, sn, &realfilename);
    if (isSnapshot) {
        LOG(INFO) << "IncreaseEpoch Snapshot not allowed, sn: " << sn
            << ", realfilename: " << realfilename;
        return -LIBCURVE_ERROR::NOT_SUPPORT;
    }
...
int FileClient::Extend(const std::string& filename,
    ...
    bool isSnapshot = curve::client::ServiceHelper::GetSnapSeqFromFilename(
        filename, sn, &realfilename);
    if (isSnapshot) {
        LOG(INFO) << "Extend Snapshot not allowed, sn: " << sn <<
            ", realfilename: " << realfilename;
        return -LIBCURVE_ERROR::NOT_SUPPORT;
    }
    if (mdsClient_ != nullptr) {
...
int FileClient::Unlink(const std::string& filename,
    ...
    bool isSnapshot = curve::client::ServiceHelper::GetSnapSeqFromFilename(
        filename, sn, &realfilename);
    if (isSnapshot) {
        LOG(INFO) << "Unlink Snapshot not allowed, sn: " << sn <<
            ", realfilename: " << realfilename;
        return -LIBCURVE_ERROR::NOT_SUPPORT;
    }
    if (mdsClient_ != nullptr) {
...
int FileClient::GetFileInfo(const std::string& filename,
    ...
    bool isSnapshot = curve::client::ServiceHelper::GetSnapSeqFromFilename(
        filename, sn, &realfilename);
    if (isSnapshot) {
        LOG(INFO) << "StatFile Snapshot sn: " << sn
            << ", realfilename: " << realfilename;
    }
    if (mdsClient_ != nullptr) {
