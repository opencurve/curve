...
/**
 *Responsible for managing ChunkServer links and providing access to the upper level
 *Specify the read/write interfaces for the chunk of the copyset
 */
class CopysetClient {
 public:
    CopysetClient()
    ...
    * @param: correctedSn used to modify the chunk when creating CloneChunk
    ...
    */
    ...
    /**
     *Receive upper level shutdown notification when the file is closed, based on the valid status of the session
     *Set exitFlag, if there is an RPC timeout return in the invalid state of the session
     *Some RPCs will directly return errors. If the session is normal, it will continue to be issued normally
     *RPC, until the retry count ends or successfully returns
     */
    void ResetExitFlag() {
        if (sessionNotValid_) {
            exitFlag_ = true;
 ...
 private:
    ...
    //Is the session valid? If the session is invalid, it is necessary to stop the retry RPC
    //RPC stops by re pushing this rpc to the request scheduler queue, so that it does not
    //Block threads inside brpc to prevent operations on one file from affecting other files
