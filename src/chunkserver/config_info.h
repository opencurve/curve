...
struct CopysetNodeOptions {
    ...
    //The time interval for taking regular snapshots is 3600s by default, which is 1 hour
    ...
    //If the difference between the follower and leader logs exceeds catchupMargin,
    //Will execute install snapshot for recovery, default: 1000
    int catchupMargin;
    ...
    //When the recycle bin and heartbeat module determine that the chunkserver is not in the copyset configuration group,
    //Notify the copysetManager to move the copyset directory to the recycle bin
    ...
    //Limit the number of copyset concurrent recovery loads during chunkserver startup, with a value of 0 indicating no limit
    ...
    //Possible exceptions: 1. Currently, most replicas have not yet been restored; 2. Network issues and other issues preventing the acquisition of leaders
    //3. Due to other reasons, it is not possible to obtain the committed index of the leader
    uint32_t checkRetryTimes = 3;
    //the difference bewteen the current peer's applied_index and leader's committed_index is less than this value
