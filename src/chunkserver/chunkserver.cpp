...
void ChunkServer::InitCopysetNodeOptions() {
    ...
    copysetNodeOptions_.snapshotIntervalS = conf_.GetIntValue("copyset.snapshot_interval");
    copysetNodeOptions_.catchupMargin = conf_.GetIntValue("copyset.catchup_margin");
    copysetNodeOptions_.chunkDataUri = conf_.GetStringValue("copyset.chunk_data_uri");
    copysetNodeOptions_.chunkSnapshotUri = conf_.GetStringValue("copyset.chunk_data_uri");
    copysetNodeOptions_.logUri = conf_.GetStringValue("copyset.raft_log_uri");
    copysetNodeOptions_.raftMetaUri = conf_.GetStringValue("copyset.raft_meta_uri");
    copysetNodeOptions_.raftSnapshotUri = conf_.GetStringValue("copyset.raft_snapshot_uri");
...
int ChunkServer::Init(int argc, char** argv) {
    ...
    LOG_IF(FATAL, !conf_.LoadConfig()) << "Failed to open config file: " << conf_.GetConfigPath();
    ...
    if (createTestCopyset) {
        ...
        LOG(INFO) << "Created test copyset: <" << poolId << "," << copysetId << ">";
    }
...
int ChunkServer::Fini() {
    ...
    if (createTestCopyset) {
        ...
        LOG(INFO) << "Test copyset removed: <" << poolId << "," << copysetId << ">";
    }
...
int ChunkServer::ReplaceConfigWithCmdFlags() {
    // TODO(wenyu): replace config items which provided in commandline flags, so commandline flags have higher priority
