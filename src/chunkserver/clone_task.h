...
class CloneTask : public Uncopyable,
 ...
 public:
    CloneTask(std::shared_ptr<ReadChunkRequest> request,
        ...
        : core_(core), readRequest_(request), done_(done), isComplete_(false) {}

    virtual ~CloneTask() {}

    virtual std::function<void()> Closure() {
        auto sharedThis = shared_from_this();
        return [sharedThis]() { sharedThis->Run(); };
    }
    ...
    virtual bool IsComplete() { return isComplete_; }

 protected:
    // Clone Core Logic
    std::shared_ptr<CloneCore> core_;
    // Information related to this task
    std::shared_ptr<ReadChunkRequest> readRequest_;
    // Closure to be executed after the task is completed
