...
class HeartbeatHelper {
 public:
    /**
     * Build a new configuration for the specified replication group based on
     * the conf issued by mds, and use it for ChangePeer
     *
     * @param[in] conf mds issued the change command needupdatecopyset[i]
     * @param[out] newPeers specifies the target configuration for the
     * replication group
     *
     * @return false - Failed to generate newpeers, true - Successfully
     * generated newpeers
     */
    static bool BuildNewPeers(const CopySetConf& conf,
    ...
    /**
     * Determine whether the copysetConf sent by mds is legal, and the following
     * two situations are illegal:
     * 1. The copyset does not exist in chunkserver
     * 2. The epoch recorded in the copyset issued by mds is smaller than the
     * epoch recorded in the copyset on chunkserver at this time
     *
     * @param[in] conf mds issued the change command needupdatecopyset[i]
     ...
     */
    static bool CopySetConfValid(const CopySetConf& conf,
                                 const CopysetNodePtr& copyset);

    /**
     * Determine whether the specified copyset in chunkserver(csEp) needs to be
     * deleted
     *
    ...
    /**
     * Determine whether the specified chunkserver copyset has been loaded
     * completely
     *
     * @return false-copyset loading completed, true-copyset not loaded
     * completed
     */
