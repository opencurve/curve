...
// The closure of the chunk service layer encapsulates the closure of the rpc
// layer, which is used to count metric information when requesting returns
class ChunkServiceClosure : public braft::Closure {
 public:
    explicit ChunkServiceClosure(
        ...
        // Increment by 1 when the closure is created, and decrement by 1 when
    ...
    /**
     * This function will be called at the end of the guard lifecycle of the
     * closure Currently, this function mainly performs some metric statistics
     * on the returned results of read and write requests If there are similar
     * scenarios in the future (doing some processing at the end of the service
     * request), logic can be added internally
     */
    void Run() override;
 ...
 private:
    ...
    void OnRequest();
    /**
     * Record the results of request processing, such as whether the request was
     * incorrect, the delay of the request, etc
     */
    void OnResonse();
