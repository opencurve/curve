...
int CopysetNodeManager::Run() {
    ...
    if (copysetLoader_ != nullptr) {
        ret = copysetLoader_->Start(copysetNodeOptions_.loadConcurrency);
        if (ret < 0) {
...
int CopysetNodeManager::ReloadCopysets() {
    ...
    for (; it != items.end(); ++it) {
        ...
        LOG(INFO) << "Parsed groupid " << groupId << " as "
        ...
        if (copysetLoader_ == nullptr) {
            ...
            copysetLoader_->Enqueue(std::bind(&CopysetNodeManager::LoadCopyset,
                                              this, poolId, copysetId, true));
        }
    }

    // If loading is successful, wait for all copysets to load and close the
    // thread pool
    if (copysetLoader_ != nullptr) {
        while (copysetLoader_->QueueSize() != 0) {
            ::sleep(1);
        }
        // Even when the queue size is 0, the threads in the thread pool may
        // still be executing. The 'stop' function internally performs thread
        // joining to ensure that all tasks are completed before exiting.
        copysetLoader_->Stop();
        copysetLoader_ = nullptr;
    }
...
void CopysetNodeManager::LoadCopyset(const LogicPoolID& logicPoolId,
    ...
    // chunkserver starts the loading copyset phase and will reject external
    // requests to create copysets Therefore, no other threads will load or
    // create the same copyset, and locking is not necessary at this time
    Configuration conf;
...
bool CopysetNodeManager::CheckCopysetUntilLoadFinished(
    ...
    while (retryTimes < copysetNodeOptions_.checkRetryTimes) {
        ...
        // Failure to obtain leader status is usually because a leader has not
        // been elected yet, or the leader's heartbeat has not been received by
        // the current node. Typically, leader information can be obtained
        // through several retries. If multiple retries fail to obtain the
        // information, it is assumed that the copyset may not be able to elect
        // a leader at the moment, and the operation exits directly.
        if (!getSuccess) {
            ++retryTimes;
        ...
        // When the last log of the current replica lags behind the first log
        // saved on the leader, in this situation, the replica will recover by
        // installing a snapshot, and it can be safely ignored to avoid blocking
        // the checking thread.
        ...
        // Determine whether the logs that have been applied to the current
        // replica are close to the logs that have been committed
        int64_t margin =
            leaderStaus.committed_index - status.known_applied_index;
        bool catchupLeader =
            margin < (int64_t)copysetNodeOptions_.finishLoadMargin;
        if (catchupLeader) {
    ...
    LOG(WARNING) << "check copyset " << ToGroupIdString(logicPoolId, copysetId)
                 << " failed.";
    return false;
}
...
std::shared_ptr<CopysetNode> CopysetNodeManager::GetCopysetNode(
    ...
    if (copysetNodeMap_.end() != it) return it->second;
...
bool CopysetNodeManager::CreateCopysetNode(const LogicPoolID& logicPoolId,
    ...
    GroupId groupId = ToGroupId(logicPoolId, copysetId);
    // If the local copyset has not been fully loaded yet, external copyset
    ...
    // When copysetnode is deconstructed, shutdown may be called, which may lead
    // to coprocessor disconnection So when creating a copysetnode fails, it
    // cannot occupy the write lock, wait for the write lock to be released
    // before destructing
...
int CopysetNodeManager::AddService(brpc::Server* server,
    ...
    do {
        ...
        ret = server->AddService(
            new ChunkServiceImpl(chunkServiceOptions, epochMap),
            brpc::SERVER_OWNS_SERVICE);
...
bool CopysetNodeManager::DeleteCopysetNode(const LogicPoolID& logicPoolId,
...

    {
        ...
        if (copysetNodeMap_.end() != it) {
            // TODO(yyk) There may be a risk of deadlock, which needs to be
...

    {
        ...
        if (copysetNodeMap_.end() != it) {
            ...
            LOG(INFO) << "Delete copyset "
                      << ToGroupIdString(logicPoolId, copysetId) << " success.";
        }
    }
