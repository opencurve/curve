...
int CopysetNodeManager::ReloadCopysets() {
    ...
    if (copysetLoader_ != nullptr) {
        ...
        // Even when the queue size is 0, the threads in the thread pool may still be executing.
        // The 'stop' function internally performs thread joining to ensure that all tasks are completed before exiting.
        copysetLoader_->Stop();
        copysetLoader_ = nullptr;
    }
...
bool CopysetNodeManager::CheckCopysetUntilLoadFinished(
    ...
    while (retryTimes < copysetNodeOptions_.checkRetryTimes) {
        ...
        // Failure to obtain leader status is usually because a leader has not been elected yet, or the leader's heartbeat has not been received by the current node.
        // Typically, leader information can be obtained through several retries.
        // If multiple retries fail to obtain the information, it is assumed that the copyset may not be able to elect a leader at the moment, and the operation exits directly.
        if (!getSuccess) {
            ++retryTimes;
        ...
        // When the last log of the current replica lags behind the first log saved on the leader,
        // in this situation, the replica will recover by installing a snapshot, and it can be safely ignored to avoid blocking the checking thread.
