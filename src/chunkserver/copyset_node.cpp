...
void CopysetNode::on_apply(::braft::Iterator& iter) {
    for (; iter.valid(); iter.next()) {
        ...
        if (nullptr != closure) {
             ...
             * log entry will be deserialized, Then obtain Op information for
             * application 2.2. follower apply
             */
...
void CopysetNode::save_snapshot_background(::braft::SnapshotWriter* writer,
    ...
    if (0 == fs_->List(chunkDataApath_, &files)) {
        for (const auto& fileName : files) {
            ...
            // list of snapshot files in the meta information When raft
            // downloads a snapshot, after downloading the chunk, a separate
            // snapshot list will be obtained
...
int CopysetNode::on_snapshot_load(::braft::SnapshotReader* reader) {
    ...
    if (fs_->DirExists(snapshotChunkDataDir)) {
        ...
        // the snapshot is loaded If delete_file or rename fails, the current
        // node status will be set to ERROR If delete_file or during the rename
        // the process restarts, and after copyset is set, the snapshot will be
        // loaded Since rename ensures atomicity, after loading the snapshot,
        // the data directory must be restored
        bool ret =
     ...
     * there is no need for on_configuration_committed. It should be noted that
     * the log of the joint stage will be ignored here.
     */
