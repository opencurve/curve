...
int CopysetNode::on_snapshot_load(::braft::SnapshotReader *reader) {
     ...
     *
     * (1) For instance, when adding a new peer and immediately reading data, the data store may return "chunk not exist."
     * This is because the newly added peer initially has no data, 
     * and when the data store is initialized, it is not aware of
     * the data that the new peer receives after the leader recovers its data.
     *
     * (2) When a peer recovers all of its data through an install snapshot operation, it is performed through a rename operation.
     * If a file was previously open in the data store, the rename operation can succeed, but the old file can only be deleted
     * after the data store closes it. Therefore, it is necessary to reinitialize the data store, close the file's file descriptor (fd),
     * and then reopen the new file. Otherwise, the data store will continue to operate on the old file. 
     * Once the data store closes, the corresponding fd, any subsequent write operations will be lost. 
     * Additionally, if the datastore is not reinitialized
     * and the new file is not reopened, it may result in reading the old data rather than the recovered data.
     */
...
void CopysetNode::UpdateAppliedIndex(uint64_t index) {
    ...
    if (index > curIndex) {
         ...
         * First, it compares whether curIndex is equal to appliedIndex. If it is equal, it means that no one has modified appliedindex.
         * In this case, it tries to update appliedIndex with the value of index, and if the update is successful, it's done.
         * If curIndex is not equal to appliedindex, it indicates that someone else has updated appliedIndex in the meantime. In this case, it returns the current
         * value of appliedindex through curIndex and returns false. This entire process is atomic.
         */
...
int CopysetNode::GetConfChange(ConfigChangeType *type,
     ...
     * a new leader is elected and a noop entry is committed, consider the following scenario:
     *
     * In a replication group with three members {ABC}, the current epoch is 5, and A is the leader. A receives a configuration change log that adds D, and assume that B also receives the configuration change
     * log {ABC+D}. Then, leader A crashes, and B is elected as the new leader.
     * Before B commits the noop entry, the maximum epoch value it can query on B is still 5,
     * but the queried configuration is {ABCD}. Therefore, here, before the new leader B commits the noop entry,
     * which is effectively committing the hidden configuration change log {ABC+D}, it does not allow returning the configuration and configuration change information to the user
     * to avoid epoch and configuration information inconsistency.
     */
