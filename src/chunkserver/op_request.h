...
inline bool existCloneInfo(const ChunkRequest* request) {
    if (request != nullptr) {
        if (request->has_clonefilesource() && request->has_clonefileoffset()) {
            return true;
        }
    }
    return false;
...
class ChunkOpRequest : public std::enable_shared_from_this<ChunkOpRequest> {
 public:
    ChunkOpRequest();
    ChunkOpRequest(std::shared_ptr<CopysetNode> nodePtr, RpcController* cntl,
    ...
    /**
     * Processing a request actually involves proposing to the corresponding
     * copyset
     */
     ...
     */
    virtual void OnApply(uint64_t index, ::google::protobuf::Closure* done) = 0;

    /**
     * NOTE: During subclass implementation, prioritize the use of
     * datastore/request passed in as parameters Obtain detailed request
     * information from the reverse sequence of the log entry for processing,
     * request The relevant context and dependent data store are passed in from
     * parameters
     * 1. Restart the replay log, read the op log entry from the disk, and then
     * execute the on apply logic
     * 2. follower execute the logic of on apply
     * @param datastore: chunk data persistence layer
     * @param request: The detailed request information obtained after
     * deserialization
     * @param data: The data to be processed by the request obtained after
     * deserialization
     */
 ...
 public:
     ...
     * op meta: refers to the metadata of op, where is the length of the op
     * request section op data: refers to the data serialized by the request
     * through protobuf
     * @param request: Chunk Request
     * @param data: The data content contained in the request
     * @param log: Output parameter to store serialized data. Users are
     * responsible for ensuring that data != nullptr.
     * @return 0 succeeded, -1 failed
     */
    static int Encode(const ChunkRequest* request, const butil::IOBuf* data,
                      butil::IOBuf* log);

    /**
     * Deserialize, obtain ChunkOpRequest from log entry, and deserialize the
     * current ChunkRequest and data Will be passed out from the output
     * parameter, rather than being placed in the member variable of
     * ChunkOpRequest
     ...
     * @return nullptr, failed, otherwise return the corresponding
     * ChunkOpRequest
     */
 ...
 protected:
     ...
     * @return 0 succeeded, -1 failed
     */
    int Propose(const ChunkRequest* request, const butil::IOBuf* data);
...
class DeleteChunkRequest : public ChunkOpRequest {
 public:
    DeleteChunkRequest() : ChunkOpRequest() {}
...
class ReadChunkRequest : public ChunkOpRequest {
 ...
 public:
    ReadChunkRequest() : ChunkOpRequest() {}
...
class WriteChunkRequest : public ChunkOpRequest {
 public:
    WriteChunkRequest() : ChunkOpRequest() {}
    WriteChunkRequest(std::shared_ptr<CopysetNode> nodePtr, RpcController* cntl,
                      const ChunkRequest* request, ChunkResponse* response,
                      ::google::protobuf::Closure* done)
        : ChunkOpRequest(nodePtr, cntl, request, response, done) {}
...
class DeleteSnapshotRequest : public ChunkOpRequest {
 public:
    DeleteSnapshotRequest() : ChunkOpRequest() {}
...
class CreateCloneChunkRequest : public ChunkOpRequest {
 public:
    CreateCloneChunkRequest() : ChunkOpRequest() {}
...
class PasteChunkInternalRequest : public ChunkOpRequest {
 public:
    PasteChunkInternalRequest() : ChunkOpRequest() {}
    PasteChunkInternalRequest(std::shared_ptr<CopysetNode> nodePtr,
                              const ChunkRequest* request,
                              ChunkResponse* response, const butil::IOBuf* data,
                              ::google::protobuf::Closure* done)
        : ChunkOpRequest(nodePtr, nullptr, request, response, done) {
        if (data != nullptr) {
            data_ = *data;
        }
    }
...
class ScanChunkRequest : public ChunkOpRequest {
 public:
    ...
    ScanChunkRequest(std::shared_ptr<CopysetNode> nodePtr,
                     ScanManager* scanManager, const ChunkRequest* request,
                     ChunkResponse* response, ::google::protobuf::Closure* done)
        : ChunkOpRequest(nodePtr, nullptr, request, response, done),
