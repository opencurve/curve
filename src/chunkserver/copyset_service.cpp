...
void CopysetServiceImpl::GetCopysetStatus(RpcController *controller,
    ...
    /**
     * Considering that calculating the hash value for query hash requires reading all chunk data from a copyset,
     * which is a very time-consuming operation, the request will have a "query hash" field. If it is set to false,
     * then there is no need to query the hash value of the copyset.
     */
