...
braft::FileAdaptor* CurveFilesystemAdaptor::open(
    ...
    // The use of the sync flag in Open is to avoid focusing on the close
    // one-time sync, which may cause jitter for 16MB chunk files
    oflag |= O_SYNC;

    // First, determine whether the current file needs to be filtered. If it
    // needs to be filtered, simply follow the following logic instead of
    // chunkfilepool If the open operation carries the create flag, it will be
    // taken from chunkfilepool, otherwise it will maintain its original meaning
    // If the file to be opened already exists, use the original meaning
    // directly
    if (!NeedFilter(path) && (oflag & O_CREAT) &&
...
bool CurveFilesystemAdaptor::delete_file(const std::string& path,
                                         bool recursive) {
    // 1. If it is a directory and recursive=true, then traverse the directory
    // content to recycle
    // 2. If it is a directory and recursive=false, then determine whether the
    // directory content is empty, and return false if it is not empty
    ...
    if (lfs_->DirExists(path)) {
        ...
        if (lfs_->FileExists(path)) {
            ...
            if (NeedFilter(path)) {
                ...
                // corresponding path file, and if it does not match, it will be
                // deleted directly
...
bool CurveFilesystemAdaptor::rename(const std::string& old_path,
    ...
    if (!NeedFilter(new_path) && lfs_->FileExists(new_path)) {
        // The chunkfilepool will internally check the legality of the
        // corresponding path file, and if it does not match, it will be deleted
        // directly
