...
/**
 * The purpose of RaftSnapshotFilesystemAdaptor is to take over the logic of creating chunk files
 * for internal snapshots in braft. Currently, within Curve, we directly retrieve pre-formatted chunk files
 * from the chunk file pool. However, within braft, 
 * the creation of chunk files during an install snapshot
 * process does not interact with the chunk file pool. Therefore, we want the install snapshot process
 * to also be able to retrieve chunk files directly from the chunk file pool. To achieve this,
 * we have implemented a hook in the file system operations within the install snapshot process.
 * This hook allows us to use the file system interface provided by Curve for file creation and deletion.
 */
...
/**
 * CurveFilesystemAdaptor inherits from Raft's PosixFileSystemAdaptor class. Within the Raft framework,
 * it uses the PosixFileSystemAdaptor class for file operations during snapshots. However, we only want
 * to use the `getchunk` and `recyclechunk` interfaces provided by the chunkfilepool when creating or
 * deleting files. Therefore, in this context, we have only implemented the `open` and `delete_file` interfaces.
 * Other interfaces are still used with the original internal Raft interfaces when called.
 */
class CurveFilesystemAdaptor : public braft::PosixFileSystemAdaptor {
 public:
     ...
     * @return: FileAdaptor is a class within Raft that encapsulates a file descriptor (fd). After opening a path with the `open` call, 
     *          all subsequent read and write operations on that file are performed through a pointer to this FileAdaptor class.
     *          It internally defines the following operations:
    ...
    * Raft internally uses the rename function of the local file system. If the target new path already exists as a file, it will overwrite that file. This behavior leads to the creation of a
    * 'temp_snapshot_meta' file, which is set up to ensure the atomic modification of the 'snapshot_meta' file.
    * Using rename helps ensure the atomicity of modifying the 'snapshot_meta' file. However, if the 'temp_snapshot_meta'
    * file is allocated from the chunk file pool and renamed directly, the chunk file used by the 'temp_snapshot_meta'
    * file will never be released. In this situation, a significant number of pre-allocated chunks can be consumed.
    * Therefore, the rename function is overridden here to first release the resources associated with the new path,
    * and then perform the rename operation.
