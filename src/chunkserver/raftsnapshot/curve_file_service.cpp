...
void CurveFileService::get_file(::google::protobuf::RpcController* controller,
    ...
    if (iter == _reader_map.end()) {
        lck.unlock();
        /**
         *In order to distinguish errors that do not exist in the file, and considering installation snapshots
         *The uri format of is: remote://ip:port/reader_id So using ENXIO
         *Represents an error where the reader ID does not exist
         */
    ...
    if (request->filename() == BRAFT_SNAPSHOT_ATTACH_META_FILE) {
        ...
        //Indicates that there is no snapshot attachment file list
