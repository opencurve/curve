...
CurveSnapshotCopier::~CurveSnapshotCopier() { CHECK(!_writer); }
...
void CurveSnapshotCopier::load_attach_meta_table() {
    ...
    // If the attach_meta_table is empty, then there are no snapshot attachment
    // files
    if (0 == meta_buf.size()) {
        return;
    }

    if (_remote_snapshot._attach_meta_table.load_from_iobuf_as_remote(
            meta_buf) != 0) {
...
int CurveSnapshotCopier::filter_before_copy(
    CurveSnapshotWriter* writer, braft::SnapshotReader* last_snapshot) {
...
void CurveSnapshotCopier::filter() {
    _writer = reinterpret_cast<CurveSnapshotWriter*>(
        _storage->create(!_filter_before_copy_remote));
    if (_writer == NULL) {
    ...
    if (_filter_before_copy_remote) {
        ...
        if (filter_before_copy(_writer, reader) != 0) {
            ...
            _writer =
                reinterpret_cast<CurveSnapshotWriter*>(_storage->create(true));
        }
        if (reader) {
...
void CurveSnapshotCopier::copy_file(const std::string& filename, bool attch) {
    ...
    lck.unlock();
    if (!session->status().ok()) {
        // If the file does not exist, delete the file that was just opened
        if (session->status().error_code() == ENOENT) {
            bool rc = _fs->delete_file(file_path, false);
            if (!rc) {
                LOG(ERROR) << "Fail to delete file" << file_path << " : "
                           << ::berror(errno);
                set_error(errno, "Fail to create delete file " + file_path);
            }
            return;
        }
...
void CurveSnapshotCopier::join() { bthread_join(_tid, NULL); }
