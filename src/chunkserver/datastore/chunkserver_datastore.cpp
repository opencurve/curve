...
CSErrorCode CSDataStore::DeleteChunk(
    ChunkID id, SequenceNum sn, std::shared_ptr<SnapContext> ctx) {
...
// to find the obj in < chunkfile, sn, snapshot>
void CSDataStore::searchChunkForObj(SequenceNum sn,
    ...
    for (; false == isFinish; ) {
        ...
        if (nullptr != cloneFile) {
            ...
            // if it is write, and the clone chunk is the self chunk,
            // use the lockless func
            if ((true == isWrite) && (selfPtr.get() == cloneFile.get())) {
                isFinish = cloneFile->DivideObjInfoByIndexLockless(
                    cloneSn, bitRanges, notInMapBitRanges, fobs->obj_infos);
            } else {
                isFinish = cloneFile->DivideObjInfoByIndex(
                    cloneSn, bitRanges, notInMapBitRanges, fobs->obj_infos);
            }
        ...
        // it is the end of the vector, just break the loop
        if (it_index == ctx->clones.end()) {
            break;
        }
        ...
        // it is the rootFile, use the rootFile to process
        if (it_index == ctx->clones.end()) {
            assert(cloneNo > 0);
            // if the cloneNo is not zero,
            // use the cloneNo to get the rootChunkFile
            if (0 != cloneNo) {
                rootChunkFile = datastore->GetCloneCache(
                    ctx->virtualId, cloneNo);
            }
            break;
        }
    ...
    if (nullptr != rootChunkFile) {
        ...
        isFinish = rootChunkFile->DivideObjInfoByIndex(
            cloneSn, bitRanges, notInMapBitRanges, fobsi->obj_infos);
        ...
        for (auto& btmp : bitRanges) {
            ...
            tinfo.length = (btmp.endIndex - btmp.beginIndex + 1)
                << datastore->BLOCK_SIZE_SHIFT;
            tinfo.sn = 0;
...
CSErrorCode CSDataStore::ReadByObjInfo(
    CSChunkFilePtr fileptr, char* buf, ObjectInfo& objInfo) {
    CSErrorCode errorCode;
    if (nullptr == fileptr) {
        DVLOG(9)  << "ReadByObjInfo read file = null , snapptr = "
            << static_cast<const void *> (objInfo.snapptr)
    ...
    // should memset with 0, or with never mind? root chunk does not exist
    if (nullptr == fileptr) {
        ...
        errorCode = fileptr->ReadSpecifiedChunk(objInfo.sn, buf,
            objInfo.offset, objInfo.length);
        if (errorCode != CSErrorCode::Success) {
            LOG(WARNING) << "Read chunk file failed."
                         << "ReadByObjInfo read sn = " << objInfo.sn;
            return errorCode;
        ...
        errorCode = fileptr->ReadSpecifiedSnap(objInfo.sn, objInfo.snapptr,
            buf, objInfo.offset, objInfo.length);
        if (errorCode != CSErrorCode::Success) {
            LOG(WARNING) << "Read chunk file failed."
                         << "ReadByObjInfo read sn = " << objInfo.sn;
            return errorCode;
...
void CSDataStore::SplitDataIntoObjs(SequenceNum sn,
    ...
    DVLOG(3) << "SplitDataIntoObjs input offset = " << offset
             << ", length = " << length
    ...
    searchChunkForObj(sn, objInfos, beginIndex,
        endIndex, ctx, datastore, isWrite);
    ...
    for (auto& objInfo : objInfos) {
        if (objInfo->fileptr == nullptr) {
            ...
            outputinfo += "fileptr = " +
                std::to_string(objInfo->fileptr->getChunkId()) + ", ";
        }
        for (auto& objinfo : objInfo->obj_infos) {
            outputinfo += " sn = " + std::to_string(objinfo.sn) +
                ", offset = " + std::to_string(objinfo.offset) +
                ", length = " + std::to_string(objinfo.length) + "; ";
        }
        ...
        for (auto& fileobj : objInfos) {
            for (auto& objInfo : fileobj->obj_infos) {
                ...
                CHECK(((objInfo.offset - offset) >= 0))
                        ...
                        << ", objInfo.offset = " << objInfo.offset
                        << ", objInfo.length = " << objInfo.length;
                CHECK(((objInfo.offset - offset) + objInfo.length) <= length)
                        << "offset = " << offset << ", length = " << length
                        << ", objInfo.offset = " << objInfo.offset
                        << ", objInfo.length = " << objInfo.length;
#endif
                errorCode = ReadByObjInfo(fileobj->fileptr,
                    buf + (objInfo.offset - offset), objInfo);
                if (errorCode != CSErrorCode::Success) {
                    LOG(WARNING) << "Read chunk file failed."
                                << "ChunkID = " << id;
                    return errorCode;
...
CSErrorCode CSDataStore::FlattenChunk(ChunkID id, SequenceNum sn,
    ...
    // for clone file the orgin clone must be exists
    assert(nullptr != chunkFile);
...
CSErrorCode CSDataStore::WriteChunk(ChunkID id, SequenceNum sn,
    ...
    DVLOG(3) << "WriteChunk id = " << id << ", sn = " << sn
             ...
             << ", fileID = " << fileID << ", cloneNo = " << cloneCtx->cloneNo
             << ", virtualId = " << cloneCtx->virtualId
             << ", rootId = " << cloneCtx->rootId;
    ...
    // assert (chunkFile->getCloneNumber() > 0);
    errorCode = chunkFile->cloneWrite(
        sn, buf, offset, length, cost, cloneCtx, this, ctx);
        if (errorCode != CSErrorCode::Success) {
            LOG(WARNING) << "Write chunk file failed."
                        << "ChunkID = " << id;
            return errorCode;
...
CSErrorCode CSDataStore::WriteChunk(ChunkID id,
    ...
    DVLOG(3) << "WriteChunk id = " << id << ", sn = " << sn
             << ", offset = " << offset << ", length = " << length
             << ", chunkIndex = " << chunkIndex
             << ", fileID = " << fileID << ", cloneNo = " << 0
             << ", virtualId = " << chunkIndex
             << ", cloneSourceLocation = " << cloneSourceLocation;
