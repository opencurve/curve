...
class DownloadClosure : public Closure {
 public:
    DownloadClosure(std::shared_ptr<ReadChunkRequest> readRequest,
                    ...
                    AsyncDownloadContext* downloadCtx, Closure* done);

    void Run();

    void SetFailed() { isFailed_ = true; }

    AsyncDownloadContext* GetDownloadContext() { return downloadCtx_; }
...
class CloneCore : public std::enable_shared_from_this<CloneCore> {
 ...
 public:
    CloneCore(uint32_t sliceSize, bool enablePaste,
        ...
        : sliceSize_(sliceSize), enablePaste_(enablePaste), copyer_(copyer) {}
    virtual ~CloneCore() {}
 ...
 protected:
    /**
     * When a local chunk file exists, read data based on the locally recorded
     * clone and bitmap information Will involve reading remote files and
     * merging with local files to return results
    ...
    /**
     * When the local chunk file does not exist, read the data according to the
     * clonesource information in the user request context Not involving merge
     * local results
     * @param[in/out] readRequest: User Request&Response Context
     */
    ...
    /**
     * Set the response of the read chunk type, including the returned data and
     * other return parameters Read the written area from the local chunk, and
     * obtain the unwritten area from the cloned data Then merge the data into
     * memory
     * @param readRequest: User's ReadRequest
     * @param cloneData: The data copied from the source has the same starting
     * offset as the offset in the request
     * @return: Success returns 0, failure returns -1
     */
    int SetReadChunkResponse(std::shared_ptr<ReadChunkRequest> readRequest,
                             const butil::IOBuf* cloneData);

    // Read the previously written regions from the local chunk and merge them
    // into clone data
 ...
 private:
    ...
    uint32_t sliceSize_;
    // Determine whether a read chunk type request requires paste. True requires
    // paste, while false indicates no need
    bool enablePaste_;
