...
/**
 *Carry all the contextual closures of the op request and pass them to the raft for processing through the braft::Task,
 *There are two places to call:
 *1. The op request is processed normally by the raft, and will be called and returned when it is finally applied
 *2. After the op request was packaged for raft processing, an error occurred before it could be processed, such as leader
 *If the step down becomes a non leader, it will explicitly return an error to the client in advance
 */
