...
class ConfigurationChangeDone : public braft::Closure {
 public:
    ...
    // The configuration change information corresponding to this configuration
    // change
...
class CopysetNode : public braft::StateMachine,
 ...
 public:
    ...
    CopysetNode(const LogicPoolID& logicPoolId, const CopysetID& copysetId,
     ...
     */
    virtual int GetConfChange(ConfigChangeType* type, Configuration* oldConf,
                              Peer* alterPeer);

    /**
     * @brief: Obtain the status value of the copyset node for comparing data
    ...
    /**
     * @brief:  Get the status of the copyset node, actually calling the
     * get_status interface of the Raft node
     ...
     * @return returns true for successful acquisition, false for failed
     * acquisition
     */
    ...
    /**
     * The following interfaces are all interfaces that inherit the
 ...
 public:
     ...
     * in the context of block storage, all operations are idempotent, so there
     * is no need to actually copy the data.
     */
     ...
     *  1. Follower nodes catch up with the leader by installing a snapshot. In
     * this case, there are chunk data and snapshot data under the snapshot
     * directory.
     *  2. When a node restarts, it performs a snapshot load and then replays
     * the logs. In this case, there is no data under the snapshot directory, so
     * nothing needs to be done.
     *  TODO(wudemiao): When installing a snapshot, there is a possibility of
     * doubling the space usage. Consider the following scenario: a follower
     * lags behind and then recovers data by installing a snapshot from the
     * leader. It will first download all the data from the leader and then call
     * snapshot load to load the snapshot. During this period, the space usage
     * doubles. Later, we need to control the number of disks participating in
     * the installation of snapshots.
     */
    int on_snapshot_load(::braft::SnapshotReader* reader) override;

    /**
     * The new leader will call this interface after applying noop, indicating
     * that this leader can provide read/write services.
    ...
    /**
     * This function will be called when configuring the change log entry
     * application, and currently this interface will be utilized Update
     * configuration epoch value
     * @param conf: The latest configuration of the current replication group
     * @param index log index
     */
    void on_configuration_committed(const Configuration& conf,
                                    int64_t index) override;  // NOLINT
     ...
     * @param ctx: Change the leader up and down to obtain the reasons for the
     * new leader and start following
     */
 ...
 private:
    inline std::string GroupId() { return ToGroupId(logicPoolId_, copysetId_); }
 ...
 private:
    ...
    // Copy the current tenure of the group. If<=0, it indicates that it is not
    // a leader
    ...
    // The target of the transfer leader is valid when the status is
    // TRANSFERRING
    Peer transferee_;
