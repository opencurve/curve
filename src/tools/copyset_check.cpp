...
void CopysetCheck::PrintHelp(const std::string& command) {
    ...
    if (command == kCheckCopysetCmd) {
         ...
         "-logicalPoolId=2 -copysetId=101 [-mdsAddr=127.0.0.1:6666] [-margin=1000]" << std::endl << std::endl;  // NOLINT
    } else if (command == kCheckChunnkServerCmd) {
        std::cout << "curve_ops_tool check-chunkserver "
        "-chunkserverId=1 [-mdsAddr=127.0.0.1:6666] [-margin=1000]" << std::endl;
        ...
        std::cout << "curve_ops_tool check-server -serverId=1 [-mdsAddr=127.0.0.1:6666] [-margin=1000]" << std::endl;  // NOLINT
