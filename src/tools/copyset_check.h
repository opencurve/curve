...
class CopysetCheck : public CurveTool {
 public:
     ...
     *   The standard for replication group health is that no replica is in the
     * following state. The following order is sorted by priority, If the above
     * one is met, the following one will not be checked
     *   1. The leader is empty (the information of the replication group is
     * based on the leader, and cannot be checked without a leader)
     ...
     *   6. For a cluster, it is also necessary to determine whether the number
     * of copysets and the number of leaders on the chunkserver are balanced,
     *        Avoid scheduling that may cause instability in the cluster in the
     * future
     * @param command The command to be executed by currently
     * includescheck-copyset, check-chunkserver, check-server, check-cluster,
     * etc
 ...
 private:
    ...
    // Print the results of the inspection, how many copies are there in total,
    // and how many are unhealthy
    void PrintStatistic();
