...
class CopysetCheckCore {
 public:
     ...
     * @param[out] unhealthyChunkServers optional parameter, a list of unhealthy
     * chunkservers with copyset on the server
     *
     * @return Health returns 0, unhealthy returns -1
     */
     ...
     * @return returns 0 if the check is normal, or -1 if the check fails or
    ...
    /**
     * @brief to obtain a list of copysets, usually called after checking, and
     * then printed out
    ...
    /**
     * @brief: Obtain a list of chunkservers with service exceptions during the
     * inspection process, which is usually called after the inspection and
     * printed out
    ...
    /**
     * @brief: Obtain the list of failed chunkservers for copyset during the
     * check process, which is usually called after the check and printed out
 ...
 private:
    /**
     * @brief Analyze the replication group information for the specified
     * groupId from iobuf, Each replication group's information is placed in a
     * map
     *
     * @param gIds: The groupId of the replication group to be queried. If it is
     * empty, all queries will be performed
     * @param iobuf The iobuf to analyze
     * @param[out] maps A list of copyset information, where each copyset's
    ...
    ChunkServerHealthStatus CheckCopysetsOnChunkServer(
        const std::string& chunkserverAddr,
        const std::set<std::string>& groupIds, bool queryLeader = true,
        std::pair<int, butil::IOBuf>* record = nullptr, bool queryCs = true);

    /**
     * @brief Check the health status of all copysets on a server
     *
     * @param serverId Server ID
     * @param serverIp Just specify one of the server's IP, serverId, or
     * serverIp
     * @param queryLeader Does the send RPC queries to the server where the
     * leader is located, For checking the cluster, all servers will be
     * traversed without querying
     *
     * @return Health returns 0, unhealthy returns -1
     */
    ...
    /**
     * @brief: Analyze whether the copyset is healthy based on the copyset
     * information in the leader's map, and return 0 if it is healthy. Otherwise
     *         Otherwise, an error code will be returned
     *
     * @param map The copyset information of the leader is stored as key value
     * pairs
     *
     ...
     *
     * @param chunkserverAddr Address of  chunkserver
     * @param[out] iobuf The responseattachment returned by is valid when 0 is
     * returned
     *
     * @return returns 0 for success, -1 for failure
     */
    ...
    /**
     * @brief: Using the copyset configuration group in mds as a reference,
     * check if chunkserver is in the copyset configuration group
     *
 ...
 private:
    ...
    // Used to store the copyset list on accessed chunkservers to avoid
    // duplicate RPCs
    std::map<std::string, std::set<std::string>> chunkserverCopysets_;

    // When querying a single copyset, save the detailed information of the
    // replication group
