...
class SnapshotCloneClient {
 public:
     ...
     * @param serverAddr Address of snapshot clone server, supporting multiple
     * addresses separated by ','
     * @param dummyPort dummyPort list, if only one is entered
     *        All servers use the same dummy port, separated by strings if there
     * are multiple Set different dummy ports for each server
     * @return
     *   Success: 0
     *   Failed: -1
    ...
    /**
     * @brief Get the address of the snapshot clone server for the current
     * service
     */
     ...
     *          Only when consistent with the service address is considered
     * online
