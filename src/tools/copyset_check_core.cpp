...
ChunkServerHealthStatus CopysetCheckCore::CheckCopysetsOnChunkServer(
    ...
    if (res != 0) {
        //If the query for chunkserver fails and it is considered offline, remove all the
        //Add copysets to peerNotOnlineCopysets_ inside
    ...
    for (auto& copysetInfo : copysetInfos) {
        ...
        if (state == kStateLeader) {
            ...
            //Other situations include ERROR, UNINITIALIZED, SHUTTING, and SHUTDOWN, all of which are considered unhealthy
            //Inside the copyset
    ...
    //Traversing copysets without leaders
...
//The information of each copyset will be stored in a map, and the key of the map includes
//groupId: The groupId of the replication group
//peer_id: 10.182.26.45:8210:0 format peer id
//state: The state of the node, such as LEADER, FOLLOWER, CANDIDATE, etc
//peers: Members in the configuration group, separated by spaces
//last_log_id: The index of the last log entry
//leader: This key only exists when the state is LEADER, pointing to the replication group leader
//
...
//next_index is the next index to be sent to the follower
//flying_append_entries_size is the number of unconfirmed entries sent out
//Idle indicates that the snapshot is not being installed. If the snapshot is being installed, it is installing snapshot {12, 3},
//1234 and 3 are the index and term of the last log entry included in the snapshot, respectively
//hc, ac, and ic are heartbeat, append entry, respectively sent to the follower,
//Number of rpc for install snapshots
