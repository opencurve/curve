...
class ConsistencyCheck : public CurveTool {
 public:
     ...
     * @param checkHash Does check hash? If false, check apply index instead of
     * hash
     ...
     * @return returns 0 for success, -1 for failure
     */
    int CheckCopysetConsistency(const CopySet copysetId, bool checkHash);
 ...
 private:
     ...
     * @param[out] copysetIds  The list copysetIds is valid when the return
     * value is 0
     * @return returns 0 for success, -1 for failure
     */
    ...
    /**
     * @brief Check the consistency of the hash of the specified chunk in the
     * copyset
     * @param copysetId The copysetId to be checked
     * @param csAddrs The address of the chunkserver corresponding to the
     * copyset
     * @return consistently returns 0, otherwise returns -1
     */
    int CheckCopysetHash(const CopySet& copyset, const CsAddrsType& csAddrs);
     ...
     * @return consistently returns 0, otherwise returns -1
     */
    int CheckChunkHash(const Chunk& chunk, const CsAddrsType& csAddrs);
     ...
     * @return consistently returns 0, otherwise returns -1
     */
    int CheckApplyIndex(const CopySet copyset, const CsAddrsType& csAddrs);
