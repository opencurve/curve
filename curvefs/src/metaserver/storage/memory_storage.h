...
enum CONTAINER_TYPE : unsigned char {
    kTypeUnordered = 1,
    kTypeUnorderedSeralized = 2,
    kTypeOrdered = 3,
    kTypeOrderedSeralized = 4,
};
...
class ValueWrapper {
 public:
    ...
    std::shared_ptr<ValueType> Message() {
        return value_;
    }
...
class MemoryStorage : public KVStorage, public StorageTransaction {
 public:
    using UnorderedContainerType =
        std::unordered_map<std::string, std::shared_ptr<ValueWrapper>>;

    using UnorderedSeralizedContainerType =
        std::unordered_map<std::string, std::string>;

    using OrderedContainerType =
        absl::btree_map<std::string, std::shared_ptr<ValueWrapper>>;
