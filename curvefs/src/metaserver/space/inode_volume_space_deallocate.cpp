...
void InodeVolumeSpaceDeallocate::CalDeallocatableSpace() {
    ...
    uint64_t typicalInode = 0;
    for (iter->SeekToFirst();; iter->Next()) {
        // update according to metaserver client
        if (increase.size() >= executeOpt_.batchClean ||
            (!iter->Valid() && increase.size() > 0)) {
        ...
        if (!ok) {
            ...
            typicalInode = key.inodeId;
        }

        // fill IncreaseDeallocatableBlockGroup
        ok = DeallocatableSpaceForInode(key, &increase);
        if (!ok) {
...
bool InodeVolumeSpaceDeallocate::DeallocatableSpaceForInode(
...
void InodeVolumeSpaceDeallocate::DeallocatbleSpaceForVolumeExtent(
    ...
    std::unordered_set<uint64_t> relatedBlockGroup;
    ...
    for (const auto &next : slice.extents()) {
        ...
        if (relatedBlockGroup.count(blockGroupOffset)) {
            ...
            relatedBlockGroup.emplace(blockGroupOffset);
    ...
    for (const auto &item : relatedBlockGroup) {
        ...
        VLOG(6) << "InodeVolumeSpaceDeallocate deallocatable space for inode:"
                ...
                << ", related block group:" << item << ", increase size:"
...
bool InodeVolumeSpaceDeallocate::DeallocateInode(uint64_t blockGroupOffset,
    ...
    for (const auto &inodeId : inodelist) {
        ...
        // 1
        UpdateDeallocateInodeExtentSlice(blockGroupOffset, decrease, &sliceList,
                                         &deallocatableVolumeSpace);
        UpdateVolumeExtentClosure closure(nullptr, true);
        metaCli_->AsyncUpdateVolumeExtent(fsId_, inodeId, sliceList, &closure);
...
void InodeVolumeSpaceDeallocate::UpdateDeallocateInodeExtentSlice(
    uint64_t blockGroupOffset, uint64_t *decrease,
    VolumeExtentSliceList *sliceList,
    std::vector<Extent> *deallocatableVolumeSpace) {
    uint64_t boundary =
        ...
        extents->erase(
            std::remove_if(
                ...
                [&](const VolumeExtent &extent) {
                    // by default, 4K bitmap is not pre-allocated, so the extent
                    // will not cross block group.
