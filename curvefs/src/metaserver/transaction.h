...
class TxManager {
 ...
 private:
    ...
    bool InsertPendingTx(uint32_t fsId, const RenameTx& tx);

    void DeletePendingTx(uint32_t fsId);

    bool FindPendingTx(uint32_t fsId, RenameTx* pendingTx);
 ...
 private:
    ...
    std::unordered_map<uint32_t, RenameTx> pendingTx_;
};
