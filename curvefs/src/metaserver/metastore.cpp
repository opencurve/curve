...
bool MetaStore::Load(const std::string& path) {
    auto callbackFunc = [&](ENTRY_TYPE type, uint32_t paritionId,
        ...
        if (type == ENTRY_TYPE::PARTITION) {
            ...
            std::shared_ptr<Partition> partition = GetPartiton(paritionId);
            if (partition == nullptr) {
                LOG(ERROR) << "MetaStore Load GetPartiton fail, type INODE";
                return false;
            }
            ...
            std::shared_ptr<Partition> partition = GetPartiton(paritionId);
            if (partition == nullptr) {
                LOG(ERROR) << "MetaStore Load GetPartiton fail, type DENTRY";
                return false;
            }
...
void MetaStore::SaveBack(const std::string& path, OnSnapshotSaveDone* done) {
    ...
    std::vector<std::shared_ptr<Iterator>> iteractorList;
    ...
    iteractorList.push_back(partitionContainerIter);

    // add inode and dentry iterator
    for (auto it : partitionMap_) {
        // add inode iterator
        std::shared_ptr<ContainerIterator<std::unordered_map<
            curvefs::metaserver::InodeKey, curvefs::metaserver::Inode,
            curvefs::metaserver::hashInode>>>
            inodeIter = std::make_shared<ContainerIterator<std::unordered_map<
                curvefs::metaserver::InodeKey, curvefs::metaserver::Inode,
                curvefs::metaserver::hashInode>>>(
                ENTRY_TYPE::INODE, it.second->GetPartitionId(),
                it.second->GetInodeContainer());
        iteractorList.push_back(inodeIter);

        // add dentry iterator
        std::shared_ptr<ContainerIterator<std::unordered_map<
            curvefs::metaserver::DentryKey, curvefs::metaserver::Dentry,
            curvefs::metaserver::HashDentry>>>
            dentryIter = std::make_shared<ContainerIterator<std::unordered_map<
                curvefs::metaserver::DentryKey, curvefs::metaserver::Dentry,
                curvefs::metaserver::HashDentry>>>(
                ENTRY_TYPE::DENTRY, it.second->GetPartitionId(),
                it.second->GetDentryContainer());
        iteractorList.push_back(dentryIter);
    }

    auto mergeIterator = std::make_shared<MergeIterator>(iteractorList);
...
MetaStatusCode MetaStore::CreateDentry(const CreateDentryRequest* request,
    ...
    ReadLockGuard readLockGuard(rwLock_);
    std::shared_ptr<Partition> partition = GetPartiton(request->partitionid());
    if (partition == nullptr) {
...
*/
std::shared_ptr<Partition> MetaStore::GetPartiton(uint32_t fsId,
                                                  uint64_t inodeId) {
    for (auto it : partitionMap_) {
        if (it.second->IsInodeBelongs(fsId, inodeId)) {
            return it.second;
        }
    }
    return nullptr;
...
std::shared_ptr<Partition> MetaStore::GetPartiton(uint32_t partitionId) {
    auto it = partitionMap_.find(partitionId);
    if (it != partitionMap_.end()) {
        return it->second;
    }
