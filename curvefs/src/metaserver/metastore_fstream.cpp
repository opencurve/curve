...
bool MetastoreFStream::LoadPartition(uint32_t partitionId,
    ...
    if (!converter_->ParseFromString(value, &partitionInfo)) {
...
bool MetastoreFStream::LoadInode(uint32_t partitionId,
    ...
    Inode inode;
    if (!converter_->ParseFromString(value, &inode)) {
...
bool MetastoreFStream::LoadDentry(uint32_t partitionId,
    ...
    Dentry dentry;
    if (!converter_->ParseFromString(value, &dentry)) {
...
bool MetastoreFStream::LoadPendingTx(uint32_t partitionId,
    ...
    if (!converter_->ParseFromString(value, &pendingTx)) {
...
bool MetastoreFStream::LoadS3ChunkInfoList(uint32_t partitionId,
                                           const std::string& key,
                                           const std::string& value) {
    auto partition = GetPartition(partitionId);
    if (nullptr == partition) {
    ...
    if (convetrer_->ParseFromString(key, &key4list)) {
        LOG(ERROR) << "Decode Key4S3ChunkInfoList failed";
        return false;
    } else if (convetrer_->ParseFromString(value, &list)) {
    ...
    MetaStatusCode rc = partition->GetOrModifyS3ChunkInfo(
        key4list.fsId, key4list.inodeId, list, &iterator, false, false);
    if (rc != MetaStatusCode::OK) {
...
std::shared_ptr<Iterator> MetastoreFStream::NewPartitionIterator() {
    ...
    for (const auto& item : partitionMap_) {
        ...
        if (!converter_->SerializeToString(partitionInfo, &value)) {
            return nullptr;
        }
...
std::shared_ptr<Iterator> MetastoreFStream::NewPendingTxIterator(
    ...
    if (partition->FindPendingTx(&pendingTx)) {
        if (!converter_->SerializeToString(pendingTx, &value)) {
            return nullptr;
        }
...
bool MetastoreFStream::Save(const std::string& path) {
    ...
    for (const auto& item : partitionMap_) {
        ...
        iterator = NewInodeS3ChunkInfoList(partition);  // inode s3chunkinfo
        children.push_back(iterator);
    }
