...
bool MetaStoreFStream::Save(const std::string& path,
    ...
    if (succ) {
        LOG(INFO) << "MetaStoreFStream save success: " << oss.str();
    } else {
        LOG(ERROR) << "MetaStoreFStream save failed: " << oss.str();
    }
