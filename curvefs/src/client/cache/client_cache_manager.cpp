...
void FsCacheManager::FlowControl(uint64_t writeSize) {
    {
        std::lock_guard<std::mutex> lockguard(ioMtx_);
        uint64_t size = wDataCacheByte_.load(std::memory_order_relaxed);
        if ((size + writeSize) >= writeCacheMaxByte_) {
            LOG(INFO) << "write cache is full, wait flush. size:" << size
                      << ", writeCacheMaxByte:" << writeCacheMaxByte_;
            // offer to do flush
    ...
    uint64_t memCacheRatio = MemCacheRatio();
...
int FileCacheManager::Write(uint64_t offset, uint64_t length,
                            const char *dataBuf) {
    uint64_t chunkSize = fsCacheManager_->GetChunkSize();
...
void FileCacheManager::WriteChunk(uint64_t index, uint64_t chunkPos,
    ...
    if (dataCache) {
        ...
        chunkCacheManager->WriteNewDataCache(fsCacheManager_, chunkPos,
                                             writeLen, dataBuf);
    }
...
FileCacheManager::FindOrCreateChunkCacheManager(uint64_t index) {
    ...
    ChunkCacheManagerPtr chunkCacheManager =
        std::make_shared<ChunkCacheManager>(index, fsCacheManager_);
...
void FileCacheManager::Read(uint64_t inodeId, uint64_t offset, uint64_t length,
                           char *dataBuf, std::vector<ReadRequest> *totalRequests) {
    uint64_t chunkSize = fsCacheManager_->GetChunkSize();
    uint64_t index = offset / chunkSize;
    uint64_t chunkPos = offset % chunkSize;
    uint64_t readLen = 0;
...
class AsyncPrefetchCallback {
 public:
    AsyncPrefetchCallback(uint64_t inode, S3Adaptor *s3Client)
...
void ChunkCacheManager::ReadByWriteCache(uint64_t chunkPos, uint64_t readLen,
    ...
    if (dataWCacheMap_.empty()) {
        ...
        ReadRequest request;
        request.offset = index_ * chunkSize + chunkPos;
        request.len = readLen;
        request.buf = dataBuf + dataBufOffset; 
    ...
    for (; iter != dataWCacheMap_.end(); iter++) {
        ...
        if (((chunkPos + len) >= iter->second->GetChunkPos()) &&
            ...
            for (; iterDel != waitDelVec.end(); iterDel++) {
                ...
                fsCacheManager_->DataCacheNumFetchSub(1);
                VLOG(9) << "FindWriteableDataCache() DataCacheByteDec1 len:"
                        << iter->second->GetLen();
                fsCacheManager_->DataCacheByteDec(
...
void ChunkCacheManager::WriteNewDataCache(FsCacheManager *fsCacheManager,
                                          uint32_t chunkPos, uint32_t len,
                                          const char *data) {
    DataCachePtr dataCache = std::make_shared<DataCache>(
        fsCacheManager, this->shared_from_this(), chunkPos, len, data);
...
DataCache::DataCache(FsCacheManager *fsCacheManager,
                     ChunkCacheManagerPtr chunkCacheManager, uint64_t chunkPos,
                     uint64_t len, const char *data)
    : fsCacheManager_(fsCacheManager), chunkCacheManager_(chunkCacheManager),
      dirty_(true), delete_(false), inReadCache_(false) {
    uint64_t blockSize = fsCacheManager->GetBlockSize();
    uint32_t pageSize = fsCacheManager->GetPageSize();
    chunkPos_ = chunkPos;
    len_ = len;
...
CURVEFS_ERROR DataCache::Flush(uint64_t inodeId, bool force, bool toS3) {
    uint64_t blockSize = fsCacheManager_->GetBlockSize();
    uint64_t chunkSize = fsCacheManager_->GetChunkSize();
    uint32_t flushIntervalSec = fsCacheManager_->GetFlushInterval();
    uint32_t fsId = fsCacheManager_->GetFsId();
    uint64_t blockPos;
    uint64_t blockIndex;
    ...
    if (isFlush) {
        ...
        auto ret =
            fsCacheManager_->GetStorageAdaptor()->GetUnderStorage()->Write(
                inodeId, offset, len_, data);
        if (ret < 0) {
