 ...
 * Author: huyao
 */
#ifndef CURVEFS_SRC_CLIENT_CACHE_CLIENT_CACHE_MANAGER_H_
#define CURVEFS_SRC_CLIENT_CACHE_CLIENT_CACHE_MANAGER_H_
...
struct ReadRequest {
    ...
    uint64_t len = 0; // file len
    uint64_t bufOffset = 0; // offset of user's read buf
};
...
class DataCache : public std::enable_shared_from_this<DataCache> {
 public:
    DataCache(FsCacheManager *fsCacheManager,
 ...
 private:
    FsCacheManager *fsCacheManager_;
...
class ChunkCacheManager
 ...
 public:
    ChunkCacheManager(uint64_t index, FsCacheManager *fsCacheManager)
        : index_(index), fsCacheManager_(fsCacheManager) {}
    virtual ~ChunkCacheManager() = default;

    virtual void WriteNewDataCache(FsCacheManager *fsCacheManager,
...
class FileCacheManager {
 public:
    ...
    virtual void Read(uint64_t inodeId, uint64_t offset, uint64_t length,
                     char *dataBuf);
...
class FsCacheManager {
 public:
    FsCacheManager(S3Adaptor *s3ClientAdaptor,
    ...
    void FlowControl(uint64_t writeSize);
...
#endif  // CURVEFS_SRC_CLIENT_CACHE_CLIENT_CACHE_MANAGER_H_
