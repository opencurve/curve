...
void VFS::InitOption(std::shared_ptr<Configure> cfg, FuseClientOption* option) {
    Configuration conf;
...
VFS::VFS(std::shared_ptr<Configure> cfg) {
    ...
    op_ = std::make_shared<OperationsImpl>(client_, userPerm);
...
CURVEFS_ERROR VFS::Mount(const std::string& fsname,
    ...
    rc = op_->Mount(fsname, mountpoint);
    return rc;
