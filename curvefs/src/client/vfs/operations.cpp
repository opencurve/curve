...
CURVEFS_ERROR OperationsImpl::Write(Ino ino,
    ...
    auto req = ctx->GetRequest();
    auto fi = ctx->GetFileInfo();
    return client_->FuseOpWrite(req, ino, buffer, size,
                                offset, fi, fileOut);
}
...
CURVEFS_ERROR OperationsImpl::Lookup(Ino parent,
    ...
    if (rc == CURVEFS_ERROR::OK) {
        auto fs = client_->GetFileSystem();
        auto attrWatcher = fs->BorrowMember().attrWatcher;
        fs->SetEntryTimeout(entryOut);
        attrWatcher->RemeberMtime(entryOut->attr);  // TODO(Wine93): remove it.
    }
    return rc;
}
...
CURVEFS_ERROR OperationsImpl::GetAttr(Ino ino, AttrOut* attrOut) {
    ...
    if (rc == CURVEFS_ERROR::OK) {
        ...
        attrWatcher->RemeberMtime(attrOut->attr);  // TODO(Wine93): remove it.
    }
    return rc;
}
