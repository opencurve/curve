...
class VFS {
 public:
    ...
    VFS(std::shared_ptr<Configure> cfg,
        std::shared_ptr<FuseClient> client = nullptr);
 ...
 private:
    void InitOption(std::shared_ptr<Configure> cfg, FuseClientOption* option);

    void PurgeAttrCache(Ino ino);

    void PurgeEntryCache(Ino parent, const std::string& name);

    CURVEFS_ERROR DoMkDir(const std::string& path, uint16_t mode);
 ...
 private:
    std::shared_ptr<FuseS3Client> fuseClient_;
