...
ChannelManager<T>::GetOrCreateStreamChannel(const T &id,
    ...
    // NOTE: we must use "pooled" channel instead of "single" channel
    // for streaming RPC, because the BRPC can't distinguish the general
    // RPC with streaming RPC in one connection.
