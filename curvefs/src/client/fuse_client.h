 ...
 public:
    FuseClient()
      : mdsClient_(std::make_shared<MdsClientImpl>()),
        ...
        inodeManager_(std::make_shared<InodeCacheManagerImpl>()),
        dentryManager_(std::make_shared<DentryCacheManagerImpl>()),
    ...
    virtual CURVEFS_ERROR write(fuse_req_t req, fuse_ino_t ino,
        const char *buf, size_t size, off_t off,
        struct fuse_file_info *fi, size_t *wSize) = 0;

    virtual CURVEFS_ERROR read(fuse_req_t req,
            fuse_ino_t ino, size_t size, off_t off,
            struct fuse_file_info *fi,
            char **buffer,
            size_t *rSize) = 0;

    virtual CURVEFS_ERROR lookup(fuse_req_t req, fuse_ino_t parent,
        const char *name, fuse_entry_param *e);

    virtual CURVEFS_ERROR open(fuse_req_t req, fuse_ino_t ino,
              struct fuse_file_info *fi);

    virtual CURVEFS_ERROR create(fuse_req_t req, fuse_ino_t parent,
        const char *name, mode_t mode, struct fuse_file_info *fi,
        fuse_entry_param *e) = 0;

    virtual CURVEFS_ERROR mknod(fuse_req_t req, fuse_ino_t parent,
            const char *name, mode_t mode, dev_t rdev,
            fuse_entry_param *e) = 0;

    virtual CURVEFS_ERROR mkdir(fuse_req_t req, fuse_ino_t parent,
            const char *name, mode_t mode,
            fuse_entry_param *e);

    virtual CURVEFS_ERROR unlink(fuse_req_t req, fuse_ino_t parent,
        const char *name);

    virtual CURVEFS_ERROR rmdir(fuse_req_t req, fuse_ino_t parent,
        const char *name);

    virtual CURVEFS_ERROR opendir(fuse_req_t req, fuse_ino_t ino,
             struct fuse_file_info *fi);

    virtual CURVEFS_ERROR readdir(
            fuse_req_t req, fuse_ino_t ino, size_t size, off_t off,
            struct fuse_file_info *fi,
            char **buffer,
            size_t *rSize);

    virtual CURVEFS_ERROR getattr(fuse_req_t req, fuse_ino_t ino,
             struct fuse_file_info *fi, struct stat *attr);

    virtual CURVEFS_ERROR setattr(
