...
CURVEFS_ERROR FuseS3Client::Init(const FuseClientOption &option) {
    ...
    s3Adaptor_->Init(s3AdaptorOption, s3Client_);
    return ret;
}
...
void FuseS3Client::init(void *userdata, struct fuse_conn_info *conn) {
...
void FuseS3Client::destroy(void *userdata) {
    struct MountOption *mOpts = (struct MountOption *) userdata;
    std::string fsName = (mOpts->fsName == nullptr) ? "" : mOpts->fsName;
    std::string mountPointStr =
        (mOpts->mountPoint == nullptr) ? "" : mOpts->mountPoint;
...
CURVEFS_ERROR FuseS3Client::write(fuse_req_t req, fuse_ino_t ino,
...
CURVEFS_ERROR FuseS3Client::read(fuse_req_t req,
...
CURVEFS_ERROR FuseS3Client::create(fuse_req_t req, fuse_ino_t parent,
    const char *name, mode_t mode, struct fuse_file_info *fi,
    fuse_entry_param *e) {
    return MakeNode(req, parent, name, mode, FsFileType::TYPE_S3, e);
}

CURVEFS_ERROR FuseS3Client::mknod(fuse_req_t req, fuse_ino_t parent,
