...
DEFINE_string(copysetId, "1,2,3", "copyset id");
DEFINE_bool(detail, false, "show more info mation");
...
std::function<void(curve::common::Configuration*, google::CommandLineFlagInfo*)>
    SetRpcTimeout =
        std::bind(&SetFlagInfo<uint64_t>, std::placeholders::_1,
                  std::placeholders::_2, "rpcTimeout", &FLAGS_rpcTimeout);
...
std::function<std::string(const std::vector<std::string>&)> StrVec2Str =
    [](const std::vector<std::string>& strVec) -> std::string {
...
std::function<std::string(const mds::heartbeat::CopySetInfo&)>
    HeartbeatCopysetInfo2Str =
        [](const mds::heartbeat::CopySetInfo& copySetInfo) -> std::string {
    std::stringstream ret;
...
std::function<std::string(const common::Peer&)> CommomPeer2Str =
    [](const common::Peer& peer) -> std::string {
    std::stringstream ret;
    ret << "id: " << peer.id() << " address: " << peer.address();
    return ret.str();
};

std::function<std::string(const common::PartitionInfo&)>
    CommomPartitionInfo2Str =
        [](const common::PartitionInfo& partition) -> std::string {
    std::stringstream ret;
...
std::function<std::string(const metaserver::copyset::CopysetStatusResponse&)>
...
std::function<std::string(const metaserver::copyset::COPYSET_OP_STATUS&)>
    CopysetOpStatus2Str =
