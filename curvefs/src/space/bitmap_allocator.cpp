...
inline bool HasAllocOffsetHint(const SpaceAllocateHint& hint) {
    return hint.leftOffset != hint.INVALID_OFFSET ||
           hint.rightOffset != hint.INVALID_OFFSET;
}

inline bool IsAllocTypeBig(const SpaceAllocateHint& hint) {
...
uint64_t BitmapAllocator::Alloc(uint64_t size, const SpaceAllocateHint& hint,
    ...
    } else if (HasAllocOffsetHint(hint)) {
