...
FSStatusCode FsInfoWrapper::DeleteMountPoint(const std::string& mp) {
    ...
    if (iter != fsInfo_.mountpoints().end()) {
...
void FsInfoWrapper::MergeMountPoints(const FsInfoWrapper& another) {
    if (another.fsInfo_.mountpoints_size() == 0) {
        return;
    } else if (fsInfo_.mountpoints_size() == 0) {
        *fsInfo_.mutable_mountpoints() = another.fsInfo_.mountpoints();
        fsInfo_.set_mountnum(fsInfo_.mountpoints_size());
        return;
    } else {
        fsInfo_.mutable_mountpoints()->MergeFrom(another.fsInfo_.mountpoints());
        auto* mps = fsInfo_.mutable_mountpoints();
        std::sort(mps->begin(), mps->end());
        auto iter = std::unique(mps->begin(), mps->end());
        mps->erase(iter, mps->end());
        fsInfo_.set_mountnum(mps->size());
    }
}

void FsInfoWrapper::Merge(const FsInfoWrapper& another) {
    MergeMountPoints(another);
}
