...
class AbstractVolumeSpace {
 public:
    ...
    virtual bool UpdateDeallocatableBlockGroup(
        uint32_t metaserverId, const DeallocatableBlockGroupVec &groups,
        const BlockGroupDeallcateStatusMap &stats, uint64_t *issue) = 0;
};
...
class VolumeSpace final : public AbstractVolumeSpace {
 public:
    ...
    bool UpdateDeallocatableBlockGroup(
        uint32_t metaserverId, const DeallocatableBlockGroupVec &groups,
 ...
 private:
    ...
    // TODO(@ilixiaocui): may need use thread pool, if there are many volumes
