...
#ifndef CURVEFS_SRC_VOLUME_BITMAP_ALLOCATOR_H_
#define CURVEFS_SRC_VOLUME_BITMAP_ALLOCATOR_H_
...
class BitmapAllocator : public Allocator {
 public:
    ...
    uint64_t Alloc(const uint64_t size,
                   ...
                   std::vector<Extent>* exts) override;

    bool DeAlloc(const uint64_t off, const uint64_t len) override;

    bool DeAlloc(const std::vector<Extent>& exts) override;

    uint64_t StartOffset() const { return opt_.startOffset; }

    uint64_t Total() const override { return opt_.length; }
    ...
    bool MarkUsed(const std::vector<Extent>& extents) override;

    bool MarkUsable(const std::vector<Extent>& extents) override;
 ...
 private:
     ...
     */
    uint64_t AllocInternal(const AllocOrder& order,
                           const uint64_t size,
                           const AllocateHint& hint,
                           std::vector<Extent>* exts);

    // allocate staffs
    uint64_t AllocFromBitmap(const uint64_t size,
                             const AllocateHint& hint,
                             std::vector<Extent>* exts);
...
#endif  // CURVEFS_SRC_VOLUME_BITMAP_ALLOCATOR_H_
