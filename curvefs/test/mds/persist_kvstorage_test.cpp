...
TEST_F(PersistKVStorageTest, TestGetAndExist) {
...

    {
        ...
        EXPECT_EQ(1, wrapper.FsId());
        EXPECT_EQ("hello", wrapper.FsName());

        EXPECT_EQ(FSStatusCode::OK, storage.Get("world", &wrapper));
        EXPECT_EQ(2, wrapper.FsId());
        EXPECT_EQ("world", wrapper.FsName());
    }
}
...
TEST_F(PersistKVStorageTest, TestInsert) {
    // fs already exists
    {
        ...
        EXPECT_TRUE(storage.Exist(wrapper.FsName()));
    }
    ...
    // kvstorage error
    {
        ...
        EXPECT_FALSE(storage.Exist(newfsWrapper.FsName()));
    }
    ...
    // kvstorage persist ok
    {
        ...
        EXPECT_EQ(100, newfsWrapper.FsId());
        EXPECT_EQ("bar", newfsWrapper.FsName());
    }
}
