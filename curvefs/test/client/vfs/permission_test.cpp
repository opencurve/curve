...
TEST_F(PermissionTest, GetCurrentUid) {
    ...
    ASSERT_EQ(perm->GetCurrentUid(), 0);
    ...
    ASSERT_EQ(perm->GetCurrentUid(), 1000);
}

TEST_F(PermissionTest, GetCurrentGid) {
    auto builder = PermissionBuilder();

    // CASE 1: gid (0)
    auto perm = builder.SetOption([&](UserPermissionOption* option) {
        option->gids = std::vector<uint32_t> { 0, 1000 };
    }).Build();
    ...
    // CASE 2: gid (1000)
    perm = builder.SetOption([&](UserPermissionOption* option) {
        option->gids = std::vector<uint32_t> { 1000, 0 };
    }).Build();

    ASSERT_EQ(perm->GetCurrentGid(), 1000);
}

TEST_F(PermissionTest, GetCurrentUmask) {
    auto builder = PermissionBuilder();
    ...
    ASSERT_EQ(perm->GetCurrentUmask(), 0022);
    ...
    ASSERT_EQ(perm->GetCurrentUmask(), 0);
}
