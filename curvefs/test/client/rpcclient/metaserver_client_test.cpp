    ...
    EXPECT_CALL(mockMetaServerService_, GetOrModifyS3ChunkInfo(_, _, _, _))
        .WillOnce(Invoke([&](
            ...
            auto defer = absl::MakeCleanup([&]() {
                streamSender_->Close(connection);
            });
            {
                brpc::ClosureGuard doneGuard(done);
                brpc::Controller* cntl =
                    static_cast<brpc::Controller*>(controller);
                connection = streamSender_->Open(cntl, nullptr,
                                                 StreamOptions());
                if (nullptr == connection) {
                    LOG(ERROR) << "Open connnection for stream sender failed";
                    return;
                }
                streamId_ = connection->GetStreamId();
                response->set_appliedindex(applyIndex);
                response->set_statuscode(curvefs::metaserver::OK);
            }
