...
TEST_F(SpaceAllocServiceTest, TestStatSpace) {
    // test failure
    {
        ...
        EXPECT_CALL(space_, StatSpace(_, _))
            .WillOnce(Return(SPACE_NOT_FOUND));
    ...
    // test success
    {
        ...
        SpaceStat stat(100 * kGiB, 50 * kGiB, 4 * kKiB);
        EXPECT_CALL(space_, StatSpace(_, _))
            .WillOnce(DoAll(SetArgPointee<1>(stat), Return(SPACE_OK)));
...
TEST_F(SpaceAllocServiceTest, TestAllocateSpace) {
    // test failure
    {
        ...
        EXPECT_CALL(space_, AllocateSpace(_, _, _, _))
            .WillOnce(Return(SPACE_NO_SPACE));
        ...
        EXPECT_FALSE(cntl.Failed());
        EXPECT_EQ(SPACE_NO_SPACE, response.status());
    }
