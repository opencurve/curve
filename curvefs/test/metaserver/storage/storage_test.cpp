...
void TestHGet(std::shared_ptr<KVStorage> kvStorage) {
    ...
    s = kvStorage->HSet("partition:1", "1:1", "2:2");
    ASSERT_TRUE(s.ok());
    s = kvStorage->HGet("partition:1", "1:1", &value);
    ASSERT_TRUE(s.ok());
    ASSERT_EQ(value, "2:2");
...
void TestHSet(std::shared_ptr<KVStorage> kvStorage) {
    ...
    s = kvStorage->HSet("partition:1", "", "value1");
    ASSERT_TRUE(s.ok());
    ...
    s = kvStorage->HSet("partition:1", "key1", "");
    ASSERT_TRUE(s.ok());
    s = kvStorage->HGet("partition:1", "key1", &value);
    ASSERT_TRUE(s.ok());
    ASSERT_EQ(value, "");
...
void TestHSize(std::shared_ptr<KVStorage> kvStorage) {
    ...
    // CASE 5: big size
    for (int i = 0; i < 100; i++) {
        s = kvStorage->HSet("partition:1", "key" + std::to_string(i), "value");
        ASSERT_TRUE(s.ok());
    }
    size = kvStorage->HSize("partition:1");
    ASSERT_EQ(size, 100);
...
void TestSGet(std::shared_ptr<KVStorage> kvStorage) {
    ...
    s = kvStorage->SSet("partition:1", "1:1", "2:2");
    ASSERT_TRUE(s.ok());
    s = kvStorage->SGet("partition:1", "1:1", &value);
    ASSERT_TRUE(s.ok());
    ASSERT_EQ(value, "2:2");
...
void TestSSet(std::shared_ptr<KVStorage> kvStorage) {
    ...
    // CASE 3: set empty key
    s = kvStorage->SSet("partition:1", "", "value1");
    ASSERT_TRUE(s.ok());
    ...
    // CASE 4: set empty value
    s = kvStorage->SSet("partition:1", "key1", "");
    ASSERT_TRUE(s.ok());
    s = kvStorage->SGet("partition:1", "key1", &value);
    ASSERT_TRUE(s.ok());
    ASSERT_EQ(value, "");
...
void TestSSeek(std::shared_ptr<KVStorage> kvStorage) {
    ...
    for (iterator->SeekToFirst();
        ...
        ASSERT_EQ(iterator->Value(), expect[size].second);
        size++;
    }
    ASSERT_EQ(size, 3);
...
void TestSGetAll(std::shared_ptr<KVStorage> kvStorage) {
    ...
    for (iterator->SeekToFirst(); iterator->Valid(); iterator->Next()) {
        ...
        ASSERT_EQ(iterator->Value(), "value" + std::to_string(size));
    }
    ASSERT_EQ(size, 2);
    ASSERT_EQ(iterator->Size(), 2);
...
void TestSSize(std::shared_ptr<KVStorage> kvStorage) {
    ...
    // CASE 5: big size
    for (int i = 0; i < 100; i++) {
        s = kvStorage->SSet("partition:1", "key" + std::to_string(i), "value");
        ASSERT_TRUE(s.ok());
    }
    size = kvStorage->SSize("partition:1");
    ASSERT_EQ(size, 100);
