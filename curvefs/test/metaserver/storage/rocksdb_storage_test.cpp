...
TEST_F(RocksDBStorageTest, OpenCloseTest) {
    ...
    s = kvStorage_->HSet("partition:1", "key1", "value1");
    ...
    s = kvStorage_->SSet("partition:1", "key1", "value1");
    ASSERT_TRUE(s.IsDBClosed());
...
TEST_F(RocksDBStorageTest, GetStatisticsTest) {
    ...
    // CASE 2: get statistics failed
    /*
    auto options = options_;
    options.DataDir = "/__notfound__";
    kvStorage_ = std::make_shared<RocksDBStorage>(options);
    ASSERT_FALSE(kvStorage_->GetStatistics(&statistics));
    */
}

TEST_F(RocksDBStorageTest, ComparatorTest) {
    RocksDBStorageComparator cmp;
    std::string str4num1 = EncodeNumber(1);
    std::string str4num2 = EncodeNumber(2);
