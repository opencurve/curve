...
public class CurveFSInputStream extends FSInputStream {
  ...
  public CurveFSInputStream(Configuration conf, CurveFSProto curvefs,
    ...
    LOG.debug(
        "CurveInputStream constructor: initializing stream with fh " + fh
  ...
  public synchronized void seek(long targetPos) throws IOException {
    LOG.trace(
        "CurveInputStream.seek: Seeking to position " + targetPos + " on fd "
        + fileHandle);
    if (targetPos > fileLength) {
      throw new IOException(
          "CurveInputStream.seek: failed seek to position " + targetPos
