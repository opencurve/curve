...
public class CurveFsMount extends CurveFsProto {
    // init
    private native long nativeCurveFsNew();
    private native void nativeCurveFsDelete(long instancePtr);
    ...
    private static native Dirent[] nativeCurveFsListDir(long instancePtr, String path);
    // file*
    private static native int nativeCurveFsCreate(long instancePtr, String path, int mode, File file);
    private static native int nativeCurveFsOpen(long instancePtr, String path, int flags, File file);
    private static native long nativeCurveFsLSeek(long instancePtr, int fd, long offset, int whence);
    private static native int nativieCurveFsRead(long instancePtr, int fd, long offset, byte[] buffer, long size);
    private static native int nativieCurveFsWrite(long instancePtr, int fd, long offset, byte[] buffer, long size);
    ...
    // Flags for open(): must be synchronized with JNI if changed.
    ...
    // Whence flags for seek(): must be synchronized with JNI if changed.
    public static final int SEEK_SET = 0;
    public static final int SEEK_CUR = 1;
    public static final int SEEK_END = 2;

    // Attribute flags for setattr(): must be synchronized with JNI if changed.
    ...
    public static final String REGEX_CURVEFS_CONF = "^" + PREFIX_KEY + "\\..*";
    ...
    static {
        try {
            ...
            System.out.println("Load curvefs native library failed: " + e.getMessage());
        }
    }
    ...
    private void setConf(Configuration conf) throws IOException {
        ...
        for (Map.Entry<String,String> entry : m.entrySet()) {
            ...
            nativeCurveFsConfSet(instancePtr, key.substring(PREFIX_KEY.length() + 1), value);
        }
    ...
    public void shutdown() throws IOException {
        ...
        nativeCurveFsDelete(instancePtr);
        initialized = false;
    }
    ...
    @Override
    public Dirent[] listdir(String path) throws IOException {
        return nativeCurveFsListDir(instancePtr, path);
    }

    @Override
    public void create(String path, int mode, File file) throws IOException {
        nativeCurveFsCreate(instancePtr, path, mode, file);
    }

    public void open(String path, int flags, File file) throws IOException {
        nativeCurveFsOpen(instancePtr, path, flags, file);
    }
    ...
    @Override
    public int read(int fd, long offset, byte[] buffer, long size) throws IOException {
        return nativieCurveFsRead(instancePtr, fd, offset, buffer, size);
    }

    @Override
    public int write(int fd, long offset, byte[] buffer, long size) throws IOException {
