...
/*
TEST(CloseQemuDetachedVolumes, CloseQemuDetachedVolumes_6)
{
    int fd = 6;
    char* filename =
const_cast<char*>("rbd:rbd/volume01:auth_supported=none:mon_host=10.182.30.27:6789");
    FdImage_t fd_image;
    fd_image.filename = filename;
    ...
    EXPECT_CALL(mock, GetUuidLockfile()).WillOnce(testing::Return(lock_file));
    EXPECT_CALL(mock, LockFile(::testing::_)).WillOnce(testing::Return(0));
    EXPECT_CALL(mock, UnLockFile(::testing::_)).WillOnce(testing::Return());
    EXPECT_CALL(mock, CloseImage(::testing::_)).WillOnce(testing::Return(0));
    EXPECT_CALL(mock, RmFd(::testing::_)).WillOnce(testing::Return(0));

    EXPECT_EQ(0, CloseQemuDetachedVolumes(0));
}
*/
TEST(CloseQemuDetachedVolumes,
     CloseQemuDetachedVolumes_7) {
    int fd = 6;
    char* filename = const_cast<char*>(
        "rbd:rbd/volume01:auth_supported=none:mon_host=10.182.30.27:6789");
    FdImage_t fd_image;
