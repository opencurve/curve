...
struct HeartbeatManagerOption {
    ...
    // Heartbeat timeout detection thread detection interval (duration:
    // milliseconds)
...
struct NebdClientInfo {
    NebdClientInfo(int pid2, const std::string& version2, uint64_t timeStamp2)
...
class HeartbeatManager {
 public:
    ...
    virtual int Fini();
    // After receiving the heartbeat, part2 will update the timestamp of the
    // files included in the heartbeat recorded in memory through this interface
    // The heartbeat detection thread will determine whether the file needs to
    // be closed based on this timestamp
    virtual bool UpdateFileTimestamp(int fd, uint64_t timestamp);
    // After receiving the heartbeat, part2 will update the timestamp of part1
 ...
 private:
    // The current running status of heartbeatmanager, where true indicates
    // running and false indicates not running
