...
int NebdFileEntity::Close(bool removeMeta) {
    CHECK(executor_ != nullptr) << "file entity is not inited. "
                                << "filename: " << fileName_;
    // This is used to prevent conflicts with other user requests to ensure that a file is not closed, and requests sent to the backend after the file has been closed result in failures.
    WriteLockGuard writeLock(rwLock_);
    // The mutex lock here is to prevent conflicts with open requests. The following scenarios may lead to concurrent close and open operations:
    // part2 restarts, causing the file to be reopened. Due to a timeout, the file is about to be closed.
    // At this point, a user sends a request to mount a volume, which involves opening the file.
    std::unique_lock<bthread::Mutex> lock(fileStatusMtx_);
    if (status_ == NebdFileStatus::OPENED) {
