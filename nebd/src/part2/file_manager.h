...
class NebdFileManager {
 public:
     ...
     * @param removeRecord: Do you want to remove the file record? True means
     * remove, false means not remove If it is a close request passed from
    ...
    // If entity already exists, return entity pointer; otherwise, return
    // nullptr
 ...
 private:
    ...
    // If the entity does not exist, create a new entity, insert a map, and then
    // return
    ...
    // If the entity does not exist, generate a new entity, insert a map, and
    // then return
 ...
 private:
    // The current running status of the filemanager, where true indicates
