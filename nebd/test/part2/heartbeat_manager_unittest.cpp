...
TEST_F(HeartbeatManagerTest, CheckTimeoutTest) {
    ...
    EXPECT_CALL(*entity3, GetFileTimeStamp()).WillRepeatedly(Return(curTime));
    ...
    // Expected results
    EXPECT_CALL(*entity1, Close(false)).Times(AtLeast(1));
    EXPECT_CALL(*entity2, Close(false)).Times(0);
    EXPECT_CALL(*entity3, Close(false)).Times(0);
...
TEST_F(HeartbeatManagerTest, UpdateTimeStampTest) {
    ...
    EXPECT_CALL(*fileManager_, GetFileEntity(1)).WillOnce(Return(entity));
    EXPECT_CALL(*entity, UpdateFileTimeStamp(100)).Times(1);
    ASSERT_TRUE(heartbeatManager_->UpdateFileTimestamp(1, 100));

    EXPECT_CALL(*fileManager_, GetFileEntity(1)).WillOnce(Return(nullptr));
