...
typedef struct C_UserInfo {
    ...
    char owner[NAME_MAX_SIZE];
    // When owner="root", password needs to be provided as the key for
    // calculating the signature password information needs to end with '\0'
 ...
 * @return: Success returns 0, failure returns less than 0, and there may be
 * multiple possibilities, such as internal errors or the file already exists
 */
 ...
 * @return: Successfully returned the read length, otherwise
 * -LIBCURVE_ERROR::FAILED, etc
 */
 ...
 * @return: Successfully returned the write length, otherwise -
 * LIBCURVE_ERROR::FAILED, etc
 */
 ...
 * @param: fd is the file descriptor returned by the current open
 * @param: aioctx is an asynchronous read/write IO context that stores basic IO
 * information
...
/**
 * Forced deletion of files, unlink deletion of files on the mds side is not a
 * true deletion, Instead, it was placed in the garbage collection bin, and when
 * deleted using the DeleteForce interface, it was directly deleted
 * @param: userinfo is the user information
 * @param: filename The file name to be deleted
 * @return: Successfully returned 0,
 *          Otherwise, it may return to -LIBCURVE_ERROR::FAILED,
 ...
 * @return: Successfully returned a non empty DirInfo_ Pointer t, otherwise
 * return a null pointer
 */
DirInfo_t* OpenDir(const char* dirpath, const C_UserInfo_t* userinfo);

/**
 * Enumerate directory contents, only after the user OpenDir is successful can
 * they be listed
 * @param[in][out]: dirinfo is the pointer returned by OpenDir, which internally
 * places the information returned by mds into the substructures
 * @return: Successfully returned 0,
 *          Otherwise, it may return to -LIBCURVE_ERROR::FAILED,
 * -LIBCURVE_ERROR::AUTHFAILED, etc
 */
 ...
 * @param: filename File name
 * @param: userinfo is the user information
 * @param: finfo is an output parameter that carries the basic information of
 * the current file
 * @return: Successfully returned 0,
 *          Otherwise, it may return to -LIBCURVE_ERROR::FAILED,
 * -LIBCURVE_ERROR::AUTHFAILED, etc
 */
int StatFile(const char* filename, const C_UserInfo_t* userinfo,
             FileStatInfo* finfo);
 ...
 * @param: userinfo The user information for performing this operation, only the
 * root user can perform changes
 * @return: Successfully returned 0,
 *          Otherwise, return to -LIBCURVE_ERROR::FAILED,
 * -LIBCURVE_ERROR::AUTHFAILED, etc
 */
...
typedef struct UserInfo {
    ...
    std::string owner;
    // When owner=root, password needs to be provided as the key for calculating
    // the signature
    std::string password;
...
class CurveClient {
 public:
     ...
     */
    virtual int Open(const std::string& filename, const OpenFlags& openflags);
     ...
     * @return successfully returns fd, failure returns -1
     */
    virtual int ReOpen(const std::string& filename, const OpenFlags& openflags);
     ...
     * Extension file
     * @param filename File name, format: File name_ User name_
     * @param newsize The expanded size
     * @return returns an error code
     */
    virtual int Extend(const std::string& filename, int64_t newsize);
