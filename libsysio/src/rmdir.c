...
_sysio_p_rmdir(struct pnode *pno)
{
	...
	if (err)
		return err;
	/*
	 * Don't allow unlink of a root or a mount point.
	 */
	if (pno->p_cover || pno->p_mount->mnt_root == pno)
		return -EBUSY;
	/*
...
SYSIO_INTERFACE_NAME(rmdir)(const char *path)
{
	...
	SYSIO_INTERFACE_ENTER(rmdir, "%s", path);
	do {
	...
	SYSIO_INTERFACE_RETURN(err ? -1 : 0, err, rmdir, "%d", 0);
}
