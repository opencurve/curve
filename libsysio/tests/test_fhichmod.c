 ...
 * Link files.
 *
 * Usage: test_fhichmod <path> <perms>
 */
...
static int chmodit(const char *path, const char *s);
...
main(int argc, char *const argv[])
{
	...
	n = chmodit(argv[optind], argv[optind + 1]);
...
static int
chmodit(const char *path, const char *s)
{
	long	perms;
	char	*cp;
	...
	int	err;
	struct file_handle_info_sattr setattr;

	perms = strtol(s, &cp, 0);
	if (*s == '\0' || *cp != '\0' || perms < 0 || perms > 0xffff) {
		(void )fprintf(stderr, "Invalid permissions\n");
		return -1;
	}

	err = _test_fhi_find(&root_handle, path, &handle);
	if (err) {
		perror(path);
		return -1;
	}
	FHISATTR_CLEAR(&setattr);
	FHISATTR_SET_MODE(&setattr, (mode_t )perms);
	err = SYSIO_INTERFACE_NAME(fhi_setattr)(&handle, &setattr);
	if (err) {
		perror("fhi_chmod");
		return -1;
	}
