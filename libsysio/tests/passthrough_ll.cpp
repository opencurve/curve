...
extern int curvefs_reply_entry(fuse_req_t req, const struct fuse_entry_param *e);
extern int curvefs_reply_err(fuse_req_t req, int err);
extern int curvefs_reply_open(fuse_req_t req, const struct fuse_file_info *fi);
extern int curvefs_reply_attr(fuse_req_t req, const struct stat *attr, double attr_timeout);
extern int curvefs_reply_write(fuse_req_t req, size_t count);
extern int curvefs_reply_data(fuse_req_t req, struct fuse_bufvec *bufv, enum fuse_buf_copy_flags flags);
extern int curvefs_fill_reply_data(struct fuse_bufvec *bufv);
extern int curvefs_reply_read(fuse_req_t req, size_t count);
...
{
	if (ino == FUSE_ROOT_ID) {
		CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "lo_inode fd=%d se=%p req=%p\n", lo_data(req)->root.fd, req->se, req);
			     ...
			     struct fuse_file_info *fi)
{
	...
	curvefs_reply_attr(req, &buf, lo->timeout);
#if 0
...
static struct lo_inode *lo_find(struct lo_data *lo, struct stat *st)
{
	...
	for (p = lo->root.next; p != &lo->root; p = p->next) {
		CURVEFS_FUSE_PRINTF (FUSE_LOG_ERR, "in lo_find\n");
			 ...
			 struct fuse_entry_param *e)
{
	...
	if (NULL == name) {
		...
		res = fstatat(lo_fd(req, parent), "", &e->attr, AT_EMPTY_PATH | AT_SYMLINK_NOFOLLOW);
		...
		fuse_log(FUSE_LOG_ERR, "lo_do_lookup inode=%p\n", inode);
		if (inode) {
	...
	CURVEFS_FUSE_PRINTF (FUSE_LOG_ERR, "lo_do_lookup newfd=%d\n", newfd);
	...
	if (inode) {
		...
		CURVEFS_FUSE_PRINTF (FUSE_LOG_ERR, "lo_do_lookup inode=%p mode=%d\n", inode, e->attr.st_mode);
...
static void lo_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
	...
	CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "lo_lookup(parent=%" PRIu64 ", name=%s, root fd=%d)\n", parent, name, lo_data(req)->root.fd);
	...
	CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "lo_do_lookup err=%d\n", err);
    gettimeofday(&end, NULL);
    time = end.tv_sec + (end.tv_usec/1000000.0) - start.tv_sec - (start.tv_usec/1000000.0);
	CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs lo_lookup cost %lf seconds\n", time);

	if (err)
		curvefs_reply_err(req, err);
#if 0
		fuse_reply_err(req, err);
#endif
	else
		curvefs_reply_entry(req, &e);
#if 0
			     ...
			     const char *link)
{
	...
	gettimeofday(&end, NULL);
    time = end.tv_sec + (end.tv_usec/1000000.0) - start.tv_sec - (start.tv_usec/1000000.0);
	CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs lo_mknod_symlink cost %lf seconds\n", time);
	...
	while (1) {
		...
		if (plus) {
			...
			struct stat st = {
				...
				.st_mode = (mode_t) d->entry->d_type << 12,
			};
...
static void lo_open(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi)
{
	...
	if (fd == -1)
		return (void) curvefs_reply_err(req, errno);
#if 0
		return (void) fuse_reply_err(req, errno);
#endif
...
{
	CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "coming into lo_release\n");
	(void) ino;

	close(fi->fh);

	curvefs_reply_err(req, 0);
#if 0	
	fuse_reply_err(req, 0);
#endif
}
		    ...
		    off_t offset, struct fuse_file_info *fi)
{
	...
	struct fuse_bufvec dstbuf = FUSE_BUFVEC_INIT(size);
	curvefs_fill_reply_data(&dstbuf);

	res = fuse_buf_copy(&dstbuf, &buf, (fuse_buf_copy_flags) 0);
	if (res < 0)
		curvefs_reply_err(req, -res);
	else
		curvefs_reply_read(req, (size_t) res);

#if 0
curvefs_reply_data(req, &buf, FUSE_BUF_SPLICE_MOVE);
...
static void lo_write_buf(fuse_req_t req, fuse_ino_t ino,
			 ...
			 struct fuse_file_info *fi)
{
	CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "coming into lo_write_buf\n");
	...
	gettimeofday(&end, NULL);
    time = end.tv_sec + (end.tv_usec/1000000.0) - start.tv_sec - (start.tv_usec/1000000.0);
	CURVEFS_FUSE_PRINTF (FUSE_LOG_ERR, "curvefs lo_write_buf cost %lf seconds res=%ld\n", time, res);
	...
	if(res < 0)
		...
		curvefs_reply_write(req, (size_t) res);
#if 0
...
const struct fuse_lowlevel_ops lo_oper = {
	.init		= lo_init,
	.destroy	= lo_destroy,
	.lookup		= lo_lookup,
	.forget		= lo_forget,
