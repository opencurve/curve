 ...
 * Create files.
 *
 * Usage: test_fhicreate [export-path] [path...]
 *
...
static int createit(const char *path);
...
createit(const char *path)
{
	...
	err = SYSIO_INTERFACE_NAME(fhi_create)(&where, 0644);
	if (err) {
		perror(path);
		return -1;
	}
