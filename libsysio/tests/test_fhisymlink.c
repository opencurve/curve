 ...
 * Link files.
 *
 * Usage: test_fhisymlink <old-path> <new-path>
 */
...
static int symlinkit(const char *from, const char *to);
...
main(int argc, char *const argv[])
{
	...
	n = symlinkit(argv[optind], argv[optind + 1]);
...
static int
symlinkit(const char *from, const char *to)
{
	...
	int	err;
	struct file_handle_info_dirop_args arg;
	...
	arg.fhida_path = to;
	arg.fhida_dir = &root_handle;
	err = SYSIO_INTERFACE_NAME(fhi_symlink)(from, &arg);
	if (err) {
		perror("Can't symlink");
		return -1;
	}
	err = _test_fhi_find(&root_handle, from, &handle);
	if (err) {
		perror(to);
		return -1;
	}
