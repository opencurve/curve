...
iox_server_t _g_server = NULL;

#ifdef CURVEFS_SERVER_CO_POOL

#define CURVEFS_CO_POOL_SIZE    32

stCoRoutine_t* __g_co_pool[CURVEFS_CO_POOL_SIZE];
bool __g_co_alloc[CURVEFS_CO_POOL_SIZE];
CO_FUNC_t __g_co_func[CURVEFS_CO_POOL_SIZE];
void* __g_co_args[CURVEFS_CO_POOL_SIZE];
int __g_co_idx[CURVEFS_CO_POOL_SIZE];

void* co_pool_thread (void* args) {
    int idx = *((int *)args);

    //printf ("co_pool_thread the idx=%d\n", idx);

    for (; ;) {
        if (__g_co_alloc[idx]) {
            (*__g_co_func[idx]) (__g_co_args[idx]);
            
            __g_co_func[idx] = NULL;
            __g_co_args[idx] = NULL;
            __g_co_alloc[idx] = false;

            co_yield(__g_co_pool[idx]);
        }
    }
...
void co_pool_create(stCoRoutine_t** co, CO_FUNC_t func, void* args) {
    int i;

    for (i=0; i<CURVEFS_CO_POOL_SIZE; i++) {
        if (false == __g_co_alloc[i]) {
            __g_co_alloc[i] = true;
            __g_co_func[i] = func;
            __g_co_args[i] = args;
            break;
    ...
    *co = __g_co_pool[i];
//    printf ("co_pool_create the idx=%d co=%p\n", i, __g_co_pool[i]);
//    co_resume(__g_co_pool[i]);
...
void init_co_pools() {
    ...
    for (i=0; i<CURVEFS_CO_POOL_SIZE; i++) {
        co_create(& (__g_co_pool[i]), NULL, co_pool_thread, (void *) (&__g_co_idx[i]));
        //printf ("init_co_pools idx=%d co=%p\n", i, __g_co_pool[i]);
    }
...
static void* curvefs_process_syscall(void* args) {
    char* mem_req = NULL;
    int reqsize = 0;
    int rv = 0;
    ...
    reqsize = getReqSize(&mem_req);
    enum func_no fno = getFuncNo(&mem_req);
    CURVEFS_FUSE_PRINTF (FUSE_LOG_ERR, "curvefs_process_syscall reqsize=%d fno=%d\n", reqsize, fno);
    switch(fno) {
    case FUNC_NO_ACCESS:
    {
        CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_ACCESS into\n");
        gettimeofday(&start, NULL);
        rv = convert_to_libsysio_access (mem_req);
        gettimeofday(&end, NULL);
        time = end.tv_sec + (end.tv_usec/1000000.0) - start.tv_sec - (start.tv_usec/1000000.0);
        printf("convert_to_libsysio_access access cost %lf seconds\n", time);

        CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_ACCESS res=%d\n", rv);
        rv = reply_libsysio_ret((char *)args, rv);
        if (-1 == rv) {
            CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_ACCESS failed\n");
        }
        break;
    }
    case FUNC_NO_OPEN:
    {
        CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_OPEN into\n");
        rv = convert_to_libsysio_open(mem_req);

        rv = reply_libsysio_ret((char*)args, rv);
        if (-1 == rv) {
            CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_OPEN failed\n");
        }
        break;
    }
    case FUNC_NO_FTRUNCATE:
    {
        CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_FTRUNCATE into\n");
        rv = convert_to_libsysio_ftruncate (mem_req);

        rv = reply_libsysio_ret ((char*)args, rv);
        if (-1 == rv) {
            CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_FTRUNCATE failed\n");
        }
        break;
    }
    case FUNC_NO_CLOSE:
    {
        CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_CLOSE into\n");
        rv = convert_to_libsysio_close (mem_req);

        rv = reply_libsysio_ret ((char*)args, rv);
        if (-1 == rv) {
            CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_CLOSE failed\n");
        }
        break;
    }
    case FUNC_NO_PWRITE:
    {
        CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_PWRITE into\n");
        gettimeofday(&start, NULL);
        ssize_t res = convert_to_libsysio_pwrite (mem_req);

        gettimeofday(&end, NULL);
        time = end.tv_sec + (end.tv_usec/1000000.0) - start.tv_sec - (start.tv_usec/1000000.0);
        CURVEFS_FUSE_PRINTF ("convert_to_libsysio_pwrite access cost %lf seconds\n", time);

        rv = reply_libsysio_ssize ((char*)args, res);
        if (-1 == rv) {
            CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_PWRITE failed\n");
        }
        break;
    }
    case FUNC_NO_FSYNC:
    {
        CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_FSYNC into\n");
        rv = convert_to_libsysio_fsync (mem_req);

        rv = reply_libsysio_ret ((char*)args, rv);
        if (-1 == rv) {
            CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_FSYNC failed\n");
        }
        break;
    }
    case FUNC_NO_STAT:
    {
        CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_STAT into\n");
        rv = convert_to_libsysio_stat (mem_req);

        rv = reply_libsysio_ret ((char*)args, rv);
        if (-1 == rv) {
            CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_STAT failed\n");
        }
        break;
    }
    case FUNC_NO_PREAD:
    {
        CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_PREAD into\n");
        ssize_t res = convert_to_libsysio_pread (mem_req);

        rv = reply_libsysio_ssize ((char*)args, res);
        if (-1 == rv) {
            CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_PREAD failed\n");
        }
        break;
    }
    case FUNC_NO_UNLINK:
    {
        CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_UNLINK into\n");
        rv = convert_to_libsysio_unlink (mem_req);

        rv = reply_libsysio_ret ((char*)args, rv);
        if (-1 == rv) {
            CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "curvefs_process_syscall FUNC_NO_UNLINK failed\n");
        }
        break;
    }
    default:
...
extern void fuse_request_loop();

/* just use the busy polling mode */
static void* vfs_cmd_test(void* args) {

    CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "vfs_cmd_test into\n");

    iox_server_storage_t serverStorage;
    _g_server = iox_server_init(&serverStorage, "CurveFS", "curvefs_client", "request", NULL);

    CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "vfs_cmd_test before runing\n");

#ifdef CURVEFS_CLIENT_POLL

    while (keepRunning) {
        //! [process request]
        char* request = NULL;
        while (ServerRequestResult_SUCCESS == iox_server_take_request(_g_server, (const void**)&request)) {
            CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "vfs_cmd_test take_request\n");

            stCoRoutine_t *co_req = NULL;
#ifdef CURVEFS_SERVER_CO_POOL
            co_pool_create(&co_req, curvefs_process_syscall, request);
#else
            co_create(&co_req, NULL, curvefs_process_syscall, request);
#endif
            co_resume(co_req);
        }

        //process fuse request loop
        fuse_request_loop ();
    ...
    //! [create waitset and attach server]
    iox_ws_t waitset = iox_ws_init(&waitsetStorage);
    if (WaitSetResult_SUCCESS != iox_ws_attach_server_state (waitset, _g_server, ServerState_HAS_REQUEST, 0U, NULL)) {
        CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "vfs_cmd_test iox_ws_attach_server_state failed exited\n");
        return NULL;
    }
    ...
    while (keepRunning) {
        ...
        uint64_t numberOfNotifications = iox_ws_timed_wait(waitset, timeout, notificationArray, NUMBER_OF_NOTIFICATIONS, &missedNotifications);
        for (uint64_t i = 0; i < numberOfNotifications; ++i) {
            if (iox_notification_info_does_originate_from_server (notificationArray[i], _g_server)) {
                //! [process request]
                char* request = NULL;
                while (ServerRequestResult_SUCCESS == iox_server_take_request(_g_server, (const void**)&request)) {
                    stCoRoutine_t *co_req = NULL;
        ...
        for (; (false == __g_fuse_msg->_mpsc_front->was_empty()) || (false == __g_vfsgate_msg->_mpsc_back->was_empty());) {
...
#ifdef CURVEFS_SERVER_CO_POOL
        co_pool_create(&co_inst, fuse_process_request, curReq);
#else
...
int main(int argc, char** argv) {
    ...
    iox_runtime_init("curvefs_server");

    CURVEFS_FUSE_PRINTF(FUSE_LOG_ERR, "main after iox_runtime_init\n");
