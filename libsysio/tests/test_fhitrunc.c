...
static int truncit(const char *path, const char *s);
...
main(int argc, char *const argv[])
{
	...
	n = truncit(argv[optind], argv[optind + 1]);
...
static int
truncit(const char *path, const char *s)
{
	long long fsiz;
	char	*cp;
	...
	if (*s == '\0' ||
	    *cp != '\0' ||
	    fsiz < 0 ||
	    (fsiz == LLONG_MAX && errno == ERANGE)) {
		(void )fprintf(stderr, "Invalid size\n");
		return -1;
	}
	...
	FHISATTR_SET_SIZE(&setattr, (off64_t )fsiz);
