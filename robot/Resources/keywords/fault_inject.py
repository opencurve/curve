...
def package_loss_all(ssh, dev, percent):
    ori_cmd = "sudo tc qdisc add dev %s root netem loss %d%% 2>&1" % (
        dev, percent)
...
def package_reorder_all(dev, ms, percent1, percent2):
    ori_cmd = "tc qdisc change dev %s root netem delay %s reorder %d%% %d%%" % (
        dev, ms, percent1, percent2)
    cmd = shell_operator.gen_remote_cmd(config.ssh_user, config.ssh_hostname, 1046, config.ssh_key, ori_cmd,
                                        sudo_flag=True, sudo_way="")
    print cmd
    # rc = shell_operator.run_exec(cmd)
...
def eth_down_for_a_monent(dev, time):
    ori_cmd = "ip link set %s down 2>&1 && sleep %d 2>&1 && ip link set %s up 2>&1" % (
        dev, time)
    cmd = shell_operator.gen_remote_cmd(config.ssh_user, config.ssh_hostname, 1046, config.ssh_key, ori_cmd,
                                        sudo_flag=True, sudo_way="")
    print cmd
...
def add_rate_limit(dev, downlink, uplink):
    ori_cmd = "wget -N -P /tmp nos.netease.com/nfit-software/taaslimit.sh 2>&1 && chmod a+rx /tmp/taaslimit.sh 2>&1 " \
              "&& mv /tmp/taaslimit.sh /sbin/taaslimit 2>&1 && chown root:root /sbin/taaslimit && taaslimit %s %d %d 2>&1" % (
                  dev, downlink, uplink)
    cmd = shell_operator.gen_remote_cmd(config.ssh_user, config.ssh_hostname, 1046, config.ssh_key, ori_cmd,
                                        sudo_flag=True, sudo_way="")
    print cmd
...
def stop_rwio():
    ssh = shell_operator.create_ssh_connect(
        config.client_list[0], 1046, config.abnormal_user)
...
def init_recover_disk(fio_size):
    ori_cmd = "sudo fio -name=/dev/nbd2 -direct=1 -iodepth=32 -rw=write -ioengine=libaio -bs=1024k -size=%dG -numjobs=1 -time_based" % int(
        fio_size)
    ssh = shell_operator.create_ssh_connect(
        config.client_list[0], 1046, config.abnormal_user)
    rs = shell_operator.ssh_exec(ssh, ori_cmd)
...
def recover_disk():
    ...
    md5 = test_curve_stability_nbd.get_vol_md5("recover")
    assert md5 == config.recover_vol_md5, "Data is inconsistent after translation,md5 is %s,recover md5 is %s" % (
        config.recover_vol_md5, md5)
...
def get_chunkserver_list():
    ...
    ssh = shell_operator.create_ssh_connect(
        client_host, 1046, config.abnormal_user)
    rs = shell_operator.ssh_exec(ssh, cmd)
...
def get_cs_copyset_num(host, cs_id):
    ...
    cmd = "curve_ops_tool check-chunkserver -chunkserverAddr=%s:%d |grep 'total copysets'" % (
        host, cs_number)
    ssh = shell_operator.create_ssh_connect(
        client_host, 1046, config.abnormal_user)
    rs = shell_operator.ssh_exec(ssh, cmd)
...
def init_vm():
    ssh = shell_operator.create_ssh_connect(
        config.nova_host, 1046, config.nova_user)
...
def attach_new_vol(fio_size, vdbench_size):
    ori_cmd = "bash curve_test.sh create %d %d" % (
        int(fio_size), int(vdbench_size))
    ssh = shell_operator.create_ssh_connect(
        config.nova_host, 1046, config.nova_user)
    rs = shell_operator.ssh_exec(ssh, ori_cmd)
    logger.info("exec cmd %s" % ori_cmd)
...
def clean_nbd():
    for client_ip in config.client_list:
        ...
        ssh = shell_operator.create_ssh_connect(
            client_ip, 1046, config.abnormal_user)
        rs = shell_operator.ssh_exec(ssh, cmd)
        if rs[1] != []:
...
def map_nbd():
    ...
    cmd = "curve create --filename /fiofile --length 10 --user test --stripeUnit %d  --stripeCount %d" % (
        random.choice(stripeUnit), random.choice(stripeCount))
    rs = shell_operator.ssh_exec(ssh, cmd)
    assert rs[3] == 0, "create /fiofile fail：%s" % rs[2]
    cmd = "curve create --filename /vdbenchfile --length 10 --user test --stripeUnit %d  --stripeCount %d" % (
        random.choice(stripeUnit), random.choice(stripeCount))
    rs = shell_operator.ssh_exec(ssh, cmd)
    assert rs[3] == 0, "create /vdbenchfile fail：%s" % rs[2]
    # test recover recyclebin file
    cmd = "curve create --filename /recover --length 10 --user test --stripeUnit %d  --stripeCount %d" % (
        random.choice(stripeUnit), random.choice(stripeCount))
    rs = shell_operator.ssh_exec(ssh, cmd)
...
def get_chunkserver_status(host):
    ...
    up_cs = [int(i.split()[0][11:])
             for i in filter(lambda x: "active" in x, chunkserver_lines)]
    down_cs = [int(i.split()[0][11:])
               for i in filter(lambda x: "down" in x, chunkserver_lines)]
...
def up_all_cs():
    ...
    for host in config.chunkserver_list:
        ssh = shell_operator.create_ssh_connect(
            host, 1046, config.abnormal_user)
        try:
            cs_status = get_chunkserver_status(host)
            down_cs = cs_status["down"]
            if down_cs == []:
...
#        logger.debug("exec %s"%ori_cmd)
    if csid == -1:
        ...
        if get_cs_copyset_num(host, csid) == 0:
            ori_cmd = "sudo rm -rf /data/chunkserver%d/chunkserver.dat" % (
                csid)
            rs = shell_operator.ssh_exec(ssh, ori_cmd)
            assert rs[3] == 0
...
def exec_deleteforce():
    ...
    cmd = "scp -i %s -o StrictHostKeyChecking=no -P 1046 robot/Resources/keywords/deleteforce-test.py %s:~/" % (
...
def check_chunkserver_online(num=120):
    ...
    if int(online_num[0]) != num:
        ...
        assert int(
            online_num[0]) == num, "chunkserver online num is %s" % online_num
...
def wait_cluster_healthy(limit_iops=8000):
    ...
    if check != 1:
        ...
        assert check == 1, "cluster is not healthy in %d s,cluster status is:\n %s,copysets status is:\n %s" % (
            config.recover_time, cluster_status, copysets_status)
    ...
    try:
        ...
        while i < 600:
            ...
            if rs[3] == 0:
                break
            logger.info(
                "check_hash false return is %s,return code is %d" % (rs[1], rs[3]))
            time.sleep(3)
            i = i + 3
        if rs[3] != 0:
        ...
        if rs[3] == 0:
            ...
            for i in range(0, 3):
                ...
                ssh = shell_operator.create_ssh_connect(
                    host, 1046, config.abnormal_user)
                ori_cmd = "sudo cp /data/%s/copysets/%s/data/chunk_%s /data/log/%s" % (
                    chunkserver, groupId, chunkID, chunkserver)
                rs = shell_operator.ssh_exec(ssh, ori_cmd)
                if rs[3] != 0:
                    logger.error("cp chunk fail,is %s" % rs[1])
            assert False, "checkconsistecny fail,error is %s" % (
                "".join(rs[1]).strip())
...
def check_data_consistency():
    try:
        ...
        if rs[1] != []:
            t = time.time()
            ori_cmd = "mv /home/nbs/output /home/nbs/vdbench-output/output-%d && mv /home/nbs/nohup.out /home/nbs/nohup-%d" % (
                int(t), int(t))
            rs = shell_operator.ssh_exec(ssh, ori_cmd)
            ori_cmd = "mkdir output && touch nohup.out"
            rs = shell_operator.ssh_exec(ssh, ori_cmd)
#            logger.error("find error in %s"%rs[1])
            assert False, "find data consistency error,save log to vm /root/vdbench-output/output-%d" % int(
                t)
    except Exception as e:
        ssh.close()
        raise
...
def test_kill_chunkserver_num(num):
    ...
    logger.info("|------begin test kill chunkserver num %d,host %s------|" %
                (num, chunkserver_host))
    try:
        ...
        if float(end_iops)/float(start_iops) < 0.9:
            raise Exception(
                "client io is slow, = %d more than 5s" % (end_iops))
    except Exception as e:
...
def test_start_chunkserver_num(num, host=None):
    ...
    logger.info("|------begin test start chunkserver num %d,host %s------|" %
                (num, chunkserver_host))
    try:
        start_mult_cs_process(chunkserver_host, num)
        end_iops = get_cluster_iops()
...
def test_outcs_recover_copyset():
    chunkserver_host = random.choice(config.chunkserver_list)
    logger.info("|------begin test out one chunkserver,host %s------|" %
                (chunkserver_host))
    try:
        ...
        if num != 0:
            #    assert num != 0
            raise Exception("host %s chunkserver %d not recover to 0 in %d,now is %d" % (
                chunkserver_host, cs_list[0], config.recover_time, num))
    except Exception as e:
        #        raise AssertionError()
...
def test_upcs_recover_copyset(host, copyset_num):
    if host == None:
        ...
        chunkserver_host = host
    logger.info("|------begin test up one chunkserver,host %s------|" %
                (chunkserver_host))
    try:
        ...
        while i < config.recover_time:
            ...
            if abs(num - copyset_num) <= 10:
                break
        if abs(num - copyset_num) > 10:
            logger.error("get host %s chunkserver %d copyset num is %d" %
                         (chunkserver_host, cs_list[0], num))
            raise Exception(
...
def stop_all_cs_not_recover():
    chunkserver_host = random.choice(config.chunkserver_list)
    logger.info("|------begin test stop all chunkserver,host %s------|" %
                (chunkserver_host))
    try:
        ...
        for cs in dict:
            ...
            if num != dict[cs]:
                #    assert num != 0
                raise Exception("stop all chunkserver not recover fail,cs id %d,copysets num from %d to %d" % (
                    cs, dict[cs], num))
    except Exception as e:
        #        raise AssertionError()
        logger.error("error is %s" % e)
...
def pendding_all_cs_recover():
    chunkserver_host = random.choice(config.chunkserver_list)
    logger.info(
        "|------begin test pendding all chunkserver,host %s------|" % (chunkserver_host))
    ssh = shell_operator.create_ssh_connect(
        chunkserver_host, 1046, config.abnormal_user)
    ssh_mds = shell_operator.create_ssh_connect(
        config.mds_list[0], 1046, config.abnormal_user)
    try:
        stop_host_cs_process(chunkserver_host)
        list = get_chunkserver_status(chunkserver_host)
        down_list = list["down"]
        ...
        for cs in down_list:
            ...
            assert rs[3] == 0, "pendding chunkserver %d fail,rs is %s" % (
                cs, rs)
        time.sleep(180)
        test_kill_mds(2)
        i = 0
        while i < config.recover_time:
            ...
            if num == 0:
                break
        if num != 0:
            logger.error("exist chunkserver %d copyset %d" %
                         (chunkserver_id, num))
...
def pendding_all_cs_recover_online():
    ...
    logger.info(
        "|------begin test pendding all chunkserver online,host %s------|" % (chunkserver_host))
    ssh = shell_operator.create_ssh_connect(
        chunkserver_host, 1046, config.abnormal_user)
    ssh_mds = shell_operator.create_ssh_connect(
        config.mds_list[0], 1046, config.abnormal_user)
...
def test_suspend_recover_copyset():
    chunkserver_host = random.choice(config.chunkserver_list)
    logger.info("|------begin test suspend recover,host %s------|" %
                (chunkserver_host))
    try:
        cs_list = kill_mult_cs_process(chunkserver_host, 1)
        begin_num = get_cs_copyset_num(chunkserver_host, cs_list[0])
        ...
        while i < config.recover_time:
            ...
            num = get_cs_copyset_num(chunkserver_host, cs_list[0])
            time.sleep(1)
            logger.info("now cs copyset num is %d,begin_num is %d" %
                        (num, begin_num))
...
def test_suspend_delete_recover_copyset():
    chunkserver_host = random.choice(config.chunkserver_list)
    logger.info(
        "|------begin test suspend delete recover,host %s------|" % (chunkserver_host))
    try:
        cs_list = kill_mult_cs_process(chunkserver_host, 1)
        begin_num = get_cs_copyset_num(chunkserver_host, cs_list[0])
        # time.sleep(config.recover_time)
...
def test_stop_chunkserver_host():
    start_iops = get_cluster_iops()
    chunkserver_host = random.choice(config.chunkserver_list)
    logger.info("|------begin test stop chunkserver host %s------|" %
                (chunkserver_host))
    try:
        stop_host_cs_process(chunkserver_host)
        end_iops = get_cluster_iops()
...
def test_restart_chunkserver_num(num):
    start_iops = get_cluster_iops()
    chunkserver_host = random.choice(config.chunkserver_list)
    logger.info("|------begin test restart chunkserver num %d,host %s------|" %
                (num, chunkserver_host))
    try:
...
def stop_scheduler():
    ssh = shell_operator.create_ssh_connect(
        config.mds_list[0], 1046, config.abnormal_user)
    for mds_host in config.mds_list:
        logger.info("|------begin stop copyset scheduler %s------|" %
                    (mds_host))
...
def test_kill_diff_host_chunkserver():
    ...
    logger.info("|------begin test kill diff host chunkserver,host1 %s,host2 %s------|" %
                (chunkserver_host1, chunkserver_host2))
    try:
...
def test_mds_loss_package(percent):
    ...
    ssh = shell_operator.create_ssh_connect(
        mds_host, 1046, config.abnormal_user)
...
def test_cs_delay_package(ms):
    ...
    ssh = shell_operator.create_ssh_connect(
        chunkserver_host, 1046, config.abnormal_user)
    dev = get_hostip_dev(ssh, chunkserver_host)
    logger.info("|------begin test host %s dev %s delay package------|" %
                (chunkserver_host, dev))
    try:
...
def test_chunkserver_cpu_stress(stress=50):
    chunkserver_host = random.choice(config.chunkserver_list)
    logger.info(
        "|------begin test chunkserver cpu stress,host %s------|" % (chunkserver_host))
...
def test_client_cpu_stress(stress=50):
    #    client_host = random.choice(config.client_list)
    client_host = config.client_list[0]
    logger.info("|------begin test client cpu stress,host %s------|" %
                (client_host))
    cmd = "scp -i %s -o StrictHostKeyChecking=no -P 1046 robot/Resources/keywords/cpu_stress.py \
...
def test_chunkserver_mem_stress(stress=50):
    chunkserver_host = random.choice(config.chunkserver_list)
    logger.info(
        "|------begin test chunkserver mem stress,host %s------|" % (chunkserver_host))
...
def test_client_mem_stress(stress=50):
    client_host = config.client_list[0]
    logger.info("|------begin test client mem stress,host %s------|" %
                (client_host))
    cmd = "free -g |grep Mem|awk \'{print $2}\'"
    ssh = shell_operator.create_ssh_connect(
        client_host, 1046, config.abnormal_user)
...
def test_chunkserver_network_stress():
    chunkserver_host = random.choice(config.chunkserver_list)
    logger.info(
        "|------begin test chunkserver network stress,host %s------|" % (chunkserver_host))
...
def test_mds_network_stress():
    mds_host = random.choice(config.mds_list)
    logger.info(
        "|------begin test mds network stress,host %s------|" % (mds_host))
...
def test_client_network_stress():
    client_host = config.client_list[0]
    logger.info(
        "|------begin test client network stress,host %s------|" % (client_host))
...
def test_ipmitool_restart_chunkserver():
    ...
    logger.info(
        "|------begin test chunkserver ipmitool cycle,host %s------|" % (chunkserver_host))
    ssh = shell_operator.create_ssh_connect(
        chunkserver_host, 1046, config.abnormal_user)
...
def test_ipmitool_restart_client():
    client_host = config.client_list[0]
    logger.info(
        "|------begin test client ipmitool cycle,host %s------|" % (client_host))
    ssh = shell_operator.create_ssh_connect(
        client_host, 1046, config.abnormal_user)
    ipmitool_cycle_restart_host(ssh)
    time.sleep(60)
...
def test_ipmitool_reset_chunkserver():
    chunkserver_host = random.choice(config.chunkserver_reset_list)
    logger.info(
        "|------begin test chunkserver ipmitool reset,host %s------|" % (chunkserver_host))
    ssh = shell_operator.create_ssh_connect(
        chunkserver_host, 1046, config.abnormal_user)
...
def test_ipmitool_restart_mds():
    ...
    logger.info(
        "|------begin test mds ipmitool cycle,host %s------|" % (mds_host))
    ssh = shell_operator.create_ssh_connect(
        mds_host, 1046, config.abnormal_user)
    ipmitool_cycle_restart_host(ssh)
    time.sleep(60)
...
def analysis_data(ssh):
    ...
    if randr_4k_iops < 75000:
        assert float(75000 - randr_4k_iops) / \
            75000 < 0.02, "4k_randr_iops did not meet expectations,expect more than 75000"
    if randw_4k_iops < 56000:
        assert float(56000 - randw_4k_iops) / \
            56000 < 0.02, "4k_randw_iops did not meet expectations,expect more than 56000"
    if read_512k_BW < 450:
        assert float(450 - read_512k_BW) / \
            450 < 0.02, "512k_read_bw did not meet expectations,expect more than 450"
    if write_512k_BW < 135:
        assert float(135 - write_512k_BW) / \
            135 < 0.02, "512k_write_bw did not meet expectations,expect more than 135"
...
def add_data_disk():
    ...
    for ip in vm_ip_list:
        ori_cmd = "ssh %s -o StrictHostKeyChecking=no " % ip + "\"" + \
...
def create_vm_image(vm_name):
    ssh = shell_operator.create_ssh_connect(
        config.nova_host, 1046, config.nova_user)
    ori_cmd = "source OPENRC && nova list |grep %s | awk '{print $2}'" % (
        vm_name)
    rs = shell_operator.ssh_exec(ssh, ori_cmd)
    logger.info("vm uuid is %s" % rs[1])
    thrash_vm_uuid = "".join(rs[1]).strip()
    ori_cmd = "source OPENRC && nova image-create %s image-%s" % (
        thrash_vm_uuid, vm_name)
    rs = shell_operator.ssh_exec(ssh, ori_cmd)
...
def get_all_curvevm_active_num(num):
    ...
    for uuid in rs[1]:
        ...
        assert status == "up", "get vm status fail,not up.is %s,current vm id is %s" % (
            status, uuid)
    return active_num
...
def do_thrasher(action):
    ...
    if type(action) is types.StringType:
        logger.debug(
            "Startup FailureXXXXXXXXXXXXXXXXXXX %s XXXXXXXXXXXXXXXXXXXXXXXXX" % action)
        globals()[action]()
    else:
        logger.debug("Startup FailureXXXXXXXXXXXXXXXXXXX %s,%s XXXXXXXXXXXXXXXXXXXXXX" % (
