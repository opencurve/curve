...
def add_config():
    ...
    for host in config.mds_list:
        ...
        # change offline time
        ori_cmd = R"sed -i 's/mds.heartbeat.offlinetimeoutMs=.*/mds.heartbeat.offlinetimeoutMs=%d/g' mds.conf" % (
            config.offline_timeout*1000)
        rs = shell_operator.ssh_exec(ssh, ori_cmd)
        assert rs[3] == 0, "change host %s mds config fail" % host
        # change clean_follower_afterMs  time
        ori_cmd = R"sed -i 's/mds.heartbeat.clean_follower_afterMs=.*/mds.heartbeat.clean_follower_afterMs=%d/g' mds.conf" % (
            300000)
        rs = shell_operator.ssh_exec(ssh, ori_cmd)
        assert rs[3] == 0, "change host %s mds config fail" % host
...
#        assert rs[3] == 0,"change host %s mds config fail"%host
        ...
        assert rs[3] == 0, "change host %s mds config fail" % host
        # add mysql conf
        ori_cmd = R"sed -i 's/mds.DbUrl=localhost/mds.DbUrl=%s/g' mds.conf" % (
            config.abnormal_db_host)
        rs = shell_operator.ssh_exec(ssh, ori_cmd)
        assert rs[3] == 0, "change host %s mds config fail" % host
        # add etcd conf
        ori_cmd = R"sed -i 's/mds.etcd.endpoint=127.0.0.1:2379/mds.etcd.endpoint=%s/g' mds.conf" % (
            etcd_addrs)
        rs = shell_operator.ssh_exec(ssh, ori_cmd)
        assert rs[3] == 0, "change host %s mds config fail" % host
    ...
    for host in config.client_list:
        ...
        cmd = "scp -i %s -o StrictHostKeyChecking=no -P 1046 conf/client.conf %s:~/" %\
            (config.pravie_key_path, host)
        shell_operator.run_exec2(cmd)
        ori_cmd = R"sed -i 's/mds.listen.addr=127.0.0.1:6666/mds.listen.addr=%s/g' client.conf" % (
            addrs)
        rs = shell_operator.ssh_exec(ssh, ori_cmd)
        assert rs[3] == 0, "change host %s client config fail" % host
        # Configure client.conf to py_client.conf(for the host) to facilitate client replication of deadlock issues
    ...
    for host in config.chunkserver_list:
        ...
        assert rs[3] == 0, "change host %s chunkserver config fail" % host
        # change mds ip
        ori_cmd = R"sed -i 's/mds.listen.addr=127.0.0.1:6666/mds.listen.addr=%s/g' chunkserver.conf" % (
            addrs)
        rs = shell_operator.ssh_exec(ssh, ori_cmd)
        assert rs[3] == 0, "change host %s chunkserver config fail" % host
    ...
    for host in config.chunkserver_list:
        ...
        cmd = "scp -i %s -o StrictHostKeyChecking=no -P 1046  client.conf conf/cs_client.conf %s:~/" %\
            (config.pravie_key_path, host)
        shell_operator.run_exec2(cmd)
        ori_cmd = R"sed -i 's/mds.listen.addr=127.0.0.1:6666/mds.listen.addr=%s/g' cs_client.conf" % (
            addrs)
        rs = shell_operator.ssh_exec(ssh, ori_cmd)
    ...
    for host in config.snap_server_list:
        ...
        # Modify snapshot_clone_server.conf etcd configuration
        ori_cmd = "sed -i \"s/etcd.endpoint=\S*/etcd.endpoint=%s/g\" snapshot_clone_server.conf" % (
            etcd_addrs)
        rs = shell_operator.ssh_exec(ssh, ori_cmd)
        assert rs[3] == 0, "change host %s snapshot config fail" % host
        # Modifying Database Configuration Items
        ori_cmd = R"sed -i 's/metastore.db_address=\S*/metastore.db_address=%s/g' snapshot_clone_server.conf" % (
            config.abnormal_db_host)
        rs = shell_operator.ssh_exec(ssh, ori_cmd)
...
# change snap_client.conf
        ori_cmd = "sed -i \"s/mds.listen.addr=\S*/mds.listen.addr=%s/g\" snap_client.conf" % (
            addrs)
        rs = shell_operator.ssh_exec(ssh, ori_cmd)
        assert rs[3] == 0, "change host %s snapshot config fail" % host
...
def destroy_curvefs():
    try:
        ...
        ssh = shell_operator.create_ssh_connect(
            test_client, 1046, config.abnormal_user)
        ...
        for mountpoint in config.fs_mount_dir:
            cmd = "/home/nbs/.curveadm/bin/curveadm umount %s%s" % (
                config.fs_mount_path, mountpoint)
...
def remk_test_dir():
    try:
        ...
        for test_dir in config.fs_mount_dir:
            ...
            rs = shell_operator.ssh_exec(ssh, ori_cmd)
            assert rs[3] == 0, "rm test dir %s fail,error is %s" % (
                test_dir, rs[1])
    ...
    try:
        ...
        if mountpoint == "":
            for mountpoint in config.fs_mount_dir:
                ...
                assert rs[3] == 0, "mount %s dir fail,error is %s" % (
                    mountpoint, rs[2])
        else:
            if mountfile == "":
        ...
        if mountpoint == "":
            for mountpoint in config.fs_mount_dir:
                cmd = "/home/nbs/.curveadm/bin/curveadm umount %s%s" % (
                    config.fs_mount_path, mountpoint)
                rs = shell_operator.ssh_exec(ssh, cmd)
                assert rs[3] == 0, "umount %s dir fail,error is %s" % (
                    mountpoint, rs[2])
        else:
            cmd = "/home/nbs/.curveadm/bin/curveadm umount %s%s" % (
                config.fs_mount_path, mountpoint)
...
def install_deb():
    try:
        ...
        for host in config.mds_list:
            ...
            rs = shell_operator.ssh_exec(ssh, ori_cmd)
            assert rs[3] == 0, "mds install deb fail,error is %s %s" % (
                rs[1], rs[2])
            rm_deb = "rm *%s*" % version
        ...
        for host in config.client_list:
            ...
            rs = shell_operator.ssh_exec(ssh, ori_cmd)
            assert rs[3] == 0, "sdk install deb fail,error is %s %s" % (
                rs[1], rs[2])
            rm_deb = "rm *%s*" % version
            shell_operator.ssh_exec(ssh, rm_deb)
        ...
        for host in config.chunkserver_list:
            ...
            ori_cmd = "sudo dpkg -i --force-overwrite curve-chunkserver*%s* curve-tools*%s* aws-sdk_1.0_amd64.deb" % (
                version, version)
            rs = shell_operator.ssh_exec(ssh, ori_cmd)
            assert rs[3] == 0, "chunkserver install deb fail,error is %s %s" % (
                rs[1], rs[2])
            rm_deb = "rm *%s*" % version
            shell_operator.ssh_exec(ssh, rm_deb)
    except Exception:
...
def add_config_file():
    for host in config.mds_list:
        ...
        rs = shell_operator.ssh_exec(ssh, ori_cmd)
        assert rs[3] == 0, "add host %s config fail,error is %s" % (
            host, rs[2])
    for host in config.chunkserver_list:
        ssh = shell_operator.create_ssh_connect(
            host, 1046, config.abnormal_user)
...
def create_pool():
    ...
    for host in config.chunkserver_list:
        ssh2 = shell_operator.create_ssh_connect(
            host, 1046, config.abnormal_user)
...
def wait_cinder_server_up():
    ...
    ssh = shell_operator.create_ssh_connect(
        cinder_host, 1046, config.abnormal_user)
