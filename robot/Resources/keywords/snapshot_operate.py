...
def delete_curve_file_for_shanpshot():
    curvefs = swig_operate.LibCurve()
    rc = curvefs.libcurve_delete(
        config.snapshot_file_name, config.user_name, config.pass_word)
    if rc != 0:
...
def write_curve_file_for_snapshot(file_name=config.snapshot_file_name, user_name=config.user_name,
    ...
    if rs < 0:
        logger.error(
            "write_curve_file_for_snapshot libcurve_write file fail. rc = %s" % rs)
        return rs
        raise AssertionError
    rc = curvefs.libcurve_close(fd)
    if rc != 0:
        logger.info(
            "write_curve_file_for_snapshot close libcurve file fail. rc = %s" % rc)
    return rc


def read_4k_length_curve_file():
    curvefs = swig_operate.LibCurve()
    fd = curvefs.libcurve_open(
        config.snapshot_file_name, config.user_name, config.pass_word)
...
def snapshot_normal_create(file_name=config.snapshot_file_name, user_name=config.user_name, password=config.pass_word):
    ...
    logger.info(
        "create_curve_file_for_snapshot file and return seq.value = %s" % seq)
    return seq
...
def snapshot_create_with_not_exist_file(file_name="/notexistfile", user_name=config.user_name,
    ...
    if rc != 0:
        logger.error(
            "snapshot_create_with_not_exist_file file fail. rc = %s" % rc)
    return rc
...
def snapshot_create_with_empty_str_file(file_name=" ", user_name=config.user_name,
    ...
    if rc != 0:
        logger.error(
            "snapshot_create_with_empty_str_file file fail. rc = %s" % rc)
    return rc
...
def snapshot_create_with_special_file_name(file_name="/特殊   字符`-=[]\\;',./ ~!@#$%^&*()_+{}|:\"<>?",
    ...
    if rc != 0:
        logger.error(
            "snapshot_create_with_special_file_name file fail. rc = %s" % rc)
    return rc


def get_sanpshot_info(seq):
    client = snapshot_client.CurveSnapshot()
    finfo = client.get_snapshot(
        config.snapshot_file_name, config.user_name, config.pass_word, seq)
...
def create_snapshot_and_get_snapshot_info(file_name=config.snapshot_file_name, user_name=config.user_name,
    ...
    seq = client.create_snapshot(file_name, user_name, password)
    logger.info(
        "create_snapshot_and_get_snapshot_info create snapshot success. seq = %s" % seq.value)
...
def get_normal_snapshot_segment_info(file_name=config.snapshot_file_name, user_name=config.user_name,
    ...
    offset.value = 0
    seginfo = client.get_snapshot_SegmentInfo(
        file_name, user_name, password, seq, offset)
    logger.info(
        "get_normal_snapshot_segment_info seq = %s, seginfo = %s" % seq % seginfo)
    return seginfo
...
def read_chunk_snapshot(idinfo, seq):
    ...
    if rc != len.value:
        logger.info(
            "read_chunk_snapshot fail , expect len = %s, real len = %s" % (len.value, rc))
        return rc
...
def create_clone_chunk_with_s3_object(chunkidinfo):
    ...
    correctseq.value = 0
    rc = client.create_clone_chunk(
        config.snapshot_s3_object_location, idinfo, seq, correctseq, chunksize)
    return rc
