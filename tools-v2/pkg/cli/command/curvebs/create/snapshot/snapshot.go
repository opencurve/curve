...
const (
    dirExample = `$ curve bs create snapshot --snappath /test0@snap0 --user curve`
)
...
func (fCmd *snapshotCommand) RunCommand(cmd *cobra.Command, args []string) error {
    ...
    path := config.GetBsFlagString(fCmd.Cmd, config.CURVEBS_SNAPPATH)
    row[cobrautil.ROW_FILE_NAME] = path
