...
type CopysetCommand struct {
	...
	key2Copyset    *map[uint64]*curveutil.BsCopysetInfoStatus
	Key2LeaderInfo *map[uint64]*fscopyset.CopysetLeaderInfo
	Key2Health     *map[uint64]curveutil.ClUSTER_HEALTH_STATUS
...
func (cCmd *CopysetCommand) Init(cmd *cobra.Command, args []string) error {
	...
	logicalpoolIds, errParse := curveutil.StringList2Uint32List(logicalpoolidList)
	if errParse != nil {
		return fmt.Errorf("parse logicalpoolid%v fail", logicalpoolidList)
	}
	copysetIds, errParse := curveutil.StringList2Uint32List(copysetidList)
	if errParse != nil {
	...
	for i := 0; i < len(logicalpoolIds); i++ {
		...
		key := curveutil.GetCopysetKey(uint64(logicpoolid), uint64(copysetid))
		...
		copysetKey := curveutil.GetCopysetKey(uint64(logicpoolid), uint64(copysetid))
		if cCmd.key2Copyset == nil {
			key2copyset := make(map[uint64]*curveutil.BsCopysetInfoStatus)
			cCmd.key2Copyset = &key2copyset
		}

		(*cCmd.key2Copyset)[copysetKey] = &curveutil.BsCopysetInfoStatus{
	...
	for _, cs := range *cCmd.key2Copyset {
		addr, err := curveutil.PeerAddressToAddr(cs.Info.LeaderPeer.GetAddress())
		if err.TypeCode() != cmderror.CODE_SUCCESS {
...
func (cCmd *CopysetCommand) RunCommand(cmd *cobra.Command, args []string) error {
	key2Health := make(map[uint64]curveutil.ClUSTER_HEALTH_STATUS)
	...
	for k, v := range *cCmd.key2Copyset {
		...
		if v == nil {
			...
			status, errsCheck := curveutil.CheckBsCopySetHealth(v)
			...
			if leaderInfo == nil {
				...
				row[curveutil.ROW_STATUS] = curveutil.CopysetHealthStatus_Str[int32(curveutil.COPYSET_ERROR)]
		...
		if copysetHealthCount[curveutil.COPYSET_NOTEXIST] > 0 || copysetHealthCount[curveutil.COPYSET_ERROR] > 0 {
			(*cCmd.Key2Health)[k] = curveutil.HEALTH_ERROR
		} else if copysetHealthCount[curveutil.COPYSET_WARN] > 0 {
			(*cCmd.Key2Health)[k] = curveutil.HEALTH_WARN
		} else {
			(*cCmd.Key2Health)[k] = curveutil.HEALTH_OK
		}
...
func CheckCopysets(caller *cobra.Command) (*map[uint64]curveutil.ClUSTER_HEALTH_STATUS, *cmderror.CmdError) {
