...
func (cCmd *CopysetCommand) Init(cmd *cobra.Command, args []string) error {
	...
	cCmd.Header = []string{curveutil.ROW_COPYSET_KEY, curveutil.ROW_COPYSET_ID, curveutil.ROW_POOL_ID, curveutil.ROW_LEADER_PEER, curveutil.ROW_EPOCH}

	cCmd.Rows = make([]map[string]string, 0)
	timeout := viper.GetDuration(config.VIPER_GLOBALE_RPCTIMEOUT)
	retrytimes := viper.GetInt32(config.VIPER_GLOBALE_RPCRETRYTIMES)
	getRequest := &topology.GetCopysetsInfoRequest{}
	cCmd.key2Copyset = make(map[uint64]*curveutil.CopysetInfoStatus)
	for i := range poolids {
		poolid := poolids[i]
		...
		key := curveutil.GetCopysetKey(uint64(poolid), uint64(copysetid))
		getRequest.CopysetKeys = append(getRequest.CopysetKeys, &copysetKey)

		row := make(map[string]string)
		row[curveutil.ROW_COPYSET_KEY] = fmt.Sprintf("%d", key)
		row[curveutil.ROW_POOL_ID] = fmt.Sprintf("%d", poolid)
		row[curveutil.ROW_COPYSET_ID] = fmt.Sprintf("%d", copysetid)
		row[curveutil.ROW_LEADER_PEER] = curveutil.ROW_VALUE_DNE
		row[curveutil.ROW_EPOCH] = curveutil.ROW_VALUE_DNE
...
func (cCmd *CopysetCommand) RunCommand(cmd *cobra.Command, args []string) error {
	...
	if err.TypeCode() != cmderror.CODE_SUCCESS {
		...
		for _, copysetValue := range copysetValues {
			...
			if copysetValue.GetStatusCode() != topology.TopoStatusCode_TOPO_COPYSET_NOT_FOUND {
				...
				copysetKey = curveutil.GetCopysetKey(uint64(poolid), uint64(copysetid))
				if cCmd.key2Copyset[copysetKey] == nil {
					cCmd.key2Copyset[copysetKey] = &curveutil.CopysetInfoStatus{
						Info: copysetInfo,
					}
				}
			}
			if copysetValue.GetStatusCode() == topology.TopoStatusCode_TOPO_OK {
				row := slices.IndexFunc(cCmd.Rows, func(row map[string]string) bool {
					return row[curveutil.ROW_COPYSET_KEY] == fmt.Sprintf("%d", copysetKey)
				})
				cCmd.Rows[row][curveutil.ROW_LEADER_PEER] = copysetInfo.GetLeaderPeer().String()
				cCmd.Rows[row][curveutil.ROW_EPOCH] = fmt.Sprintf("%d", copysetInfo.GetEpoch())
			} else {
	...
	indexSlice := curveutil.GetIndexSlice(cCmd.Header, []string{
		curveutil.ROW_POOL_ID, curveutil.ROW_LEADER_PEER,
	})
...
func (cCmd *CopysetCommand) UpdateCopysetsStatus(values []*topology.CopysetValue) []*cmderror.CmdError {
	cCmd.Header = append(cCmd.Header, curveutil.ROW_PEER_ADDR)
	cCmd.Header = append(cCmd.Header, curveutil.ROW_STATUS)
	cCmd.Header = append(cCmd.Header, curveutil.ROW_STATE)
	cCmd.Header = append(cCmd.Header, curveutil.ROW_TERM)
	cCmd.Header = append(cCmd.Header, curveutil.ROW_READONLY)
	cCmd.SetHeader(cCmd.Header)
	for _, row := range cCmd.Rows {
		row[curveutil.ROW_PEER_ADDR] = curveutil.ROW_VALUE_DNE
		row[curveutil.ROW_STATUS] = curveutil.ROW_VALUE_DNE
		row[curveutil.ROW_STATE] = curveutil.ROW_VALUE_DNE
		row[curveutil.ROW_TERM] = curveutil.ROW_VALUE_DNE
		row[curveutil.ROW_READONLY] = curveutil.ROW_VALUE_DNE
	}
	...
	for _, value := range values {
		if value.GetStatusCode() == topology.TopoStatusCode_TOPO_OK {
			...
			for _, peer := range peers {
				addr, err := curveutil.PeerAddressToAddr(peer.GetAddress())
				if err.TypeCode() != cmderror.CODE_SUCCESS {
					ret = append(ret, err)
				...
				if row[curveutil.ROW_PEER_ADDR] != curveutil.ROW_VALUE_DNE {
					rowNew := make(map[string]string)
					rowNew[curveutil.ROW_COPYSET_KEY] = row[curveutil.ROW_COPYSET_KEY]
					rowNew[curveutil.ROW_COPYSET_ID] = row[curveutil.ROW_COPYSET_ID]
					rowNew[curveutil.ROW_POOL_ID] = row[curveutil.ROW_POOL_ID]
					rowNew[curveutil.ROW_LEADER_PEER] = row[curveutil.ROW_LEADER_PEER]
					rowNew[curveutil.ROW_EPOCH] = row[curveutil.ROW_EPOCH]
					rowNew[curveutil.ROW_PEER_ADDR] = addr
					rowNew[curveutil.ROW_STATUS] = curveutil.ROW_VALUE_DNE
					rowNew[curveutil.ROW_STATE] = curveutil.ROW_VALUE_DNE
					rowNew[curveutil.ROW_TERM] = curveutil.ROW_VALUE_DNE
					rowNew[curveutil.ROW_READONLY] = curveutil.ROW_VALUE_DNE
					cCmd.Rows = append(cCmd.Rows, rowNew)
				} else {
					row[curveutil.ROW_PEER_ADDR] = addr
				}
			}
		}
	}
	...
	if len(addr2Request) != 0 {
		...
		for _, result := range results {
			...
			for i := range copysets {
				...
				rowIndex := slices.IndexFunc(cCmd.Rows, func(row map[string]string) bool {
					peerAddr := row[curveutil.ROW_PEER_ADDR]
					addr := result.Addr
					return (row[curveutil.ROW_COPYSET_KEY] == fmt.Sprintf("%d", copysetKey)) && (peerAddr == addr)
				})
				...
				row := cCmd.Rows[rowIndex]
				row[curveutil.ROW_STATUS] = status.GetStatus().String()
				...
				row[curveutil.ROW_STATE] = fmt.Sprintf("%d", copysetStatus.GetState())
				row[curveutil.ROW_TERM] = fmt.Sprintf("%d", copysetStatus.GetTerm())
				row[curveutil.ROW_READONLY] = fmt.Sprintf("%t", copysetStatus.GetReadonly())
			}
		}
	}
	return ret
...
func QueryCopysetInfoStatus(caller *cobra.Command) (*map[uint64]*curveutil.CopysetInfoStatus, *cmderror.CmdError) {
...
func QueryCopysetInfo(caller *cobra.Command) (*map[uint64]*curveutil.CopysetInfoStatus, *cmderror.CmdError) {
	queryCopyset := NewQueryCopysetCommand()
	queryCopyset.Cmd.SetArgs([]string{
		fmt.Sprintf("--%s", config.FORMAT), config.FORMAT_NOOUT,
	})
