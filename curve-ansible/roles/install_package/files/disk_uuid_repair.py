...
def __mount_with_atual_uuid(diskPath, record, actual):
    ...
    try:
        ...
        mntdir = subprocess.check_output(
            cmd, shell=True, stderr=subprocess.STDOUT).replace("\n", "")
    except subprocess.CalledProcessError as e:
    ...
    uuidmd5cmd = "echo uuidmd5=" + \
        hashlib.md5(actual).hexdigest() + " >> " + fileFullName
...
def __handle_inconsistent(umountDisk, record, actual):
    for disk in umountDisk:
        if disk not in record:
            print(
                "record uuid and actual uuid of %s is same, please check other reason" % disk)
            continue
        # Mount according to the actual uuid
        res = __mount_with_atual_uuid(
            "/dev/" + disk, record[disk], actual[disk])
        if res:
            continue
        else:
            return False
